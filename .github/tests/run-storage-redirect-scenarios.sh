  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    ...
    if [ -n "$pid" ]; then
      ...
      if [ -n "$hook_lines" ]; then
        echo "media_provider_hook_ready label=${label} pid=${pid}"
        return 0
      fi
    fi
    sleep_ms 200
