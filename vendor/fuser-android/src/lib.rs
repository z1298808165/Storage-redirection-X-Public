...
/// Mount the given filesystem and invoke `ready` after the mount attempt.
///
/// The callback receives `true` after the filesystem is mounted and the session has completed
/// the kernel handshake, before the blocking session loop starts. It receives `false` if option
/// validation or mounting fails.
///
/// # Errors
/// Returns an error if the options are incorrect, if the fuse device can't be mounted,
/// or if the session loop ends with an error.
pub fn mount2_with_ready<FS: Filesystem, P: AsRef<Path>, F: FnOnce(bool)>(
    filesystem: FS,
    mountpoint: P,
    options: &Config,
    ready: F,
) -> io::Result<()> {
    if let Err(error) = check_option_conflicts(options) {
        ready(false);
        return Err(error);
    }
    match Session::new(filesystem, mountpoint.as_ref(), options) {
        Ok(session) => {
            ready(true);
            session.run()
        }
