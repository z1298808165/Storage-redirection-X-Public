...
(function() {
  ...
  const LICENSES = [
    { group: '模块核心', name: 'SRX Core', license: 'GPL-3.0-or-later', url: 'https://github.com/z1298808165/storage-redirect-x' },
    { group: '模块核心', name: 'Storage-redirection-X-Public', license: 'GPL-3.0-or-later', url: 'https://github.com/Kindness-Kismet/Storage-redirection-X-Public' },
    { group: '模块核心', name: 'srx_hook', license: 'MIT', url: 'https://github.com/Kindness-Kismet/srx_hook' },
    { group: '模块核心', name: 'srx_inline_hook', license: 'MIT', url: 'https://github.com/Kindness-Kismet/srx_inline_hook' },
    { group: '模块核心', name: 'fusefixer', license: 'MIT', url: 'https://github.com/MaterialCleaner/Media-Provider-FuseFixer' },
    { group: 'Root / WebUI', name: 'KernelSU', license: 'GPL 3.0 / GPL 2.0', url: 'https://github.com/tiann/KernelSU' },
    { group: 'Hook 与 DEX', name: 'LSPlant', license: 'LGPL 3.0', url: 'https://github.com/LSPosed/LSPlant' },
    { group: 'Hook 与 DEX', name: 'DexBuilder', license: 'Apache 2.0', url: 'https://android.googlesource.com/platform/tools/dexter' },
    { group: 'Hook 与 DEX', name: 'parallel-hashmap', license: 'Apache 2.0', url: 'https://github.com/greg7mdp/parallel-hashmap' },
    { group: 'Hook 与 DEX', name: 'abseil-cpp', license: 'Apache 2.0', url: 'https://github.com/abseil/abseil-cpp' },
    { group: 'Rust 依赖', name: 'libc', license: 'MIT / Apache 2.0', url: 'https://github.com/rust-lang/libc' },
    { group: 'Rust 依赖', name: 'jni-sys', license: 'MIT / Apache 2.0', url: 'https://github.com/jni-rs/jni-sys' },
    { group: 'Rust 依赖', name: 'serde', license: 'MIT / Apache 2.0', url: 'https://github.com/serde-rs/serde' },
    { group: 'Rust 依赖', name: 'serde_json', license: 'MIT / Apache 2.0', url: 'https://github.com/serde-rs/json' },
    { group: 'Rust 依赖', name: 'once_cell', license: 'MIT / Apache 2.0', url: 'https://github.com/matklad/once_cell' },
    { group: 'Rust 依赖', name: 'log', license: 'MIT / Apache 2.0', url: 'https://github.com/rust-lang/log' },
    { group: 'APP / UI', name: 'AndroidX / Jetpack Compose', license: 'Apache 2.0', url: 'https://github.com/androidx/androidx' },
    { group: 'APP / UI', name: 'Miuix', license: 'Apache 2.0', url: 'https://github.com/compose-miuix-ui/miuix' },
    { group: 'APP / UI', name: 'AppIconLoader', license: 'Apache 2.0', url: 'https://github.com/zhanghai/AppIconLoader' },
    { group: 'APP / Kotlin', name: 'kotlinx.coroutines', license: 'Apache 2.0', url: 'https://github.com/Kotlin/kotlinx.coroutines' },
    { group: 'APP / Kotlin', name: 'kotlinx.serialization', license: 'Apache 2.0', url: 'https://github.com/Kotlin/kotlinx.serialization' },
    { group: 'APP / 系统兼容', name: 'AndroidHiddenApiBypass', license: 'Apache 2.0', url: 'https://github.com/LSPosed/AndroidHiddenApiBypass' }
  ];
  ...
  const DEFAULT_MONITOR_FILTER_OPERATIONS = ['open:read', 'open*:read', 'provider_open:read', 'rename*', 'unlink*', 'delete*', 'rmdir*', 'link*', 'symlink*', 'truncate*', 'ftruncate*', 'chmod*', 'fchmod*', 'utimens*', 'futimens*', 'attrib*'];
  const LEGACY_DEFAULT_MONITOR_FILTER_OPERATIONS = ['open:read', 'rename*', 'unlink*', 'delete*'];
  const LEGACY_FULL_DEFAULT_MONITOR_FILTER_OPERATIONS = ['open:read', 'open*:read', 'rename*', 'unlink*', 'delete*', 'rmdir*', 'link*', 'symlink*', 'truncate*', 'ftruncate*', 'chmod*', 'fchmod*', 'utimens*', 'futimens*', 'attrib*'];
  ...
  function pushModalHistory() {
    ...
    window.history.pushState({ srxPage: State.currentPage, modal: true }, '', '#/' + State.currentPage + '/modal');
  ...
  function setupHistory() {
    ...
    window.addEventListener('popstate', (event) => {
      ...
      if (page === 'app-config' && event.state?.templateEditor && event.state?.templateId) {
        ...
        openTemplateEditor(template || { id: event.state.templateId, name: '', config: { users: { [String(State.currentUserId || '0')]: createDefaultProfile() } } }, {
          skipHistory: true,
      ...
      Theme.navigateTo(page, { preserveScroll: page === 'apps' || page === 'logs', noAnimation: returningToApps || returningToLogs });
      loadPage(page);
    });
  }
  ...
  function setScrollerTopInstant(scroller, top) {
    ...
    if (typeof scroller.scrollTo === 'function') {
      try { scroller.scrollTo({ top: top || 0, behavior: 'instant' }); }
      catch { scroller.scrollTo({ top: top || 0, behavior: 'auto' }); }
    }
  ...
  function initPullRefresh(containerId, scrollerId, onRefresh) {
    ...
    const setText = value => { if (text) text.textContent = value; };
  ...
  function initNav() {
    $$('.nav-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        if (nav?.classList.contains('dragging') || Date.now() < (nav?._suppressClickUntil || 0)) return;
  ...
  function loadPage(page) {
    switch (page) {
      ...
      case 'apps': loadAppList(false); break;
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      setFeatureChipState('monitorStatusChip', !!State.globalConfig.file_monitor_enabled, '文件监控');
      setFeatureChipState('fuseFixStatusChip', State.globalConfig.fuse_fix_enabled !== false, 'FuseFixer');
      setFeatureChipState('verboseLogStatusChip', State.globalConfig.verbose_logging_enabled === true, '详细日志');
      updatePowerButton(status);
      maybeRunStartupUpdateCheck(version);
    } catch { Theme.showToast('加载状态失败', 'error'); }
  }

  function refreshDashboardCountsIfVisible(options) {
    if (State.currentPage !== 'dashboard' || document.hidden) return;
    const now = Date.now();
    if (!options?.force && now - State.dashboardLastCountsRefreshAt < DASHBOARD_REFRESH_THROTTLE_MS) return;
    ...
    const run = async () => {
      try {
        ...
        const [configuredConfigs, monitorLogRaw] = await Promise.all([
          ...
          Api.readStatsCount().then(value => value == null
            ? (Api.readFileTail ? Api.readFileTail(FILE_MONITOR_LOG, FILE_MONITOR_LOG_TAIL_LINES) : Api.readFile(FILE_MONITOR_LOG))
            : String(value))
        ]);
        if (State.currentPage !== requestPage || State.dashboardCountsRequestId !== requestId) return;
  ...
  function renderDashboardPlaceholders() {
    if (!$('#statAppCount')?.textContent || $('#statAppCount')?.textContent === '--') $('#statAppCount').textContent = '0';
    if (!$('#statEnabledAppCount')?.textContent || $('#statEnabledAppCount')?.textContent === '--') $('#statEnabledAppCount').textContent = '0';
    if (!$('#moduleVersionDisplay')?.textContent || $('#moduleVersionDisplay')?.textContent === '--') $('#moduleVersionDisplay').textContent = '...';
  }
  ...
  function renderQuickActions() {
    ...
    items.forEach(item => {
      ...
      el.innerHTML = '<span class="action-item-icon">' + item.icon + '</span><span class="action-item-label">' + item.label + '</span><span class="action-item-arrow">›</span>';
  ...
  function setFeatureChipState(id, enabled, label, stateText) {
    ...
    const labelEl = Array.from(chip.children || []).find(el => !el.classList.contains('status-dot'));
  ...
  function updatePowerButton(status) {
    ...
    btn.setAttribute('aria-label', canToggle ? modulePowerLabel(status) + '，' + btn.title : modulePowerLabel(status));
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (!enabled && verifiedStatus === 'reboot_required') {
          Api.showManagerToast('\u6a21\u5757\u5df2\u542f\u7528\uff0c\u91cd\u542f\u8bbe\u5907\u540e\u751f\u6548');
        } else {
          Api.showManagerToast(enabled ? '模块已停止，MediaProvider 已重启' : '模块已启动，MediaProvider 已重启');
        }
  ...
  async function toggleModuleRuntimeWithLoading(isStopping) {
    const loading = Theme.showLoadingDialog(isStopping ? '正在停止模块并重启 MediaProvider...' : '正在启动模块并重启 MediaProvider...');
    try {
  ...
  function confirmAction(message) {
    return new Promise(resolve => {
      Theme.showDialog(message, () => resolve(true), () => resolve(false));
    });
  }
  ...
  function compareAppsByStatus(a, b) {
    const diff = (appStatusOrder[a.status] ?? appStatusOrder.unconfigured) - (appStatusOrder[b.status] ?? appStatusOrder.unconfigured);
  ...
  async function buildAppListBuckets(installed, configuredState, requestId) {
    ...
    for (let i = 0; i < installed.length; i += 1) {
      ...
      const app = { package: pkg, configured: status !== 'unconfigured', enabled: status === 'enabled', status, label, iconSrc: Api.getAppIconSrc ? Api.getAppIconSrc(pkg) : '', isSystem: info?.isSystem ?? isSystem };
  ...
  async function loadAppList(force, options) {
    ...
    const hasCache = appListCacheUser === userKey && ((appListCache.user && appListCache.user.length) || (appListCache.system && appListCache.system.length));
    ...
    try {
      const [configuredConfigs, users] = await Promise.all([Api.readConfiguredAppConfigs(), Api.listUsers()]);
      ...
      State.availableUsers = Array.from(new Set([...(users || ['0']), ...State.availableUsers, State.currentUserId])).sort((a, b) => Number(a) - Number(b));
      if (!State.availableUsers.includes(State.currentUserId)) State.currentUserId = State.availableUsers[0] || '0';
    ...
    finally { listEl.classList.remove('is-loading'); listEl.setAttribute('aria-busy', 'false'); }
  }

  function renderAppLoadingState() {
    return '<div class="app-loading-list">' + Array.from({ length: 6 }, (_, i) =>
      '<div class="app-item skeleton" aria-hidden="true" style="animation-delay:' + (i * 28) + 'ms">' +
  ...
  function buildConfiguredState(configured, userId) {
    ...
    (configured || []).forEach(item => {
      ...
      const legacyEnabled = Object.prototype.hasOwnProperty.call(cfg, 'enabled') ? cfg.enabled === true : false;
      map.set(item.packageName, (profile ? profile.enabled === true : legacyEnabled) ? 'enabled' : 'disabled');
    });
    return map;
  }
  ...
  function renderAppUserSwitcher() {
    ...
    if (users.length <= 1) { switcher.innerHTML = ''; return; }
    switcher.innerHTML = '<button class="app-user-trigger" id="appUserTrigger" aria-haspopup="menu" aria-expanded="false">U' + escapeHtml(State.currentUserId) + '</button>' +
      '<div class="app-user-menu" role="menu">' + users.map(id => '<button class="app-user-chip' + (id === State.currentUserId ? ' active' : '') + '" data-user="' + escapeHtml(id) + '" role="menuitem">用户 ' + escapeHtml(id) + '</button>').join('') + '</div>';
    switcher.querySelector('#appUserTrigger')?.addEventListener('click', (e) => {
      e.stopPropagation();
      switcher.classList.toggle('open');
      switcher.querySelector('#appUserTrigger')?.setAttribute('aria-expanded', switcher.classList.contains('open') ? 'true' : 'false');
    });
  ...
  function applyFilters() {
    ...
    let apps = [];
    if (currentFilter === 'configured') apps = [...appListCache.user, ...appListCache.system].filter(a => a.configured).sort(compareAppsByStatus);
    else if (currentFilter === 'system') apps = appListCache.system || [];
    else apps = appListCache.user || [];
    if (query) apps = apps.filter(a => a.package.toLowerCase().includes(query) || (a.label && a.label.toLowerCase().includes(query)));
    renderAppList(apps);
  }
  ...
  function renderAppList(apps) {
    ...
    listEl.innerHTML = '';
    if (apps.length === 0) { listEl.innerHTML = '<div class="app-empty">没有找到应用</div>'; State.shouldRestoreAppListScroll = false; exitAppSelectionMode(); return; }
    ...
    const appendChunk = (start) => {
      ...
      else if (State.shouldRestoreAppListScroll) requestAnimationFrame(() => {
        restoreAppListScrollIfNeeded();
        State.shouldRestoreAppListScroll = false;
      });
    };
  ...
  function createAppItem(app) {
      ...
      item.className = 'app-item' + (State.appSelectionMode ? ' selection-mode' : '') + (selected ? ' selected' : '');
	  item.dataset.pkg = app.package;
      const initial = (app.label || app.package)[0].toUpperCase();
      const iconHtml = app.iconSrc
        ? '<div class="app-icon has-image" data-initial="' + escapeHtml(initial) + '"><img class="app-icon-img" src="' + escapeHtml(app.iconSrc) + '" alt="" loading="lazy" onerror="const icon=this.closest(\'.app-icon\'); if (icon) icon.classList.add(\'fallback\'); this.remove();"></div>'
        : '<div class="app-icon fallback" data-initial="' + escapeHtml(initial) + '"></div>';
      item.innerHTML = iconHtml +
        '<div class="app-info"><div class="app-name">' + escapeHtml(app.label || app.package) + '</div><div class="app-package">' + app.package + '</div></div>' +
        (State.appSelectionMode
          ? '<span class="app-select-mark"><span class="icon icon-check" aria-hidden="true"></span></span>'
          : '<span class="app-status ' + statusClass(app.status) + '">' + statusLabel(app.status) + '</span>');
      ...
      item.addEventListener('pointermove', event => {
        if (Math.abs(event.clientX - longPressStartX) > 8 || Math.abs(event.clientY - longPressStartY) > 8) {
          clearLongPress();
        }
      });
  ...
  function toggleAppSelection(packageName) {
    ...
    else { applyFilters(); renderBatchActionBar(); }
  }
  ...
  function renderBatchActionBar() {
    ...
    if (!State.appSelectionMode || !State.selectedApps?.size) { bar?.remove(); return; }
    if (!bar) {
    ...
    bar.innerHTML = '<div class="app-batch-status" aria-label="\u5df2\u9009 ' + State.selectedApps.size + ' \u4e2a\u5e94\u7528"><span class="app-batch-status-count">' + State.selectedApps.size + '</span><span class="app-batch-status-label">\u5df2\u9009</span></div>' +
    ...
    bar.querySelector('#batchApplyTemplate')?.addEventListener('click', () => showTemplatePickerDialog('\u9009\u62e9\u914d\u7f6e\u6a21\u677f', async template => {
      const count = State.selectedApps.size;
      const confirmed = await confirmAction('\u5c06\u6a21\u677f\u201c' + template.name + '\u201d\u5e94\u7528\u5230 ' + count + ' \u4e2a\u5e94\u7528\uff0c\u73b0\u6709\u914d\u7f6e\u4f1a\u88ab\u8986\u76d6\u3002\u662f\u5426\u7ee7\u7eed\uff1f');
      if (!confirmed) return;
    ...
    bar.querySelector('#batchCancel')?.addEventListener('click', () => exitAppSelectionMode({ refreshList: true }));
  }

  function statusClass(status) { return status === 'enabled' ? 'enabled' : status === 'disabled' ? 'disabled' : 'unconfigured'; }
  function statusLabel(status) { return status === 'enabled' ? '已启用' : status === 'disabled' ? '未启用' : '未配置'; }
  ...
  function updateCachedAppConfigured(packageName, configured, enabled) {
    ...
    if (configured && !State.configuredApps.includes(packageName)) State.configuredApps.push(packageName);
    if (!configured) State.configuredApps = State.configuredApps.filter(pkg => pkg !== packageName);
    sortAppCache();
  ...
  function initSearch() {
    const input = $('#appSearchInput'), clearBtn = $('#appSearchClear');
    let timer = null;
  ...
  function initLogSearch() {
    const input = $('#logSearchInput'), clearBtn = $('#logSearchClear');
  ...
  async function openAppConfig(packageName, options) {
    ...
    State.currentApp = packageName; State.currentPage = 'app-config'; Theme.navigateTo('app-config');
    ...
    content.innerHTML = '<div class="loading-state"><div class="spinner"></div><span>加载配置...</span></div>';
    try {
    ...
    catch { bindAppConfigActions(packageName, null, true); content.innerHTML = '<div class="app-empty">加载配置失败</div>'; }
  }
  ...
  function renderAppConfig(packageName, config, options) {
    ...
    let html = '';
    if (isTemplateMode) {
      html += '<div class="config-group template-edit-meta"><div class="switch-row"><div class="switch-label-group"><div class="switch-label">\u6a21\u677f\u540d\u79f0</div><input type="text" class="modal-input template-name-edit" id="templateNameEdit" maxlength="48" autocomplete="off" value="' + escapeHtml(State.templateEditor?.name || '') + '"></div></div></div>';
    }
    html += '<div class="user-tabs">';
    userIds.forEach(id => { html += '<button class="user-tab' + (id === savedUserId ? ' active' : '') + '" data-user="' + id + '">用户 ' + id + '</button>'; });
    html += '</div>';

    html += '<div class="config-group">';
    html += switchRow('启用重定向', 'enable', profile.enabled === true, '开启后将对此应用执行存储重定向');
    html += switchRow('仅映射模式', 'mappingOnly', !!profile.mapping_mode_only, '仅应用显式配置的路径映射，不执行完整隔离重定向');
    const readOnlyEditorKey = appReadOnlyEditorKey(packageName);
    const readOnlyEditorEnabled = (profile.read_only_paths || []).length > 0 || State.readOnlyEditorKeys.has(readOnlyEditorKey);
    html += switchRow('只读模式', 'readOnly', readOnlyEditorEnabled, '禁止写入指定真实目录；默认方案会退化通配规则，FUSE daemon 可精确匹配');
    html += '</div>';

    // Allowed paths, including ! exclude rules.
    const allowRules = getAllowedRules(profile);
    html += '<div class="config-group"><div class="config-group-header"><span class="config-group-title">允许路径</span><button class="icon-btn icon-btn-sm icon-btn-add add-allow-btn" type="button" aria-label="添加允许路径" title="添加允许路径">' + iconHtml('plus') + '</button></div><div class="path-list">';
    allowRules.forEach(p => { html += pathItem(p, p.startsWith('!'), 'allow'); });
    if (!allowRules.length) html += '<div class="app-empty" style="padding:16px;font-size:12px">允许路径可直接访问；! 可排除子路径，* 和 ? 在默认方案下会退化匹配</div>';
    html += '</div></div>';

    if (readOnlyEditorEnabled) {
      html += '<div class="config-group"><div class="config-group-header"><span class="config-group-title">只读路径</span><button class="icon-btn icon-btn-sm icon-btn-add add-readonly-btn" type="button" aria-label="添加只读路径" title="添加只读路径">' + iconHtml('plus') + '</button></div><div class="path-list">';
      (profile.read_only_paths || []).forEach(p => { html += pathItem(p, p.startsWith('!'), 'readonly'); });
      if (!(profile.read_only_paths || []).length) html += '<div class="app-empty" style="padding:16px;font-size:12px">只读路径保持可读但禁止写入；可用 ! 排除子路径，默认方案会退化通配</div>';
      html += '</div></div>';
    }

    // Sandbox paths (mapping only)
    if (!!profile.mapping_mode_only) {
      html += '<div class="config-group config-sandbox-section"><div class="config-group-header"><span class="config-group-title">沙盒路径</span><button class="icon-btn icon-btn-sm icon-btn-add add-sandbox-btn" type="button" aria-label="添加沙盒路径" title="添加沙盒路径">' + iconHtml('plus') + '</button></div><div class="path-list">';
      (profile.sandboxed_paths || []).forEach(p => { html += pathItem(p, false, 'sandbox'); });
      if (!(profile.sandboxed_paths || []).length) html += '<div class="app-empty" style="padding:16px;font-size:12px">仅映射模式下，未命中映射且匹配沙盒路径时将进入应用沙盒</div>';
      html += '</div></div>';
    }

    // Mappings
    html += '<div class="config-group"><div class="config-group-header"><span class="config-group-title">路径映射</span><button class="icon-btn icon-btn-sm icon-btn-add add-mapping-btn" type="button" aria-label="添加路径映射" title="添加路径映射">' + iconHtml('plus') + '</button></div><div class="path-list">';
    const mappings = profile.path_mappings || {};
    const entries = typeof mappings === 'object' && !Array.isArray(mappings) ? Object.entries(mappings) : (Array.isArray(mappings) ? mappings.map(m => [m.request_path, m.final_path]) : []);
    entries.forEach(([req, target]) => { html += mappingItem(req, target); });
    if (!entries.length) html += '<div class="app-empty" style="padding:16px;font-size:12px">将请求路径映射到目标路径</div>';
    html += '</div></div>';
  ...
  function switchRow(label, key, checked, hint) {
    return '<div class="switch-row"><div class="switch-label-group"><div class="switch-label">' + escapeHtml(label) + '</div>' + (hint ? '<div class="switch-hint">' + escapeHtml(hint) + '</div>' : '') + '</div><button class="toggle ' + (checked ? 'on' : '') + '" data-key="' + key + '"></button></div>';
  }
  ...
  function accentSwatchHtml(value) {
    ...
    const style = color ? ' style="--accent-icon:' + color + ';--accent-icon-bg:' + hexToRgba(color, 0.14) + '"' : '';
    return '<span class="setting-accent-pen' + (color ? '' : ' default') + '"' + style + '><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4.5 19.5l3.8-.9L18.9 8l-2.9-2.9L5.4 15.7l-.9 3.8z"></path><path d="M14.9 6.2l2.9 2.9"></path><path d="M5.4 15.7l2.9 2.9"></path><path d="M17 4.1l1.1-1.1a1.4 1.4 0 0 1 2 0l.9.9a1.4 1.4 0 0 1 0 2l-1.1 1.1"></path></svg></span>';
  }
  function clampWebUiPageScale(value) {
    const scale = Number(value);
    return Number.isFinite(scale) ? Math.max(WEBUI_PAGE_SCALE_MIN, Math.min(WEBUI_PAGE_SCALE_MAX, scale)) : 1;
  }
  ...
  function settingSelectRow(label, key, value, hint, leadingHtml) {
    return '<button class="setting-select-row" type="button" data-key="' + key + '">' +
      (leadingHtml || '') +
      '<span class="switch-label-group"><span class="switch-label">' + label + '</span>' + (hint ? '<span class="switch-hint">' + hint + '</span>' : '') + '</span>' +
      '<span class="setting-select-value">' + escapeHtml(value) + '</span>' +
  ...
  function showSettingOptionDialog(title, options, selected, onPick) {
    const rows = options.map(option => {
      ...
      const swatch = Object.prototype.hasOwnProperty.call(option, 'color') ? accentSwatchHtml(option.value) : '';
      return '<button class="setting-option-row' + selectedClass + '" type="button" data-value="' + escapeHtml(value) + '">' +
        swatch +
        '<span class="setting-option-label">' + escapeHtml(option.label) + '</span>' +
        (option.value === selected ? '<span class="auto-template-selected">✓</span>' : '') +
        '</button>';
    }).join('');
    showModalWithHistory(
      '<div class="modal-title">' + escapeHtml(title) + '</div>' +
      '<div class="setting-option-list">' + rows + '</div>',
      { backdropClose: true }
    );
  ...
  function showPageScaleDialog() {
    ...
    showModalWithHistory(
      ...
      '<input class="modal-input" id="pageScaleInput" type="number" min="80" max="110" step="1" inputmode="numeric" value="' + currentPercent + '">' +
    ...
    const updateHint = () => {
      ...
      if (hint) {
        hint.textContent = clamped === percent
          ? '当前为 ' + clamped + '%'
          : '将保存为 ' + clamped + '%';
      }
      return clamped;
  ...
  function normalizeGlobalRuntimeConfig(raw) {
    ...
    return {
      ...
      auto_enable_new_apps_template_id: isSafeTemplateId(source.auto_enable_new_apps_template_id) ? String(source.auto_enable_new_apps_template_id) : '',
  ...
  function autoRedirectTemplateStatusHtml(globalConfig, templates, fallbackNoticeId) {
    ...
    const templateId = isSafeTemplateId(globalConfig.auto_enable_new_apps_template_id) ? String(globalConfig.auto_enable_new_apps_template_id) : '';
    ...
    const hint = template
      ? '使用模板：' + template.name
      : (missingTemplate ? '模板已失效，已回退为仅开启重定向' : '仅开启重定向，无其它规则配置');
    return '<button class="auto-template-row' + (missingTemplate ? ' is-missing' : '') + '" id="autoTemplateRow" type="button">' +
      '<span class="auto-template-icon"><span class="icon icon-template" aria-hidden="true"></span></span>' +
      '<span class="auto-template-main"><span class="auto-template-title">自动配置模板</span><span class="auto-template-hint">' + escapeHtml(hint) + '</span></span>' +
      '<span class="auto-template-chevron" aria-hidden="true"><span class="icon icon-chevron-down"></span></span>' +
      '</button>';
  }
  function autoTemplateDefaultRowHtml(selected) {
    return '<div class="template-row auto-template-default' + (selected ? ' selected' : '') + '" data-template-id=""><span class="template-row-icon"><span class="icon icon-check" aria-hidden="true"></span></span><div class="template-row-main"><div class="template-row-title">仅开启重定向</div><div class="template-row-subtitle">不附加允许路径、沙盒路径或映射规则</div></div>' + (selected ? '<span class="auto-template-selected">✓</span>' : '') + '</div>';
  }
  function autoTemplateChoiceRowHtml(template, selected) {
    return '<div class="template-row' + (selected ? ' selected' : '') + '" data-template-id="' + escapeHtml(template.id) + '"><span class="template-row-icon"><span class="icon icon-template" aria-hidden="true"></span></span><div class="template-row-main"><div class="template-row-title">' + escapeHtml(template.name) + '</div><div class="template-row-subtitle">' + escapeHtml(templateSummary(template.config)) + '</div></div>' + (selected ? '<span class="auto-template-selected">✓</span>' : '') + '</div>';
  }
  async function showAutoTemplatePickerDialog(currentTemplateId, onPick) {
    ...
    const currentExists = !!safeCurrent && templates.some(template => template.id === safeCurrent);
    const rows = [
      autoTemplateDefaultRowHtml(!safeCurrent || !currentExists),
      ...templates.map(template => autoTemplateChoiceRowHtml(template, template.id === safeCurrent))
    ].join('');
    const emptyHint = templates.length ? '' : '<div class="modal-hint">模板库为空，当前只能选择仅开启重定向。</div>';
    showModalWithHistory(
      '<div class="modal-title">新应用默认配置</div>' +
      '<div class="modal-hint">选择一个模板后，新安装应用会自动使用该模板生成配置。</div>' +
      '<div class="template-card template-list-scroll template-picker-list auto-template-picker">' + rows + '</div>' +
      emptyHint,
      { backdropClose: true }
    );
  ...
  function saveUpdatePrefs(prefs) {
    ...
    try { localStorage.setItem(UPDATE_PREFS_KEY, JSON.stringify(normalized)); } catch {}
    return normalized;
  }
  ...
  function openExternalUrl(url) {
    ...
    try { window.location.href = target; return true; } catch {}
    return false;
  }

  function repositoryCardHtml(title, url, id) {
    return '<button class="repository-card" id="' + id + '" type="button">' +
      '<span class="repository-card-title">' + escapeHtml(title) + '</span>' +
      '<span class="repository-card-url">' + escapeHtml(url) + '</span>' +
      '</button>';
  }
  ...
  async function loadUpdate() {
    ...
    const releaseUrl = Api.getReleaseRepositoryUrl ? Api.getReleaseRepositoryUrl() : 'https://github.com/z1298808165/Storage-redirection-X-Public';
    const officialUrl = Api.getOfficialReleaseRepositoryUrl ? Api.getOfficialReleaseRepositoryUrl() : releaseUrl;
    content.innerHTML =
      '<div class="section update-settings-section"><h2 class="section-title">更新</h2>' +
        '<div class="config-group update-settings-card">' +
          switchRow('启动时检查更新', 'autoCheckUpdates', prefs.autoCheckUpdates, '打开 WebUI 后自动检查所选通道的新版本') +
          settingSelectRow('更新通道', 'updateChannel', updateChannelLabel(prefs.updateChannel), '选择正式版、测试版或所有通道中的最新版本') +
        '</div></div>' +
      '<div class="update-current-card">' +
        '<div class="update-current-version">当前模块版本 ' + escapeHtml(moduleVersion || '--') + '</div>' +
        '<button class="btn btn-primary btn-block" id="updateCheckNow" type="button"' + (State.updateCheckRunning ? ' disabled' : '') + '>' + (State.updateCheckRunning ? '正在检查' : '立即检查') + '</button>' +
      '</div>' +
    ...
    content.querySelector('.toggle[data-key="autoCheckUpdates"]')?.addEventListener('click', event => {
      const toggle = event.currentTarget;
      toggle.classList.toggle('on');
      saveUpdatePrefs(Object.assign({}, updatePrefs(), { autoCheckUpdates: toggle.classList.contains('on') }));
      Theme.showToast('设置已保存', 'success');
    });
    content.querySelector('.setting-select-row[data-key="updateChannel"]')?.addEventListener('click', () => {
      showSettingOptionDialog('更新通道', UPDATE_CHANNEL_OPTIONS, updatePrefs().updateChannel, value => {
        saveUpdatePrefs(Object.assign({}, updatePrefs(), { updateChannel: value }));
        loadUpdate();
      });
    });
    content.querySelector('#updateCheckNow')?.addEventListener('click', () => handleUpdateCheck(true, { currentVersionName: moduleVersion }));
    content.querySelector('#releaseRepositoryBtn')?.addEventListener('click', () => openExternalUrl(releaseUrl));
    content.querySelector('#officialRepositoryBtn')?.addEventListener('click', () => openExternalUrl(officialUrl));
  }

  function maybeRunStartupUpdateCheck(moduleVersion) {
    const prefs = updatePrefs();
    if (State.startupUpdateCheckDone || !prefs.autoCheckUpdates || !String(moduleVersion || '').trim()) return;
  ...
  function showUpdateFoundDialog(update, currentVersionName) {
    showModalWithHistory(
      ...
      '<div class="update-dialog-summary">当前模块版本 ' + escapeHtml(currentVersionName || '--') + '，发现 ' + escapeHtml(update.title || update.tagName || '新版本') + '。</div>' +
      '<div class="update-dialog-meta"><span>' + escapeHtml(updateChannelBadge(update.channel, update.prerelease)) + '</span>' + (update.tagName ? '<span>' + escapeHtml(update.tagName) + '</span>' : '') + '</div>' +
  ...
  function hiddenFileInputHtml(id, accept) {
    return '<input class="hidden-file-input" id="' + id + '" type="file" accept="' + escapeHtml(accept || '') + '" aria-hidden="true" tabindex="-1">';
  }
  function pathItem(path, isExcluded, type) {
    const text = isExcluded && !String(path).startsWith('!') ? '!' + path : path;
    return '<div class="path-item' + (isExcluded ? ' excluded' : '') + '" data-path="' + escapeHtml(text) + '" data-type="' + escapeHtml(type || 'allow') + '"><span class="path-item-text">' + escapeHtml(text) + '</span><button class="icon-btn icon-btn-sm icon-btn-danger path-item-delete" type="button" data-path="' + escapeHtml(text) + '" aria-label="移除路径" title="移除路径">' + iconHtml('x') + '</button></div>';
  }
  function mappingItem(req, target) {
    return '<div class="mapping-item" data-request="' + escapeHtml(req) + '" data-target="' + escapeHtml(target) + '"><span class="mapping-request">' + escapeHtml(req) + '</span><span class="mapping-arrow">→</span><span class="mapping-target">' + escapeHtml(target) + '</span><button class="icon-btn icon-btn-sm icon-btn-danger path-item-delete mapping-delete" type="button" data-request="' + escapeHtml(req) + '" aria-label="移除映射" title="移除映射">' + iconHtml('x') + '</button></div>';
  }

  function bindConfigEvents(packageName, users) {
    const content = $('#appConfigContent');
    content.querySelectorAll('.user-tab[data-user]').forEach(tab => {
      tab.addEventListener('click', () => { collectCurrentProfile(users); saveAppConfigIfAuto(packageName, users); State.configUserId = tab.dataset.user; renderAppConfig(packageName, { users }, isTemplateEditorActive() ? { mode: 'template' } : undefined); });
    });
    content.querySelectorAll('.toggle').forEach(toggle => {
      toggle.addEventListener('click', () => {
        ...
        if (toggle.dataset.key === 'readOnly') {
          ...
          renderAppConfig(packageName, { users }, isTemplateEditorActive() ? { mode: 'template' } : undefined);
    ...
    content.querySelector('.add-allow-btn')?.addEventListener('click', () => showAddPathDialog(packageName, users, 'allow'));
    content.querySelector('.add-readonly-btn')?.addEventListener('click', () => showAddPathDialog(packageName, users, 'readonly'));
    content.querySelector('.add-sandbox-btn')?.addEventListener('click', () => showAddPathDialog(packageName, users, 'sandbox'));
    content.querySelector('.add-mapping-btn')?.addEventListener('click', () => showAddMappingDialog(packageName, users));
    ...
    content.querySelectorAll('.mapping-delete').forEach(btn => {
      btn.addEventListener('click', (e) => { e.stopPropagation(); Theme.confirmDelete('删除映射 "' + btn.dataset.request + '"？', () => { deleteMapping(users, btn.dataset.request); renderAppConfig(packageName, { users }, isTemplateEditorActive() ? { mode: 'template' } : undefined); saveAppConfigIfAuto(packageName, users); }); });
    });
    content.querySelectorAll('.path-item').forEach(item => {
      item.addEventListener('click', () => showAddPathDialog(packageName, users, item.dataset.type || 'allow', { originalPath: item.dataset.path || '' }));
    });
    content.querySelectorAll('.mapping-item').forEach(item => {
      item.addEventListener('click', () => showAddMappingDialog(packageName, users, { originalRequest: item.dataset.request || '', originalTarget: item.dataset.target || '' }));
    });
  ...
  async function saveTemplateEditor(users, options) {
    ...
    const ok = await upsertTemplate({ id: State.templateEditor.id, name, config: { users: snapshot } });
    if (ok) {
      State.templateEditor.name = name;
      $('#configAppName').textContent = name;
      Theme.showToast(options?.auto ? '\u6a21\u677f\u5df2\u81ea\u52a8\u4fdd\u5b58' : '\u6a21\u677f\u5df2\u4fdd\u5b58', 'success');
  ...
  function bindAppConfigActions(packageName, users, disabled) {
    ...
    if (saveBtn) {
      ...
      saveBtn.onclick = users && !autoSaveEnabled ? async () => {
    ...
    if (deleteBtn) {
      ...
      if (users && isTemplateMode) {
        deleteBtn.onclick = async () => {
          ...
          Theme.confirmDelete('\u5220\u9664\u6a21\u677f\u201c' + (State.templateEditor?.name || '') + '\u201d\uff1f', async () => {
            const ok = await Api.deleteTemplate(State.templateEditor.id);
            if (ok) {
              State.templates = State.templates.filter(item => item.id !== State.templateEditor.id);
        ...
        deleteBtn.onclick = users ? () => {
          Theme.confirmDelete('\u786e\u5b9a\u5220\u9664 \"' + packageName + '\" \u7684\u5168\u90e8\u914d\u7f6e\uff1f', async () => { const ok = await Api.deleteAppConfig(packageName); if (ok) { updateCachedAppConfigured(packageName, false, false); Theme.showToast('\u914d\u7f6e\u5df2\u5220\u9664', 'success'); State.currentPage = 'apps'; State.shouldRestoreAppListScroll = true; Theme.navigateTo('apps', { preserveScroll: true }); loadAppList(false); } else Theme.showToast('\u5220\u9664\u5931\u8d25', 'error'); });
        } : null;
      }
    }
    if (templateBtn) {
      ...
      templateBtn.onclick = users && !isTemplateMode ? () => showAppTemplateActions(packageName, users) : null;
    }
  }
  ...
  function showAppTemplateActions(packageName, users) {
    ...
    document.getElementById('applyTemplateBtn')?.addEventListener('click', () => {
      ...
      showTemplatePickerDialog('应用配置模板', async template => {
        const confirmed = await confirmAction('将模板“' + template.name + '”应用到当前应用，现有配置会被覆盖。是否继续？');
        if (!confirmed) return;
  ...
  function createDefaultProfile(options) {
    const enabledDefault = options && Object.prototype.hasOwnProperty.call(options, 'enabledDefault') ? options.enabledDefault : false;
    return { enabled: enabledDefault, mapping_mode_only: false, allowed_real_paths: [], excluded_real_paths: [], sandboxed_paths: [], read_only_paths: [], path_mappings: {} };
  }
  function normalizeUsersConfig(users) {
    ...
    Object.keys(users || {}).forEach(id => {
      ...
      profile.allowed_real_paths = Array.isArray(profile.allowed_real_paths) ? profile.allowed_real_paths : [];
      profile.excluded_real_paths = Array.isArray(profile.excluded_real_paths) ? profile.excluded_real_paths : [];
      const readOnlyPaths = normalizeReadOnlyRules(profile.read_only_paths);
      profile.allowed_real_paths = sortPathRules(removeConflictingAllowedRules(mergeAllowedRules(profile.allowed_real_paths, profile.excluded_real_paths)));
      profile.excluded_real_paths = [];
      profile.read_only_paths = normalizeReadOnlyForRules(readOnlyPaths, profile.allowed_real_paths);
      profile.sandboxed_paths = sortPathRules(Array.isArray(profile.sandboxed_paths) ? profile.sandboxed_paths : []);
      if (!profile.path_mappings || typeof profile.path_mappings !== 'object') profile.path_mappings = {};
      profile.path_mappings = sortPathMappings(profile.path_mappings);
      out[id] = profile;
    });
    return out;
  }
  function getActiveConfigUserId() { return State.configUserId || State.currentUserId; }
  function getProfile(users) { return users[getActiveConfigUserId()] || createDefaultProfile(); }
  function appReadOnlyEditorKey(packageName) {
    const owner = isTemplateEditorActive() ? 'template:' + (State.templateEditor?.id || packageName || '') : 'app:' + (packageName || '');
    return owner + ':user:' + getActiveConfigUserId();
  }

  function getAllowedRules(profile) {
    return sortPathRules(removeConflictingAllowedRules(mergeAllowedRules(profile.allowed_real_paths || [], profile.excluded_real_paths || [])));
  }
  function mergeAllowedRules(allowed, excluded) {
    const seen = new Set();
    const rules = [];
    [...(allowed || []), ...(excluded || []).map(path => String(path).startsWith('!') ? path : '!' + path)].forEach(path => {
  ...
  function removeConflictingAllowedRules(rules) {
    ...
    return (rules || []).filter(rule => String(rule || '').startsWith('!') || !conflicts.has(String(rule || '')));
  }
  ...
  function normalizeReadOnlyForRules(readOnlyPaths, allowedRules) {
    const excluded = new Set((allowedRules || []).filter(rule => String(rule || '').startsWith('!')).map(rule => String(rule).slice(1)));
    return sortPathRules((readOnlyPaths || []).filter(path => {
  ...
  function comparePathNames(a, b) {
    ...
    return left.localeCompare(right, 'en', { numeric: true, sensitivity: 'base' }) || String(a).localeCompare(String(b), 'en');
  }
  ...
  function deleteMapping(users, req) {
    const p = getProfile(users); if (p.path_mappings) delete p.path_mappings[req]; users[getActiveConfigUserId()] = p;
  }
  ...
  function validatePath(raw, options) {
    ...
    if (!allowRuleSyntax && trimmed.startsWith('!')) return { valid: false, msg: '该路径不支持 ! 前缀' };
    ...
    if (pathPart.split('/').some(part => part === '.' || part === '..')) return { valid: false, msg: '路径不能包含 . 或 ..' };
  ...
  function validateMonitorFilterPath(raw, options) {
    ...
    if (hasMonitorFilterStorageRootPrefix(value)) return { valid: false, value: '', msg: '不能带存储根目录，请输入相对路径' };
    if (value.startsWith('/')) {
      if (!allowLegacyAbsolute) return { valid: false, value: '', msg: '不能使用绝对路径，请输入相对路径' };
      value = value.replace(/^\/+/, '');
    }
    value = value.replace(/^\/+|\/+$/g, '');
    if (!value) return { valid: false, value: '', msg: '路径不能为空' };
    if (value.startsWith('!')) return { valid: false, value: '', msg: '过滤路径不支持排除前缀' };
    if (value.includes('\0') || value.length > 512) return { valid: false, value: '', msg: '路径格式不正确' };
    if (value.split('/').some(part => part === '.' || part === '..')) return { valid: false, value: '', msg: '路径不能包含 . 或 ..' };
    if (hasMonitorFilterStorageRootPrefix(value)) return { valid: false, value: '', msg: '不能带存储根目录，请输入相对路径' };
  ...
  function isLegacyDefaultMonitorOperations(list) {
    return isSameStringSet(list, LEGACY_DEFAULT_MONITOR_FILTER_OPERATIONS) ||
  ...
  function showPathValidation(inputEl, hintEl, options) {
    ...
    if (!inputEl.value.trim()) { hintEl.textContent = ''; inputEl.classList.remove('invalid'); return; }
    hintEl.textContent = result.msg;
    hintEl.className = 'path-validation ' + (result.valid ? 'valid' : 'invalid');
    inputEl.classList.toggle('invalid', !result.valid);
    return result.valid;
  ...
  function buildPathBrowser(basePath, inputEl, hintEl, onSelect, options) {
    ...
    const normalizeRelative = (value) => value.trim().replace(/^!/, '').replace(/^\/+/, '').replace(/\/+/g, '/');
    ...
    const renderContents = (dir, contents) => {
      const signature = [dir, currentPrefix, currentQuery, contents.dirs.join('\0'), contents.files.join('\0')].join('\n');
      ...
      let itemCount = 0;
        if (dir !== basePath) {
          ...
          up.addEventListener('click', () => {
            ...
            const relative = parent === basePath ? '' : parent.substring(basePath.length + 1).replace(/\/?$/, '/');
        ...
        if (!directoriesOnly) contents.files.forEach(name => {
          if (currentQuery && !name.toLowerCase().includes(currentQuery)) return;
          const item = document.createElement('div');
    ...
    const loadDir = async (dir) => {
      ...
      renderedSignature = '';
      if (!container.children.length) container.innerHTML = '<div class="path-browser-loading">加载目录...</div>';
      try {
      ...
      } catch { container.innerHTML = '<div class="path-browser-loading">无法读取目录</div>'; }
    };

    const loadForInput = (value) => {
      clearTimeout(loadTimer);
      currentRulePrefix = validateOptions?.allowRuleSyntax && value.trim().startsWith('!') ? '!' : '';
  ...
  function showAddPathDialog(packageName, users, type, editOptions) {
    ...
    const validateOptions = type === 'allow'
      ? { allowRuleSyntax: true }
      : (type === 'readonly' ? { allowRuleSyntax: true, allowWildcards: true } : null);
    const actionButton = '<button class="btn btn-primary" id="modalAddBtn" type="button">' + (isEdit ? '保存修改' : '添加') + '</button>';

    showModalWithHistory(
      '<div class="modal-title">' + (isEdit ? '修改路径' : titles[type]) + '</div>' +
      '<input type="text" class="modal-input" id="pathInput" placeholder="相对路径，如 Pictures/Screenshots" autocomplete="off" value="' + escapeHtml(originalPath) + '">' +
      '<div class="path-validation" id="pathValidation"></div>' +
      '<div id="pathBrowserContainer"></div>' +
      '<div class="modal-hint">' + hints[type] + '相对路径以 ' + storageBase + ' 为根。</div>' +
      '<div class="modal-actions"><button class="btn btn-secondary modal-close" type="button">取消</button>' + actionButton + '</div>'
    );
    ...
    // Path browser
    const browser = buildPathBrowser(storageBase, input, hintEl, null, { validateOptions, autoLoad: true });
    ...
    $('#modalAddBtn')?.addEventListener('click', () => {
      ...
      if (!validatePath(rawPath, validateOptions).valid) { Theme.showToast('路径格式不正确', 'error'); return; }
      const p = getProfile(users);
      const targetKey = type === 'sandbox' ? 'sandboxed_paths' : (type === 'readonly' ? 'read_only_paths' : 'allowed_real_paths');
      p[targetKey] = p[targetKey] || [];
      if (isEdit) p[targetKey] = p[targetKey].filter(item => item !== originalPath);
      if (p[targetKey].includes(rawPath)) { Theme.showToast('路径已存在', 'error'); return; }
  ...
  function showAddMappingDialog(packageName, users, editOptions) {
    ...
    const storageBase = '/storage/emulated/' + getActiveConfigUserId();
    const actionButton = '<button class="btn btn-primary" id="modalAddMapping" type="button">' + (isEdit ? '保存修改' : '添加') + '</button>';
    const targetValidateOptions = { disallowPrivateAndroidTarget: true };

    showModalWithHistory(
      '<div class="modal-title">' + (isEdit ? '修改路径映射' : '添加路径映射') + '</div>' +
      '<input type="text" class="modal-input" id="mappingRequest" placeholder="请求路径，如 DCIM/Camera" autocomplete="off" style="margin-bottom:8px" value="' + escapeHtml(originalRequest) + '">' +
      '<div class="path-validation" id="reqValidation"></div>' +
      '<div id="reqBrowserContainer"></div>' +
      '<input type="text" class="modal-input" id="mappingTarget" placeholder="目标路径，如 Pictures/CameraBackup" autocomplete="off" style="margin-top:12px" value="' + escapeHtml(originalTarget) + '">' +
      '<div class="path-validation" id="targetValidation"></div>' +
      '<div id="targetBrowserContainer"></div>' +
      '<div class="modal-hint">路径匹配时重定向。相对于 ' + storageBase + '。</div>' +
      '<div class="modal-actions"><button class="btn btn-secondary modal-close" type="button">取消</button>' + actionButton + '</div>'
    );
    ...
    const targetBrowser = buildPathBrowser(storageBase, targetInput, targetHint, null, { validateOptions: targetValidateOptions });
    ...
    // Validation
    let reqTimer = null, targetTimer = null;
    reqInput?.addEventListener('input', () => { clearTimeout(reqTimer); reqTimer = setTimeout(() => showPathValidation(reqInput, reqHint), 180); activateBrowser('request'); });
    targetInput?.addEventListener('input', () => { clearTimeout(targetTimer); targetTimer = setTimeout(() => showPathValidation(targetInput, targetHint, targetValidateOptions), 180); activateBrowser('target'); });

    $('#modalAddMapping')?.addEventListener('click', () => {
      const req = reqInput.value.trim(), target = targetInput.value.trim();
      if (!validatePath(req).valid) { Theme.showToast('请求路径格式不正确', 'error'); return; }
      const targetValidation = validatePath(target, targetValidateOptions);
      if (!targetValidation.valid) { Theme.showToast(targetValidation.msg || '目标路径格式不正确', 'error'); return; }
      if (req === target) { Theme.showToast('路径不能相同', 'error'); return; }
      const p = getProfile(users); p.path_mappings = p.path_mappings || {};
      if (isEdit && originalRequest !== req) delete p.path_mappings[originalRequest];
      if ((!isEdit || originalRequest !== req) && Object.prototype.hasOwnProperty.call(p.path_mappings, req)) { Theme.showToast('请求路径已存在', 'error'); return; }
  ...
  async function loadSettings() {
    ...
    try {
      ...
      const selectedTemplateId = normalizedGlobal.auto_enable_redirect_for_new_apps ? normalizedGlobal.auto_enable_new_apps_template_id : '';
      if (selectedTemplateId && !templates.some(template => template.id === selectedTemplateId)) {
        const fallbackGlobal = normalizeGlobalRuntimeConfig(Object.assign({}, normalizedGlobal, {
      ...
      const themeRows =
        switchRow('动态取色', 'dynamicColor', dynamicColor, '开启后使用 WebUI 主题色系统；默认沿用当前主题色，也可指定强调色') +
        (dynamicColor
          ? settingSelectRow('强调色', 'accentColor', accentColorLabel(accentColor), '默认使用 WebUI 当前主题色，也可指定应用主题色', accentSwatchHtml(accentColor))
          : '') +
        (dynamicColor && accentColor !== 0
          ? settingSelectRow('色彩风格', 'colorStyle', optionLabel(WEBUI_COLOR_STYLE_OPTIONS, uiOptions.colorStyle, uiOptions.colorStyle), '控制强调色生成主题变量时的倾向') +
            settingSelectRow('色彩标准', 'colorSpec', optionLabel(WEBUI_COLOR_SPEC_OPTIONS, uiOptions.colorSpec, uiOptions.colorSpec), '选择 WebUI 主题色生成标准')
          : '');
      const templateSection = '<div class="section template-section" style="margin-top:16px"><h2 class="section-title">配置模板</h2>' +
        '<div class="template-card"><div class="config-group-header"><span class="config-group-title">模板库</span><button class="icon-btn icon-btn-sm icon-btn-add" id="templateAddBtn" type="button" aria-label="添加配置模板" title="添加配置模板">' + iconHtml('plus') + '</button></div>' +
        '<div class="template-list-scroll">' +
        (templates.length ? templates.map(t => templateRowHtml(t, { manage: true })).join('') : '<div class="app-empty" style="padding:18px;font-size:12px">还没有配置模板，可从应用配置页保存当前配置为模板</div>') +
        '</div></div></div>';
      content.innerHTML =
        '<div class="config-group">' +
          switchRow('文件监视', 'fileMonitor', !!State.globalConfig.file_monitor_enabled, '记录已配置普通应用和系统代写进程的文件创建操作，普通应用由 daemon 外部监控') +
          switchRow('Fuse Fixer', 'fuseFix', State.globalConfig.fuse_fix_enabled !== false, 'SRX 内置 Fuse Fixer 兼容保护，处理特殊 Unicode 字符') +
          switchRow('详细日志', 'verboseLogging', State.globalConfig.verbose_logging_enabled === true, '开启后立即记录 Rust、Java、Stats 和诊断采集日志') +
          switchRow('新应用自动重定向', 'autoEnableNewApps', State.globalConfig.auto_enable_redirect_for_new_apps === true, '收到系统新安装事件后，自动写入默认配置') +
          autoRedirectTemplateStatusHtml(State.globalConfig, templates, State.autoTemplateFallbackNoticeId) +
          switchRow('配置操作即时保存', 'appConfigAutoSave', State.globalConfig.app_config_auto_save === true, '开启后应用配置页的修改会在每个操作结束后自动保存') +
        '</div>' +
        '<div class="section experiment-settings-section" style="margin-top:16px"><h2 class="section-title">实验区</h2>' +
          '<div class="theme-settings-card"><div class="theme-settings-rows">' +
            switchRow('Fuse daemon', 'fuseDaemonRedirect', State.globalConfig.fuse_daemon_redirect_enabled === true, '仅在普通应用的通配规则前缀启用 scoped FUSE，精确处理 !、*、?；普通路径继续使用 mount namespace。可提升复杂规则准确性，但通配前缀内的高频读写会多一层用户态转发。') +
          '</div></div></div>' +
        '<div class="section theme-settings-section" style="margin-top:16px"><h2 class="section-title">主题</h2>' +
          ...
          '<div class="theme-selector">' +
            '<label class="theme-option' + (currentTheme === 'light' ? ' active' : '') + '"><input type="radio" name="theme" value="light" ' + (currentTheme === 'light' ? 'checked' : '') + '><span class="theme-option-label">浅色</span></label>' +
            '<label class="theme-option' + (currentTheme === 'dark' ? ' active' : '') + '"><input type="radio" name="theme" value="dark" ' + (currentTheme === 'dark' ? 'checked' : '') + '><span class="theme-option-label">深色</span></label>' +
            '<label class="theme-option' + (currentTheme === 'system' ? ' active' : '') + '"><input type="radio" name="theme" value="system" ' + (currentTheme === 'system' ? 'checked' : '') + '><span class="theme-option-label">跟随系统</span></label>' +
          '</div>' +
        ...
        '<div class="section visual-settings-section" style="margin-top:16px"><h2 class="section-title">视觉效果</h2>' +
          '<div class="theme-settings-card">' +
          '<div class="theme-settings-rows">' +
            settingSelectRow('界面缩放', 'pageScale', webUiPageScaleLabel(uiOptions.pageScale), '调整 WebUI 页面密度，范围 80% - 110%') +
            switchRow('悬浮底栏', 'floatingNav', uiOptions.floatingNav !== false, '启用底部导航与配置操作栏的悬浮容器形态') +
            switchRow('液态玻璃特效', 'liquidGlass', uiOptions.liquidGlass !== false, '启用全局实时背景模糊和液态水滴动效') +
            switchRow('背景模糊效果', 'blurEffect', uiOptions.blurEffect !== false, '关闭后所有弹窗、提示和浮动面板改用实色表面') +
          '</div></div></div>' +
        templateSection +
        '<div class="section backup-restore-section" style="margin-top:16px"><h2 class="section-title">备份还原</h2>' +
          '<div class="backup-restore-card">' +
            ...
            hiddenFileInputHtml('backupRestoreFile', '.srxbak.zip,.srxbak.json,application/zip,application/json') +
          '</div></div>' +
      ...
      content.querySelectorAll('input[name="theme"]').forEach(radio => {
        radio.addEventListener('change', () => { Theme.apply(radio.value); content.querySelectorAll('.theme-option').forEach(o => o.classList.remove('active')); radio.closest('.theme-option')?.classList.add('active'); });
      });
      content.querySelectorAll('.toggle').forEach(toggle => {
        toggle.addEventListener('click', async () => {
          ...
          if (key === 'floatingNav' || key === 'liquidGlass' || key === 'blurEffect' || key === 'dynamicColor') {
      ...
      content.querySelector('.setting-select-row[data-key="accentColor"]')?.addEventListener('click', () => {
        showSettingOptionDialog('强调色', WEBUI_ACCENT_COLOR_OPTIONS, Number(Theme.getUiOption('accentColor')) || 0, value => {
          Theme.setUiOption('accentColor', value);
          loadSettings();
        });
      });
      content.querySelector('.setting-select-row[data-key="colorStyle"]')?.addEventListener('click', () => {
        showSettingOptionDialog('色彩风格', WEBUI_COLOR_STYLE_OPTIONS, Theme.getUiOption('colorStyle') || 'TonalSpot', value => {
          Theme.setUiOption('colorStyle', value);
          loadSettings();
        });
      });
      content.querySelector('.setting-select-row[data-key="colorSpec"]')?.addEventListener('click', () => {
        showSettingOptionDialog('色彩标准', WEBUI_COLOR_SPEC_OPTIONS, Theme.getUiOption('colorSpec') || 'Spec2025', value => {
          Theme.setUiOption('colorSpec', value);
          loadSettings();
        });
      });
      content.querySelector('.setting-select-row[data-key="pageScale"]')?.addEventListener('click', showPageScaleDialog);
      content.querySelector('#autoTemplateRow')?.addEventListener('click', () => {
        showAutoTemplatePickerDialog(State.globalConfig?.auto_enable_new_apps_template_id || '', async templateId => {
          try {
            await saveGlobalConfigFromSettings(content, {
      ...
      content.querySelector('#backupImportBtn')?.addEventListener('click', () => content.querySelector('#backupRestoreFile')?.click());
      content.querySelector('#backupRestoreFile')?.addEventListener('change', handleBackupFileSelected);
    ...
    } catch { content.innerHTML = '<div class="app-empty">加载设置失败</div>'; }
  }

  function buildGlobalConfigFromSettings(content, overrides) {
    return normalizeGlobalRuntimeConfig(Object.assign({}, State.globalConfig || {}, {
      file_monitor_enabled: content.querySelector('.toggle[data-key="fileMonitor"]')?.classList.contains('on') ?? true,
      fuse_fix_enabled: content.querySelector('.toggle[data-key="fuseFix"]')?.classList.contains('on') ?? true,
      fuse_daemon_redirect_enabled: content.querySelector('.toggle[data-key="fuseDaemonRedirect"]')?.classList.contains('on') === true,
      verbose_logging_enabled: content.querySelector('.toggle[data-key="verboseLogging"]')?.classList.contains('on') === true,
      auto_enable_redirect_for_new_apps: content.querySelector('.toggle[data-key="autoEnableNewApps"]')?.classList.contains('on') === true,
      auto_enable_new_apps_template_id: State.globalConfig?.auto_enable_new_apps_template_id || '',
      app_config_auto_save: content.querySelector('.toggle[data-key="appConfigAutoSave"]')?.classList.contains('on') === true
    }, overrides || {}));
  }
  ...
  async function saveGlobalConfigFromSettings(content, overrides, options) {
    ...
    setFeatureChipState('verboseLogStatusChip', config.verbose_logging_enabled === true, '详细日志');
    Theme.showToast('设置已保存', 'success');
  ...
  async function handleAutoEnableNewAppsToggle(content, toggle) {
    if (!toggle.classList.contains('on')) {
      await saveGlobalSettingImmediate(content, toggle, 'autoEnableNewApps', { reloadSettings: true });
      return;
    }
  ...
  function isSameStringSet(left, right) {
    ...
    return left.map(item => String(item || '').toLowerCase()).sort().join('\n') ===
      right.map(item => String(item || '').toLowerCase()).sort().join('\n');
  }
  ...
  function cloneForBackupDigest(value, options) {
    ...
    if (options?.legacyDefaultMonitorOperations === true && copy.monitor_filters) {
      ...
      if (isCurrentDefault) copy.monitor_filters.excluded_operations = LEGACY_DEFAULT_MONITOR_FILTER_OPERATIONS.slice();
    }
    if (options?.legacyFullDefaultMonitorOperations === true && copy.monitor_filters) {
      const ops = copy.monitor_filters.excluded_operations;
      const isCurrentDefault = isSameStringSet(ops, DEFAULT_MONITOR_FILTER_OPERATIONS);
      if (isCurrentDefault) copy.monitor_filters.excluded_operations = LEGACY_FULL_DEFAULT_MONITOR_FILTER_OPERATIONS.slice();
    }
    return copy;
  }
  ...
  async function sha256Hex(value) {
    ...
    return Array.from(h).map(word => word.toString(16).padStart(8, '0')).join('');
  }
  ...
  async function verifyBackupDigest(canonical, integrity) {
    const algorithm = String(integrity?.algorithm || '').trim().toUpperCase();
  ...
  function normalizeBackupUiPreferences(raw) {
    ...
    if (has('update_channel')) {
      ...
      if (UPDATE_CHANNEL_OPTIONS.some(item => item.value === channel)) ui.update_channel = channel;
    }
  ...
  function normalizeBackupColorStyle(value) {
    const raw = String(value || '').trim();
    const lower = raw.toLowerCase();
    return WEBUI_COLOR_STYLE_OPTIONS.find(item => item.value.toLowerCase() === lower)?.value || null;
  }

  function normalizeBackupColorSpec(value) {
    const raw = String(value || '').trim();
    const lower = raw.toLowerCase();
    return WEBUI_COLOR_SPEC_OPTIONS.find(item => item.value.toLowerCase() === lower)?.value || null;
  }
  ...
  function restoreBackupUiPreferences(ui) {
    ...
    if (has('floating_bottom_bar')) { nextUiOptions.floatingNav = ui.floating_bottom_bar !== false; uiOptionsChanged = true; }
    if (has('liquid_glass')) { nextUiOptions.liquidGlass = ui.liquid_glass !== false; uiOptionsChanged = true; }
    if (has('blur_effect')) { nextUiOptions.blurEffect = ui.blur_effect !== false; uiOptionsChanged = true; }
    if (has('dynamic_color')) { nextUiOptions.dynamicColor = ui.dynamic_color === true; uiOptionsChanged = true; }
    if (has('accent_color')) {
      const color = backupAccentColorToWebValue(ui.accent_color);
      if (color !== null) { nextUiOptions.accentColor = color; uiOptionsChanged = true; }
    }
    if (has('color_style')) {
      const style = normalizeBackupColorStyle(ui.color_style);
      if (style) { nextUiOptions.colorStyle = style; uiOptionsChanged = true; }
    }
    if (has('color_spec')) {
      const spec = normalizeBackupColorSpec(ui.color_spec);
      if (spec) { nextUiOptions.colorSpec = spec; uiOptionsChanged = true; }
    }
    if (has('page_scale')) {
      const scale = normalizeBackupPageScale(ui.page_scale);
      if (scale !== null) { nextUiOptions.pageScale = scale; uiOptionsChanged = true; }
    }
  ...
  function sanitizeBackupPath(raw, options) {
    ...
    path = path.replace(/\/+$/g, '');
    const validation = validatePath((excluded ? '!' : '') + path, { allowRuleSyntax, allowWildcards });
  ...
  function normalizeBackupUserProfile(raw) {
    ...
    if (Array.isArray(raw.allowed_real_paths)) {
      raw.allowed_real_paths.forEach(item => pushUniquePath(allowRules, sanitizeBackupPath(item, { allowRuleSyntax: true })));
    ...
    if (Array.isArray(raw.excluded_real_paths)) {
      raw.excluded_real_paths.forEach(item => {
        const path = sanitizeBackupPath(String(item ?? '').replace(/^!/, ''), { allowRuleSyntax: true });
    ...
    const readOnlyPaths = [];
    const rawReadOnly = Array.isArray(raw.read_only_paths) ? raw.read_only_paths : (typeof raw.read_only_paths === 'string' ? [raw.read_only_paths] : []);
    if (raw.read_only_paths != null && !Array.isArray(raw.read_only_paths) && typeof raw.read_only_paths !== 'string') {
      warnings.push('read_only_paths 格式不支持，已忽略');
    }
    rawReadOnly.forEach(item => pushUniquePath(readOnlyPaths, sanitizeBackupPath(item, { allowRuleSyntax: true, allowWildcards: true })));
    profile.allowed_real_paths = sortPathRules(removeConflictingAllowedRules(mergeAllowedRules(allowRules, [])));
    profile.excluded_real_paths = [];
    profile.read_only_paths = normalizeReadOnlyForRules(normalizeReadOnlyRules(readOnlyPaths), profile.allowed_real_paths);
    const sandboxed = [];
    const rawSandboxed = Array.isArray(raw.sandboxed_paths) ? raw.sandboxed_paths : (typeof raw.sandboxed_paths === 'string' ? [raw.sandboxed_paths] : []);
    if (raw.sandboxed_paths != null && !Array.isArray(raw.sandboxed_paths) && typeof raw.sandboxed_paths !== 'string') {
  ...
  function normalizeBackupAppConfig(packageName, raw) {
    ...
    if (!raw || typeof raw !== 'object' || Array.isArray(raw) || !raw.users || typeof raw.users !== 'object' || Array.isArray(raw.users)) {
      return { config: null, warnings: ['缺少 users 对象'] };
    }
    const users = {};
    const warnings = [];
    Object.keys(raw.users).sort((a, b) => Number(a) - Number(b)).forEach(userId => {
    ...
    if (!Object.keys(users).length) return { config: null, warnings: warnings.concat('没有有效用户配置') };
  ...
  function normalizeBackupApps(rawApps) {
    ...
    Object.keys(source).sort((a, b) => a.localeCompare(b)).slice(0, BACKUP_MAX_APPS + 1).forEach(packageName => {
  ...
  function normalizeTemplate(raw) {
    ...
    if (!name) return null;
    const normalized = normalizeBackupAppConfig('template.source', raw.config || raw.app_config || raw);
  ...
  function normalizeTemplates(rawTemplates) {
    ...
    templates.sort((a, b) => a.name.localeCompare(b.name, 'zh-Hans-CN') || a.id.localeCompare(b.id));
    return templates;
  }
  ...
  async function loadTemplates(force) {
    ...
    State.templates = normalizeTemplates(await Api.readTemplates(force ? { force: true } : undefined));
  ...
  function templateRowHtml(template, options) {
    const actions = options?.manage
      ? '<div class="template-row-actions"><button class="icon-btn icon-btn-sm icon-btn-danger template-delete" type="button" data-id="' + escapeHtml(template.id) + '" aria-label="删除模板" title="删除模板"><span class="icon icon-trash" aria-hidden="true"></span></button></div>'
      : '';
    return '<div class="template-row" data-id="' + escapeHtml(template.id) + '"><span class="template-row-icon"><span class="icon icon-template" aria-hidden="true"></span></span><div class="template-row-main"><div class="template-row-title">' + escapeHtml(template.name) + '</div><div class="template-row-subtitle">' + escapeHtml(templateSummary(template.config)) + '</div></div>' + actions + '</div>';
  }

  function showTemplateNameDialog(title, initialName, onConfirm) {
    showModalWithHistory(
      '<div class="modal-title">' + escapeHtml(title) + '</div>' +
      '<input type="text" class="modal-input" id="templateNameInput" placeholder="模板名称" maxlength="48" autocomplete="off" value="' + escapeHtml(initialName || '') + '">' +
  ...
  async function showTemplatePickerDialog(title, onPick) {
    ...
    const body = templates.length
      ? '<div class="template-card template-list-scroll template-picker-list">' + templates.map(t => templateRowHtml(t)).join('') + '</div>'
      : '<div class="app-empty" style="padding:24px 8px">还没有配置模板</div>';
    showModalWithHistory('<div class="modal-title">' + escapeHtml(title) + '</div>' + body, { backdropClose: true });
  ...
  async function saveCurrentConfigAsTemplate(packageName, users) {
    ...
    showTemplateNameDialog('保存为模板', '', async name => {
      const ok = await upsertTemplate({ id: createTemplateId(), name, config: { users: JSON.parse(JSON.stringify(users || {})) } });
  ...
  async function applyTemplateToPackages(template, packageNames) {
    ...
    try {
      ...
      if (ok) targets.forEach(packageName => {
  ...
  async function buildBackupPayload() {
    const [globalConfig, configuredConfigs, templatesRaw, monitorFilters, version] = await Promise.all([
      Api.readGlobalConfig(),
      Api.readConfiguredAppConfigs({ force: true }),
      Api.readTemplates(),
    ...
    return {
      ...
      summary: {
        ...
        userCount: Object.values(apps).reduce((sum, cfg) => sum + Object.keys(cfg.users || {}).length, 0),
  ...
  function normalizeBackupDirectoryPath(path) {
    const value = String(path || '').trim().replace(/\\/g, '/').replace(/\/+/g, '/').replace(/\/+$/g, '');
    ...
    if (shouldUseBrowserSavePicker() && window.showSaveFilePicker) {
      try {
        return await window.showSaveFilePicker({
          ...
          types: [{ description: 'Storage Redirect X Backup', accept: { 'application/zip': ['.zip'] } }]
        });
      } catch (error) {
  ...
  function showBackupDirectoryDialog(fileName) {
    return new Promise(resolve => {
      showModalWithHistory(
        ...
        '<input type="text" class="modal-input" id="backupDirInput" placeholder="Download" autocomplete="off" value="' + escapeHtml(BACKUP_DEFAULT_DIR) + '">' +
        '<div class="path-validation" id="backupDirValidation"></div>' +
        '<div id="backupDirBrowserContainer"></div>' +
        '<div class="modal-hint">备份文件名：' + escapeHtml(fileName) + '。相对路径以 ' + BACKUP_STORAGE_BASE + ' 为根。</div>' +
      ...
      const browser = buildPathBrowser(BACKUP_STORAGE_BASE, input, hint, updateBackupDirectoryHint, { autoLoad: true, directoriesOnly: true });
      ...
      function updateBackupDirectoryHint() {
        ...
        if (hint) {
          hint.textContent = path ? '保存到 ' + path : (input?.value.trim() ? '请选择公开存储目录' : '');
  ...
  async function saveBackupWithExistingSurface(fileName, bytes, target) {
    ...
    if (target?.type === 'path') {
      const path = await Api.saveBackupBytesToDirectory?.(target.path, fileName, uint8ToBase64(bytes));
      if (path) return path;
  ...
  async function inflateRawZipEntry(bytes) {
    if (typeof DecompressionStream !== 'function') throw new Error('当前浏览器不支持读取压缩备份包');
  ...
  async function extractBackupJsonFromZip(bytes) {
    ...
    while (offset + 30 <= bytes.length) {
      ...
      if (name === BACKUP_ZIP_ENTRY) {
        const payloadBytes = method === 0 ? data : method === 8 ? await inflateRawZipEntry(data) : null;
  ...
  async function parseBackupText(text) {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('备份格式错误');
    if (parsed.magic !== BACKUP_MAGIC) throw new Error('不是 Storage Redirect X 备份');
    if (!Number.isInteger(parsed.schema) || parsed.schema < 1 || parsed.schema > BACKUP_SCHEMA_VERSION) throw new Error('备份格式版本不支持');
    if (!parsed.module || parsed.module.id !== BACKUP_MODULE_ID) throw new Error('备份属于其它模块');
    if (!parsed.data || typeof parsed.data !== 'object' || Array.isArray(parsed.data)) throw new Error('备份缺少数据区');
    ...
    const buildDigestCandidates = async (source) => {
      ...
      return Promise.all(optionSets.map(options => {
        const cloned = Object.keys(options).length ? cloneForBackupDigest(source, options) : source;
    ...
    const legacyMonitorDigests = stableJson(legacyMonitorData) === stableJson(data)
      ? []
      : await buildDigestCandidates(legacyMonitorData);
    const legacyFullMonitorData = cloneForBackupDigest(data, { legacyFullDefaultMonitorOperations: true });
    const legacyFullMonitorDigests = stableJson(legacyFullMonitorData) === stableJson(data)
      ? []
    ...
    if (![...baseDigests, ...legacyMonitorDigests, ...legacyFullMonitorDigests, ...monitorCompatDigests].includes(expected)) throw new Error('备份校验失败，文件可能被改动');
  ...
  async function handleBackupFileSelected(event) {
    ...
    try {
      ...
      const warningHint = parsed.warnings.length ? '有 ' + parsed.warnings.length + ' 条不兼容字段会被忽略。' : '';
      const confirmed = await confirmAction(
        '将用备份覆盖当前全局设置、全部应用配置、外观偏好和检查更新设置，共 ' + appCount + ' 个应用。' + warningHint + '还原后建议重启已配置应用或相关媒体进程。是否继续？'
      );
      if (!confirmed) return;
  ...
  function countDashboardMonitorEntries(raw) {
    ...
    coalesceLogEntries(entries, mappingPaths).forEach(entry => {
      if (!shouldHideCompanionLogEntry(entry, mappingPaths) && isEffectiveMonitorEntry(entry)) count += 1;
    });
    return count;
  }
  ...
  function isEffectiveMonitorEntry(entry) {
    ...
    if (source === 'path_mapping' || source === 'sandbox_path' || source === 'redirect_root' || source === 'media_store_pending_commit') {
      return true;
    }
  ...
  function renderLogCards() {
    ...
    if (infoEl) infoEl.textContent = query ? '匹配 ' + entries.length + ' / ' + all.length + ' 条' : '共 ' + all.length + ' 条';
    if (!entries.length) { viewer.innerHTML = '<div class="log-empty">' + (query ? '没有匹配的日志' : '暂无文件操作记录') + '</div>'; return; }
    viewer.innerHTML = entries.map(renderLogCard).join('');
    bindLogCardToggles(viewer);
    if (State.shouldRestoreLogListScroll) requestAnimationFrame(() => {
      restoreLogListScrollIfNeeded();
      State.shouldRestoreLogListScroll = false;
    });
  }
  ...
  function parseLogLine(line) {
    ...
    const pkg = isModuleWebUiExport ? BACKUP_MODULE_ID : selectLogDisplayPackage(processPkg, callerPkg, watchPkg, identifyMethod);
    const appName = getLogPackageLabel(pkg);
    const operationLabel = isModuleWebUiExport ? 'export' : formatLogOperationBadge(syscallOp || eventKind);
    const action = isModuleWebUiExport ? describeModuleExportOperation(extras) : describeLogOperation(eventKind, syscallOp, extras);
    const sourceText = isModuleWebUiExport ? describeModuleExportSource(extras) : describeLogSource(processPkg, callerPkg, extras);
  ...
  function coalesceLogEntries(entries, mappingPaths = emptyMappingPaths()) {
    ...
    entries.forEach(entry => {
      ...
      if (!key) { ordered.push(entry); return; }
  ...
  function shouldHideCompanionLogEntry(entry, mappingPaths = emptyMappingPaths()) {
    ...
    const opFilter = String(entry?.extras?.op_filter || entry?.extras?.op || entry?.operationLabel || '').toLowerCase();
    return opFilter.includes('open') || opFilter.includes('provider_open') || opFilter.includes('create') || opFilter.includes('write');
  }

  function buildLogCoalesceKey(entry) {
    const path = entry?.extras?.source === 'path_mapping'
    ...
    const op = isDiagnosticArchive ? 'diagnostic_export' : normalizeMonitorCoalesceOperation(entry?.operationLabel || entry?.extras?.op_filter || entry?.extras?.op || '');
    return [
  ...
  function normalizeMonitorCoalesceOperation(value) {
    const op = String(value || '').toLowerCase().replace(/:create$|:read$/g, '');
    return (op === 'open' || op === 'openat' || op === 'openat2' || op === 'provider_open' || op === 'inotify' || op === 'close_write' || op === 'export') ? 'create' : op;
  }
  ...
  function isModuleExportLogEntry(extras, processPkg, callerPkg, watchPkg) {
    return extras?.identify_method === 'module_export' ||
  ...
  function logEntryRank(entry) {
    ...
    if (method === 'watch_package' && entry?.watchPkg && entry.watchPkg !== '-' && entry.watchPkg !== entry.processPkg) {
    ...
    if (entry?.landingPath && entry?.sourcePath && entry.landingPath !== entry.sourcePath) score += 100;
    score += ({ module_export: 125, caller: 120, provider_open: 110, recent_private_owner: 105, recent_caller: 100, path_config: 80, media_provider_fallback: 70, daemon_inotify: 75, java_stack: 70, stack: 70, path_hint: 55, thread_name: 45, path_owner: 35, owner_uid: 35, shared_uid: 10, unknown: 0 }[method] ?? 20);
    ...
    if (entry?.extras?.source === 'allowed_real_path' && entry?.watchPkg && method !== 'watch_package') score -= 220;
  ...
  function describeLogSource(processPkg, callerPkg, extras) {
    ...
    if (callerPkg && callerPkg !== '-' && callerPkg !== processPkg && !isIntermediateLogPackage(callerPkg)) {
      if (isSinglePackageName(callerPkg)) return '调用方 ' + callerPkg + '（' + formatIdentifyMethod(method) + '）';
      return '候选应用 ' + callerPkg + '（' + formatIdentifyMethod(method) + '）';
    }
    if (method === 'watch_package' && extras.watch_package && extras.watch_package !== '-' && extras.watch_package !== processPkg && !isIntermediateLogPackage(extras.watch_package)) {
  ...
  function formatIdentifyMethod(method) {
    const map = { caller: '直接调用方', module_export: '模块导出记录', provider_open: 'Provider 打开请求', media_provider_fallback: 'MediaProvider 回退', path_owner: '路径归属', owner_uid: '文件属主', path_config: '路径配置', daemon_inotify: '外部 inotify', path_hint: '路径推断', stack: '堆栈推断', java_stack: 'Java 栈推断', thread_name: '线程名推断', recent_caller: '近期调用方', recent_private_owner: '近期私有路径归属', download_owner: '下载记录', query_access: '媒体查询记录', shared_uid: '共享 UID 回退', unknown: '来源未知' };
  ...
  function selectLogDisplayPackage(processPkg, callerPkg, watchPkg, identifyMethod) {
    return [
      ...
      callerPkg,
      processPkg
    ].find(isSinglePackageName) || processPkg || '';
  }
  ...
  function isIntermediateLogPackage(value) {
    ...
    return pkg === 'com.google.android.providers.media.module' ||
  ...
  function describeErrno(errno, extras = {}) {
    ...
    const map = { 1: '失败：无权限', 2: '失败：路径不存在', 13: '失败：权限被拒绝', 20: '失败：不是目录', 21: '失败：是目录', 28: '失败：空间不足', 30: '失败：只读文件系统', 107: '失败：传输端未连接' };
    return map[errno] || (errno ? '失败 errno=' + errno : '失败');
  }

  function formatLogOperationBadge(op) {
    const value = String(op || '').trim().toLowerCase();
    if (!value) return 'unknown';
    if (value === 'provider_open:read' || value === 'provider_open:create' || value === 'provider_open:write') return value;
  ...
  function renderLogCard(entry) {
    ...
    const iconHtml = iconSrc
      ? '<div class="log-card-icon has-image" data-initial="' + escapeHtml(initial) + '" aria-hidden="true"><img class="log-card-icon-img" src="' + escapeHtml(iconSrc) + '" alt="" loading="lazy" onerror="const icon=this.closest(\'.log-card-icon\'); if (icon) { icon.classList.remove(\'has-image\'); icon.classList.add(\'fallback\'); } this.remove();"></div>'
      : '<div class="log-card-icon fallback" data-initial="' + escapeHtml(initial) + '" aria-hidden="true"></div>';
    const identityContent = iconHtml + '<div class="log-card-title">' + escapeHtml(entry.appName) + '</div>';
    const identityHtml = canOpenApp
      ? '<button class="log-app-identity" type="button" data-pkg="' + escapeHtml(entry.pkg) + '" aria-label="打开应用配置">' + identityContent + '</button>'
      : '<div class="log-app-identity is-disabled">' + identityContent + '</div>';
    const detailParts = [];
    const requestPath = requestPathForLogEntry(entry);
    if (entry.backendPath && entry.backendPath !== entry.path) detailParts.push('实际路径：' + escapeHtml(entry.backendPath));
    if (requestPath && requestPath !== entry.path) detailParts.push('请求路径：' + escapeHtml(requestPath));
    if (entry.status === 'error') detailParts.push('<span class="log-detail-error">' + escapeHtml(entry.statusText) + '</span>');
    const detailHtml = detailParts.length ? '<div class="log-detail">' + detailParts.join('<br>') + '</div>' : '';
    return '<article class="log-card ' + entry.status + '">' +
      '<div class="log-card-body">' +
        '<div class="log-card-head">' +
          identityHtml +
          '<span class="log-status ' + entry.status + '" title="' + escapeHtml(entry.status === 'error' ? entry.statusText : entry.action) + '">' + escapeHtml(entry.operationLabel || 'unknown') + '</span>' +
          '<button class="log-time" type="button" aria-label="' + escapeHtml(timeTitle) + '" title="' + escapeHtml(timeTitle) + '" aria-pressed="' + (State.logFullTime ? 'true' : 'false') + '">' + escapeHtml(timeText) + '</button>' +
        ...
        '</div>' +
        (entry.summaryText ? '<div class="log-action">' + escapeHtml(entry.summaryText) + '</div>' : '') +
        '<div class="log-path">' + escapeHtml(entry.path || '未解析到路径') + '</div>' +
        detailHtml +
      '</div>' +
    '</article>';
  }
  ...
  function renderMonitorFilterList(container, items, type) {
    ...
    container.innerHTML = items.map((value, index) =>
      '<div class="monitor-filter-item">' +
        '<span class="monitor-filter-value">' + escapeHtml(value) + '</span>' +
        '<button class="monitor-filter-delete" type="button" data-type="' + type + '" data-index="' + index + '" aria-label="删除规则">×</button>' +
      '</div>'
    ).join('');
  }
  ...
  async function showMonitorFilterDialog() {
    ...
    const modalActions = autoSave ? '' : '<div class="modal-actions"><button class="btn btn-secondary modal-close" type="button">取消</button><button class="btn btn-primary" id="monitorFilterSave" type="button">保存</button></div>';
    const draft = {
    ...
    showModalWithHistory(
      ...
      '<div class="monitor-filter-section">' +
        ...
        '<div class="monitor-filter-input-row"><input type="text" class="modal-input" id="monitorFilterPathInput" placeholder="Download 或 Android/cache" autocomplete="off"><button class="icon-btn icon-btn-sm icon-btn-add monitor-filter-add" id="monitorFilterAddPath" type="button" aria-label="添加排除路径" title="添加排除路径">' + iconHtml('plus') + '</button></div>' +
      ...
      '<div class="monitor-filter-section">' +
        ...
        '<div class="monitor-filter-input-row"><input type="text" class="modal-input" id="monitorFilterOpInput" placeholder="provider_open:read 或 open*:read" autocomplete="off"><button class="icon-btn icon-btn-sm icon-btn-add monitor-filter-add" id="monitorFilterAddOp" type="button" aria-label="添加操作类型" title="添加操作类型">' + iconHtml('plus') + '</button></div>' +
    ...
    const render = () => {
      ...
      $('#modalContent')?.querySelectorAll('.monitor-filter-delete').forEach(btn => {
    ...
    const addValue = async (key, input, label) => {
      const result = key === 'excluded_paths'
    ...
    $('#monitorFilterAddPath')?.addEventListener('click', () => addValue('excluded_paths', pathInput, '排除路径'));
    $('#monitorFilterAddOp')?.addEventListener('click', () => addValue('excluded_operations', opInput, '操作类型'));
    pathInput?.addEventListener('input', () => showMonitorFilterPathValidation(pathInput, pathValidation));
    pathInput?.addEventListener('keydown', event => { if (event.key === 'Enter') addValue('excluded_paths', pathInput, '排除路径'); });
    opInput?.addEventListener('keydown', event => { if (event.key === 'Enter') addValue('excluded_operations', opInput, '操作类型'); });
  ...
  async function loadAbout() {
    ...
    if (!list) return;
    list.innerHTML = LICENSES.map(item =>
      '<a class="license-item" href="' + item.url + '">' +
        '<span class="license-group">' + escapeHtml(item.group) + '</span>' +
        '<span class="license-name">' + escapeHtml(item.name) + '</span>' +
        '<span class="license-meta">' + escapeHtml(item.license) + '</span>' +
      '</a>'
    ).join('');
  }
  ...
  $('#logClear')?.addEventListener('click', () => {
    Theme.confirmDelete('确认清空文件监视记录？', async () => {
      try { await Api.exec('mkdir -p ' + FILE_MONITOR_LOG_DIR + ' && : > ' + FILE_MONITOR_LOG); Theme.showToast('日志已清空', 'success'); loadLogs(); }
      catch { Theme.showToast('清空失败', 'error'); }
    });
  });
  ...
  function init() { Theme.init(); setupHistory(); initNav(); initSearch(); initLogSearch(); initPullRefreshControls(); initDashboardRefreshHooks(); loadDashboard(); }
