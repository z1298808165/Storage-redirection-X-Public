...
const THEME_VIEWPORT_CONTENT = 'width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover, interactive-widget=overlays-content';
...
const Theme = {
  ...
  getUiOptions() {
    try {
      return Object.assign({}, THEME_UI_DEFAULTS, JSON.parse(localStorage.getItem(THEME_UI_KEY) || '{}'));
    } catch {
  ...
  applyUiOptions(options) {
    ...
    document.body.classList.toggle('liquid-surface-disabled', prefs.liquidGlass === false || prefs.blurEffect === false);
  ...
  applyPageScale(options) {
    ...
    document.querySelector('meta[name="viewport"]')?.setAttribute('content', THEME_VIEWPORT_CONTENT);
  ...
  syncPageScaleLayout(scaleValue) {
    ...
    if (!app) return;
    const resetProps = ['zoom', 'width', 'height', 'right', 'bottom', 'left', 'transform', 'transformOrigin'];
    if (scale === 1) {
  ...
  applyAccentOptions(options) {
    ...
    const primary = this._rgbToHex(this._adjustColor(base, { saturation, lightness: specLightness }));
    const secondary = this._rgbToHex(this._adjustColor(base, { hue: secondaryHue, saturation: Math.max(0, saturation * 0.82), lightness: spec === 'Spec2021' ? 7 : 11 }));
    const primaryRgb = this._hexToRgb(primary);
    root.style.setProperty('--color-primary', primary);
    root.style.setProperty('--color-primary-2', secondary);
    root.style.setProperty('--color-primary-bg', this._rgba(primaryRgb, this._isDarkResolved() ? 0.16 : 0.12));
    root.style.setProperty('--color-primary-border', this._rgba(primaryRgb, this._isDarkResolved() ? 0.28 : 0.24));
    root.style.setProperty('--color-info', primary);
    root.style.setProperty('--color-text-on-primary', this._relativeLuminance(primaryRgb) > 0.55 ? '#101828' : '#ffffff');
  },
  ...
  _hexToRgb(hex) {
    const value = String(hex || '#2f7dff').replace('#', '').trim();
    const normalized = value.length === 3 ? value.split('').map(ch => ch + ch).join('') : value.padStart(6, '0').slice(-6);
  ...
  _syncNavLens(activeItem) {
    document.querySelectorAll('.nav-item').forEach(item => item.classList.toggle('is-under-lens', item === activeItem));
  },
  ...
  _bindLiquidSurfaceLight() {
    ...
    const selector = '.liquid-surface, .bottom-nav, .app-batch-bar, .modal-sheet, .dialog-box, .toast, .search-input-wrapper, .filter-group, .app-config-header, .status-card, .action-list, .app-list, .config-group, .theme-selector, .theme-settings-card, .template-card, .backup-restore-card, .log-card, .app-user-menu, .license-item';
    const update = (target, clientX, clientY) => {
      ...
      surface.style.setProperty('--liquid-x', Math.max(0, Math.min(100, (clientX - rect.left) / rect.width * 100)).toFixed(2) + '%');
      surface.style.setProperty('--liquid-y', Math.max(0, Math.min(100, (clientY - rect.top) / rect.height * 100)).toFixed(2) + '%');
    };
    document.addEventListener('pointerdown', e => update(e.target, e.clientX, e.clientY), { passive: true });
  ...
  _bindNavDrag() {
    ...
    const updateLightFromPointer = (clientX, clientY) => {
      ...
      nav.style.setProperty('--nav-touch-x', clamp((clientX - box.left) / box.width * 100, 0, 100).toFixed(2) + '%');
      nav.style.setProperty('--nav-touch-y', clamp((clientY - box.top) / box.height * 100, 0, 100).toFixed(2) + '%');
    };
    const setRubberOffset = (clientX) => {
      ...
      nav.style.setProperty('--nav-rubber-x', (Math.sign(fraction) * Math.pow(Math.abs(fraction), 0.72) * 4).toFixed(2) + 'px');
    };
    ...
    const updateTargetFromPointer = (clientX, clientY) => {
      ...
      nav.style.setProperty('--nav-indicator-scale-x', (1 + clamp(Math.abs(state.velocity) / 420, 0, 0.09)).toFixed(3));
      nav.style.setProperty('--nav-indicator-scale-y', (1 - clamp(Math.abs(state.velocity) / 980, 0, 0.035)).toFixed(3));
    ...
    const kickAnimation = () => {
      ...
      const frame = () => {
        ...
        if (Math.abs(state.targetX - state.currentX) > 0.15 || Math.abs(state.targetW - state.currentW) > 0.15) {
  ...
  showDialog(message, onConfirm, onCancel) {
    ...
    cancelBtn.onclick = () => { overlay.classList.add('hidden'); document.body.classList.remove('modal-open'); if (onCancel) onCancel(); };
    actions.appendChild(cancelBtn);
    
    const confirmBtn = document.createElement('button');
    confirmBtn.className = 'btn btn-primary';
    confirmBtn.textContent = '确认';
    confirmBtn.onclick = () => { overlay.classList.add('hidden'); document.body.classList.remove('modal-open'); if (onConfirm) onConfirm(); };
  ...
  showLoadingDialog(message) {
    ...
    body.innerHTML = '<div class="loading-state dialog-loading">' +
      '<div class="spinner"></div>' +
      '<span>' + this.escapeHtml(message || '处理中...') + '</span>' +
    ...
    let progressValue = 0;
    return {
      close() { overlay.classList.add('hidden'); document.body.classList.remove('modal-open'); },
  ...
  bindModalViewport() {
    ...
    const scheduleViewportSync = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        const availableHeight = Math.max(0, keyboardActive ? viewportHeight - viewportTop : layoutHeight);
        const maxHeightRatio = keyboardActive ? 0.86 : 0.85;
        if (availableHeight) {
          overlay.style.setProperty('--modal-sheet-max-height', Math.min(Math.round(availableHeight * maxHeightRatio), 720) + 'px');
        }
  ...
  showModal(contentHtml, options) {
    ...
    return {
      ...
      getElement(sel) { return content.querySelector(sel); }
    };
  },
  ...
  navigateTo(page, options) {
    ...
    document.querySelectorAll('.nav-item').forEach(n => { n.classList.remove('active'); n.classList.remove('is-under-lens'); });
    const navItem = document.querySelector('.nav-item[data-page="' + page + '"]');
    if (navItem) { navItem.classList.add('active'); this._syncNavLens(navItem); }
