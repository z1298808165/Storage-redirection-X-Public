...
const UPDATE_MANIFEST_URL = 'https://raw.githubusercontent.com/' + DEFAULT_RELEASE_REPOSITORY + '/' + DEFAULT_RELEASE_BRANCH + '/update.json';
    ...
    return new Promise((resolve, reject) => {
      ...
      if (typeof window !== 'undefined' && window.LSPosedBridge && window.LSPosedBridge.exec) {
        ...
        const timer = setTimeout(() => {
          if (settled) return;
          settled = true;
          try { delete window[callbackId]; } catch {}
      ...
      if (typeof ksu !== 'undefined' && ksu.exec) {
        let settled = false;
        let timer = null;
        const callbackName = 'srx_exec_callback_' + Date.now() + '_' + Math.floor(Math.random() * 100000);
        const cleanup = () => { if (timer) clearTimeout(timer); try { delete window[callbackName]; } catch {} };
        ...
        try {
          const result = ksu.exec.length === 2 ? ksu.exec(cmd, finish) : ksu.exec(cmd, '{}', callbackName);
          ...
          try {
            ...
            if (result && typeof result.then === 'function') {
              result.then(res => typeof res === 'string' ? finish(0, res, '') : finish(res)).catch(reject);
            } else if (typeof result === 'string') {
              finish(0, result, '');
            }
          } catch (fallbackError) {
...
const DEFAULT_FILE_MONITOR_FILTERS = {
  ...
  excluded_operations: ['open:read', 'open*:read', 'provider_open:read', 'rename*', 'unlink*', 'delete*', 'rmdir*', 'link*', 'symlink*', 'truncate*', 'ftruncate*', 'chmod*', 'fchmod*', 'utimens*', 'futimens*', 'attrib*']
};
...
function withSrxCtlFallback(action, fallback) {
  return 'if [ -r ' + shellQuote(SRXCTL) + ' ]; then ' + srxCtlCommand(action) + '; else ' + fallback + '; fi';
}

function isManagedTempPath(path) {
  const clean = String(path || '').replace(/\/+$/g, '');
  return clean.length > '/data/local/tmp/srx_'.length &&
...
function normalizePublicStoragePath(path) {
  const clean = String(path || '').trim().replace(/\\/g, '/').replace(/\/+/g, '/');
  if (!clean || /[\r\n|]/.test(clean)) return '';
...
function publicStorageWriteTestCommand(source, target) {
  ...
  return 'src=' + shellQuote(source) + '; target=' + shellQuote(target) + '; dir=' + shellQuote(dir) + '; ' +
...
function diagnosticProgressCommand(progress, percent, phase, message) {
  ...
  return 'printf "%s|%s|%s\\n" ' +
    ...
    shellQuote(phase) + ' ' +
    shellQuote(message) + ' > ' + shellQuote(progress) + ' 2>/dev/null || true; ';
}
...
function parseCompatAppList(content) {
  ...
  String(content || '').split('\n').forEach(line => {
    ...
    if (isSafePackageName(packageName)) apps.push({ packageName, appLabel: appLabel || packageName, isSystem: false });
  });
  return apps;
}
...
function isLegacyDefaultMonitorOperations(list) {
  ...
  const normalized = list.map(item => String(item || '').toLowerCase()).sort().join('\n');
  return normalized === ['delete*', 'open:read', 'rename*', 'unlink*'].join('\n') ||
    normalized === ['attrib*', 'chmod*', 'delete*', 'fchmod*', 'ftruncate*', 'futimens*', 'link*', 'open*:read', 'open:read', 'rename*', 'rmdir*', 'symlink*', 'truncate*', 'unlink*', 'utimens*'].join('\n');
}
...
function parseSemVersion(value) {
  const match = String(value || '').trim().match(/(?:^|[^0-9])v?(\d+)\.(\d+)\.(\d+)(?:-([0-9A-Za-z.-]+))?/);
...
function manifestReleaseEntries(manifest) {
  const entries = [];
  if (manifest?.stable && hasReleaseVersion(manifest.stable)) entries.push({ channel: 'Stable', release: manifest.stable });
  if (manifest?.beta && hasReleaseVersion(manifest.beta)) entries.push({ channel: 'Beta', release: Object.assign({ prerelease: true }, manifest.beta) });
...
function findReleaseUpdate(manifest, repository, currentVersionName, channel) {
  ...
  const currentVersion = parseSemVersion(currentVersionName) || { major: 0, minor: 0, patch: 0, preRelease: [] };
  let best = null;
  ...
  const releaseRepository = String(release.repository || manifest?.repository || repository || DEFAULT_RELEASE_REPOSITORY).trim();
  const tag = String(release.tag || release.version || '').trim();
  return {
    tagName: tag,
    title: String(release.title || tag).trim(),
    htmlUrl: String(release.url || '').trim() || 'https://github.com/' + releaseRepository + '/releases/tag/' + tag,
...
const Api = {
  ...
  async writeFile(path, content) {
    ...
    try {
      ...
      await this.exec(
        'mkdir -p ' + shellQuote(targetDir) + ' ' + shellQuote(APPS_DIR) + ' && ' +
        'printf %s ' + shellQuote(encoded) + ' | base64 -d > ' + shellQuote(tmpfile) + ' && ' +
        'cp ' + shellQuote(tmpfile) + ' ' + shellQuote(path) + ' && ' +
        'chmod 644 ' + shellQuote(path) + ' && ' +
        'rm -f ' + shellQuote(tmpfile) + ' && ' +
        'touch ' + shellQuote(APPS_DIR) + ' 2>/dev/null && touch ' + shellQuote(path) + ' 2>/dev/null'
      );
  ...
  async writeRawFile(path, content, options) {
    ...
    try {
      ...
      if (encoded.length <= chunkSize) {
        await this.exec('printf %s ' + shellQuote(encoded) + ' | base64 -d > ' + shellQuote(tmpfile));
      } else {
        await this.exec(': > ' + shellQuote(tmpb64));
        for (let i = 0; i < encoded.length; i += chunkSize) {
          await this.exec('printf %s ' + shellQuote(encoded.slice(i, i + chunkSize)) + ' >> ' + shellQuote(tmpb64));
        }
      ...
      await this.exec(
        ...
        'rm -f ' + shellQuote(tmpfile) + ' ' + shellQuote(tmpb64)
      );
      return true;
    } catch (e) {
      try { await this.exec('rm -f ' + shellQuote(tmpfile) + ' ' + shellQuote(tmpb64)); } catch {}
    ...
    try {
      ...
      if (value.length <= chunkSize) {
        ...
        for (let i = 0; i < value.length; i += chunkSize) {
          await this.exec('printf %s ' + shellQuote(value.slice(i, i + chunkSize)) + ' >> ' + shellQuote(tmpb64));
        }
        await this.exec('base64 -d ' + shellQuote(tmpb64) + ' > ' + shellQuote(tmpfile));
      }
      await this.exec(
  ...
  async recordModuleExportMonitor(path, kind) {
    ...
    try {
      await this.exec(
        ...
        'printf "%s|' + MODULE_LOG_PACKAGE + '|' + MODULE_LOG_PACKAGE + '|OPEN|%s|ret=0|errno=0|identify_method=module_export|identify_reliability=high|op=provider_open|op_filter=provider_open:write|source=%s|export_kind=%s\\n" ' +
        '"${ts:-unknown}" ' + shellQuote(target) + ' ' + shellQuote(source) + ' ' + shellQuote(exportKind) + ' >> ' + shellQuote(FILE_MONITOR_LOG) + ' && ' +
        'chmod 666 ' + shellQuote(FILE_MONITOR_LOG) + ' 2>/dev/null || true'
      );
      return true;
    } catch (e) {
  ...
  buildDiagnosticArchiveCommand(stage, archive, progress) {
    if (!isManagedTempPath(stage) || !isManagedTempPath(archive) || (progress && !isManagedTempPath(progress))) throw new Error('unsafe diagnostic temp path');
    const progressArg = progress ? ' ' + shellQuote(progress) : '';
    const scriptCommand = 'if [ -r ' + shellQuote(DIAGNOSTIC_ARCHIVE_SCRIPT) + ' ]; then ' +
      '/system/bin/sh ' + shellQuote(DIAGNOSTIC_ARCHIVE_SCRIPT) + ' ' + shellQuote(stage) + ' ' + shellQuote(archive) + progressArg + '; ' +
  ...
  buildDiagnosticArchiveStartCommand(stage, archive, progress, done, runLog, pid) {
    ...
    if (paths.some(path => !isManagedTempPath(path))) throw new Error('unsafe diagnostic temp path');
    return managedTempCleanupCommand(...paths) + '; ' +
      ...
      'worker=' + shellQuote(worker) + '; script=' + shellQuote(DIAGNOSTIC_ARCHIVE_SCRIPT) + '; ' +
  ...
  buildDiagnosticArchivePollCommand(progress, done) {
    if (!isManagedTempPath(progress) || !isManagedTempPath(done)) throw new Error('unsafe diagnostic temp path');
    return 'progress=' + shellQuote(progress) + '; done=' + shellQuote(done) + '; ' +
  ...
  buildLegacyDiagnosticArchiveCommand(stage, archive, progress) {
    return 'stage=' + shellQuote(stage) + '; archive=' + shellQuote(archive) + '; module=' + shellQuote(MODULE_DIR) + '; logs=' + shellQuote(LOGS_DIR) + '; config=' + shellQuote(CONFIG_DIR) + '; ' +
  ...
  async runDiagnosticArchive(stage, archive, progress, done, runLog, pid, options) {
    ...
    const startCommand =
      ...
      'progress=' + shellQuote(progress) + '; ' +
      'done=' + shellQuote(done) + '; ' +
    ...
    while (Date.now() < deadline) {
      ...
      // 检查是否完成
      const doneCode = await this.exec('[ -f ' + shellQuote(done) + ' ] && cat ' + shellQuote(done) + ' || echo ""', { timeoutMs: 3000 }).catch(() => '');
      if (doneCode.trim() !== '') {
      ...
      // 读取进度
      const progressText = await this.exec('[ -f ' + shellQuote(progress) + ' ] && tail -n 1 ' + shellQuote(progress) + ' || echo ""', { timeoutMs: 3000 }).catch(() => '');
      ...
      if (match) {
        ...
        if (percent > lastPercent) {
          lastPercent = percent;
          options?.onProgress?.({ percent, phase, message: percent >= 98 && phase === 'done' ? '日志包已生成，正在准备写入目标文件' : message });
        }
      }
    }

    options?.onProgress?.({ percent: 99, phase: 'verify', message: '正在确认日志包' });
    const exists = await this.exec('[ -s ' + shellQuote(archive) + ' ] && echo 1 || echo 0', { timeoutMs: 10000 }).catch(() => '0');
  ...
  async copyDiagnosticArchiveToPublicPath(source, target) {
    ...
    const writeTarget = String(target || '').trim().replace(/\\/g, '/').replace(/\/+/g, '/');
    if (!normalizePublicStoragePath(writeTarget)) return false;
    const result = await this.exec(publicStorageWriteTestCommand(source, writeTarget), { timeoutMs: 60000 }).catch(() => '');
    return String(result || '').trim().split(/\s+/).pop() === 'ok';
  },
  ...
  async exportDiagnosticArchiveToDownloads(fileName, options) {
    ...
    try {
      ...
      try { await this.exec(managedTempCleanupCommand(stage, tmpArchive, progress, progress + '.tmp', done, runLog, pid, progress + '.worker.sh')); } catch {}
    }
  },

  async exportDiagnosticArchiveToDirectory(dirPath, fileName, options) {
    const safeName = sanitizeFileName(fileName, 'storage-redirect-x-logs.tar.gz');
    const dir = String(dirPath || '').trim().replace(/\\/g, '/').replace(/\/+/g, '/').replace(/\/+$/g, '');
  ...
  async writeMonitorFilters(config) {
    ...
    const ok = await this.writeRawFile(FILE_MONITOR_FILTERS_CONFIG, JSON.stringify(normalized, null, 2) + '\n', { mode: '644' });
    if (ok) {
  ...
  async writeAppConfigs(configs, options) {
    ...
    try {
      ...
      for (let i = 0; i < entries.length; i += 1) {
        ...
        const ok = await this.writeRawFile(stage + '/' + packageName + '.json', JSON.stringify(config, null, 2) + '\n', { mode: '644' });
        if (!ok) throw new Error('write staged app failed: ' + packageName);
      }
      options?.onProgress?.(total, total, '');
      await this.exec(
        'stage=' + shellQuote(stage) + '; apps=' + shellQuote(APPS_DIR) + '; global=' + shellQuote(GLOBAL_CONFIG) + '; ' +
        'mkdir -p "$apps" || exit 1; ' +
      ...
      try { await this.exec(managedTempCleanupCommand(stage)); } catch {}
  ...
  async writeTemplates(templates) {
    ...
    const ok = await this.writeRawFile(TEMPLATES_CONFIG, JSON.stringify({ templates: list }, null, 2) + '\n', { mode: '644' });
    if (ok) {
  ...
  async upsertTemplate(template) {
    ...
    templates.sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''), 'zh-Hans-CN') || String(a.id).localeCompare(String(b.id)));
  ...
  async deleteTemplate(templateId) {
    ...
    const templates = (await this.readTemplates({ force: true })).filter(item => item.id !== templateId);
    return await this.writeTemplates(templates);
  },
  ...
  async readConfiguredAppConfigs(options) {
    ...
    if (!options?.force && this._configuredAppsCache) {
      return this._configuredAppsCache.map(item => ({ packageName: item.packageName, config: item.config }));
    }
    try {
      const marker = '__SRX_APP_CONFIG__';
      const out = await this.exec(
        'mkdir -p ' + shellQuote(APPS_DIR) + '; ' +
        'for f in ' + shellQuote(APPS_DIR) + '/*.json; do ' +
  ...
  async listUsers() {
    try {
      ...
      const ids = [];
      String(out || '').replace(/UserInfo\{([0-9]+):/g, (_, id) => { ids.push(id); return ''; });
      if (!ids.length) String(out || '').replace(/\{([0-9]+):/g, (_, id) => { ids.push(id); return ''; });
  ...
  async getInstalledAppsFromPackageManager(userId) {
    ...
    for (const command of commands) {
      try {
        const out = await this.exec(
          command + ' | sed "s/package://" | awk -F= "{print \$2}"'
        );
  ...
  getInstalledAppsFromKsu() {
    ...
    try {
      ...
      const names = Array.from(new Set([...userRaw, ...systemRaw].map(item => {
  ...
  populatePackageInfo(packageNames) {
    ...
    try {
      let info = [];
      try { info = normalizePackageResult(ksu.getPackagesInfo(packageNames)); } catch {}
      if (!info.length) {
        try { info = normalizePackageResult(ksu.getPackagesInfo(JSON.stringify(packageNames))); } catch {}
      }
      info.forEach(item => {
  ...
  async getInstalledAppsFromDex(userId, options) {
    ...
    try {
      await this.exec('mkdir -p /data/Namespace-Proxy; if [ -f ' + shellQuote(LIST_APPS_DEX) + ' ]; then /system/bin/app_process64 -Djava.class.path=' + shellQuote(LIST_APPS_DEX) + ' / Main --user ' + safeUser + ' > ' + shellQuote(LIST_APPS_OUTPUT) + ' 2>/dev/null; fi');
  ...
  async getAppLabel(packageName) {
    ...
    try {
      return await this.exec('pm dump ' + shellQuote(packageName) + ' 2>/dev/null | ' +
  ...
  async listDir(dirPath) {
    try {
      ...
      const entries = out.split('\n').filter(entry => entry && entry !== './' && entry !== '../' && entry !== '.' && entry !== '..');
      const dirs = [];
      const files = [];
    ...
    try {
      const out = await this.exec(
        withSrxCtlFallback(
          ...
          'boot_ok=$(cat ' + shellQuote(MODULE_BOOT_OK) + ' 2>/dev/null); ' +
          'boot_marker=' + shellQuote(LOGS_DIR) + '/boot_${boot_id}.marker; ' +
          'if [ ! -d ' + shellQuote(MODULE_DIR) + ' ]; then echo unknown; ' +
          'elif [ -f ' + shellQuote(RUNTIME_DISABLE) + ' ] || [ -f ' + shellQuote(MODULE_DISABLE) + ' ]; then echo disabled; ' +
  ...
  async getModuleVersion() {
    try {
      const out = await this.exec('sed -n "s/^version=//p" ' + shellQuote(MODULE_PROP) + ' 2>/dev/null | head -n 1');
      return out || '';
    } catch {
      return '';
    }
  ...
  async fetchUpdateManifest(manifestUrl) {
    ...
    if (!response.ok) {
      if (response.status === 403) throw new Error('更新清单访问被 GitHub 限制：HTTP 403，请稍后或切换网络');
      if (response.status === 404) throw new Error('更新清单不存在：HTTP 404，请确认仓库分支已提交 update.json');
  ...
  async touchConfig() {
    try {
      await this.exec('mkdir -p ' + shellQuote(APPS_DIR) + ' && touch ' + shellQuote(APPS_DIR) + ' 2>/dev/null && touch ' + shellQuote(GLOBAL_CONFIG) + ' 2>/dev/null');
      return true;
    } catch {
      return false;
    }
  ...
  async restoreConfigSnapshot(snapshot) {
    ...
    try {
      await this.exec(managedTempCleanupCommand(stage, rollback) + '; mkdir -p ' + shellQuote(stageApps));
      const globalOk = await this.writeRawFile(stage + '/global.json', JSON.stringify(globalConfig, null, 2) + '\n', { mode: '644' });
      if (!globalOk) throw new Error('write staged global failed');
      const templatesOk = await this.writeRawFile(stage + '/templates.json', JSON.stringify({ templates }, null, 2) + '\n', { mode: '644' });
      if (!templatesOk) throw new Error('write staged templates failed');
      const filtersOk = await this.writeRawFile(stage + '/file_monitor_filters.json', JSON.stringify(monitorFilters, null, 2) + '\n', { mode: '644' });
      if (!filtersOk) throw new Error('write staged monitor filters failed');
      const entries = Object.entries(apps)
        .filter(([packageName]) => isSafePackageName(packageName))
        .sort(([left], [right]) => left.localeCompare(right));
      for (const [packageName, config] of entries) {
        const ok = await this.writeRawFile(stageApps + '/' + packageName + '.json', JSON.stringify(config, null, 2) + '\n', { mode: '644' });
        if (!ok) throw new Error('write staged app failed: ' + packageName);
      }
      await this.exec(
        'config=' + shellQuote(CONFIG_DIR) + '; stage=' + shellQuote(stage) + '; rollback=' + shellQuote(rollback) + '; ' +
      ...
      try { await this.exec(managedTempCleanupCommand(stage, rollback)); } catch {}
  ...
  async stopModule() {
    ...
    await this.exec(
      withSrxCtlFallback(
        'stop',
        'mkdir -p ' + shellQuote(CONFIG_DIR) + ' && touch ' + shellQuote(RUNTIME_DISABLE) + ' && printf "{\\"runtime_disabled\\":true}\\n" > ' + shellQuote(RUNTIME_STATE_CONFIG)
      )
    );
    return await this.waitForMediaProviderRestart(beforePids, { timeoutMs: 10000, intervalMs: 250 });
  },
  ...
  async startModule() {
    ...
    await this.exec(
      withSrxCtlFallback(
        'start',
        'mkdir -p ' + shellQuote(CONFIG_DIR) + ' ' + shellQuote(LOGS_DIR) + ' && ' +
        'rm -f ' + shellQuote(RUNTIME_DISABLE) + ' && ' +
        'printf "{\\"runtime_disabled\\":false}\\n" > ' + shellQuote(RUNTIME_STATE_CONFIG) + ' && ' +
        'daemon=' + shellQuote(MODULE_DIR + '/bin/srx_daemon') + '; pidfile=' + shellQuote(LOGS_DIR + '/.srx_daemon.pid') + '; ' +
  ...
  async restartMediaProvider() {
    ...
    const ok = await this.waitForMediaProviderRestart(beforePids, { timeoutMs: 8000, intervalMs: 250 });
    await this.ensureLogCollectors();
    return ok;
  },
  ...
  async getMediaProviderPids() {
    try {
      ...
      return String(out || '').split(/\s+/).filter(Boolean);
    } catch {
      return [];
    }
  },
  ...
  showManagerToast(message) {
    try {
      if (typeof ksu !== 'undefined') {
        if (typeof ksu.toast === 'function') { ksu.toast(message); return true; }
        if (typeof ksu.showToast === 'function') { ksu.showToast(message); return true; }
      }
    } catch {}
  ...
  _mockStore: {
    global: { file_monitor_enabled: true, fuse_fix_enabled: true, verbose_logging_enabled: false, auto_enable_redirect_for_new_apps: false, auto_enable_new_apps_template_id: '', app_config_auto_save: false },
  ...
  async _mockExec(cmd) {
    ...
    if (cmd.includes('__SRX_APP_CONFIG__')) {
      return Object.entries(this._mockStore.apps).map(([packageName, config]) =>
        '\n__SRX_APP_CONFIG__' + packageName + '\n' + JSON.stringify(config)
      ).join('\n');
    }
    // Simulate package list
    if (cmd.includes('pm list packages')) {
      return Object.keys(this._mockStore.apps).join('\n') +
        '\ncom.android.chrome\ncom.twitter.android\ncom.spotify.music';
    }
    ...
    if (cmd.includes('cat')) {
      ...
      if (match) {
        ...
        if (path === FILE_MONITOR_FILTERS_CONFIG) return JSON.stringify(this._mockStore.monitorFilters || DEFAULT_FILE_MONITOR_FILTERS);
        if (path === TEMPLATES_CONFIG) return JSON.stringify({ templates: this._mockStore.templates || [] });
