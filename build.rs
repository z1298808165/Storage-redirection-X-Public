...
mod tests {
    ...
    #[test]
    fn validate_hooker_dex_accepts_minimum_size() {
        let dir = temp_test_dir("valid_dex");
        let dex = dir.join("Hooker.dex");
        std::fs::write(&dex, vec![0u8; MIN_HOOKER_DEX_BYTES as usize]).unwrap();

        assert!(validate_hooker_dex(&dex).is_ok());

        let _ = std::fs::remove_dir_all(dir);
    }

    #[test]
    fn validate_hooker_dex_rejects_small_file() {
        let dir = temp_test_dir("small_dex");
        let dex = dir.join("Hooker.dex");
        std::fs::write(&dex, vec![0u8; (MIN_HOOKER_DEX_BYTES - 1) as usize]).unwrap();

        let err = validate_hooker_dex(&dex).unwrap_err();

        assert!(err.contains("Hooker.dex too small"), "{err}");
        let _ = std::fs::remove_dir_all(dir);
    }

    #[test]
    fn empty_hooker_dex_is_rejected_for_android_release_by_default() {
        assert!(!should_allow_empty_hooker_dex_with(
            "android", false, false, false
        ));
    }

    #[test]
    fn empty_hooker_dex_is_allowed_only_for_explicit_or_development_paths() {
        assert!(should_allow_empty_hooker_dex_with(
