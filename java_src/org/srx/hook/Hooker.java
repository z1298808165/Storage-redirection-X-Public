...
public class Hooker {
  ...
  private static final String READ_ONLY_DENIED_SENTINEL_PREFIX = "__SRX_READ_ONLY_DENIED__:";
  ...
  private static final int MAX_QUERY_PATH_RECORDS = 128;
  ...
  public Object callback(Object[] args) throws Throwable {
    ...
    try {
      ...
      if (result == null) {
        Cursor empty = emptyQueryCursorIfSafe(actualArgs, projectionPatch, callerUid);
    ...
    try {
      ...
      try {
        FuseReadOnlyRequest readOnlyRequest =
            parseProviderOpenReadOnlyRequest(this, args, actualArgs, callerUid);
  ...
  public Object providerFuseCallback(Object[] args) throws Throwable {
    ...
    if (request == null) {
      return insideProviderInternalCall
          ? callBackupWithProviderPassthrough(args)
          : callBackup(args);
    }
    ...
    try {
      if (recordReadOnlyFuseOperation(
          request.kind,
          request.opName,
          request.opFilter,
          request.path,
  ...
  public Object providerFileParentCallback(Object[] args) throws Throwable {
    ...
    if (!isInsertLikeMutation(MEDIA_PROVIDER_MUTATION_METHOD.get())) return callBackup(args);
    File receiver = directoryReceiver(args);
    if (receiver == null) return callBackup(args);
    Integer scopedUid = MEDIA_PROVIDER_CALLER_UID.get();
    int callerUid = scopedUid == null ? android.os.Binder.getCallingUid() : scopedUid.intValue();
  ...
  public Object providerFileOpenCallback(Object[] args) throws Throwable {
    ...
    try {
      ...
      try {
        ...
        if (status == Integer.MIN_VALUE || !shouldAllowFuseFileOpen(request)) throw t;
        Object allowed = createAllowedFileOpenResult(null, request);
        if (allowed == null) throw t;
        logFuseFileOpenAllow(request, status);
        return allowed;
      }
      logFuseRedirect(patch, result);
      int status = fileOpenResultStatus(result);
      if (status == 0 || !shouldAllowFuseFileOpen(request)) return result;
  ...
  public Object attachInfoCallback(Object[] args) throws Throwable {
    ...
    Class<?> clazz =
        provider != null
            ? provider.getClass()
            : args.length >= 1 && args[0] != null ? args[0].getClass() : null;
    if (clazz != null) {
  ...
  public Object callBackup(Object[] args) throws Throwable {
    ...
    if (args != null && args.length == 1 && args[0] instanceof Object[]) args = (Object[]) args[0];
  ...
  private Object callBackupWithProviderPassthrough(Object[] args) throws Throwable {
  ...
  public boolean unhook() {
    return target != null && doUnhook(target);
  }
  ...
  public static boolean installMediaProviderHook() {
    try {
      ...
      if (attachInfo == null) return directHooked;
      Method callback = Hooker.class.getDeclaredMethod("attachInfoCallback", Object[].class);
      callback.setAccessible(true);
      Hooker hooker = new Hooker();
      Method backup = hooker.doHook(attachInfo, callback);
      if (backup == null) return directHooked;
      ...
      try {
        ...
        if (clazz != null) {
          ...
          try {
            ...
            logWarn("java hook media query installer failed " + name, t);
          }
          try {
            tryInstallOpenHook(clazz);
          } catch (Throwable t) {
            logWarn("java hook media open installer failed " + name, t);
          }
          try {
            tryInstallMutationHook(clazz);
          } catch (Throwable t) {
            logWarn("java hook media mutation installer failed " + name, t);
          }
          try {
            tryInstallFuseHook(clazz);
          } catch (Throwable t) {
  ...
  private static Method findAttachInfo(Class<?> cpClass) {
    for (Method m : cpClass.getDeclaredMethods()) {
      if (!"attachInfo".equals(m.getName())) continue;
  ...
  private static synchronized void tryInstallQueryHook(Class<?> clazz) {
    if (!QUERY_HOOK_PENDING) return;
    for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
      String name = c.getName();
      if (!isMediaProviderClass(name)) continue;
      if (!HOOKED_QUERY_CLASSES.add(name)) continue;
      installQueryOn(c);
    }
  }
  ...
  private static synchronized void tryInstallOpenHook(Class<?> clazz) {
    for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
      String name = c.getName();
      if (!isProviderOpenHookClass(name)) continue;
      if (!HOOKED_OPEN_CLASSES.add(name)) continue;
      installOpenOn(c);
    }
  }
  ...
  private static synchronized void tryInstallMediaFileColumnHooks(Class<?> clazz) {
    ...
    try {
      boolean firstClassObservation = MEDIA_FILE_COLUMN_CLASSES_LOGGED.add(clazz);
      ...
      for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
        for (Method method : c.getDeclaredMethods()) {
          ...
          if (!HOOKED_MEDIA_FILE_COLUMN_METHODS.add(method)) continue;
          try {
            method.setAccessible(true);
            Hooker hooker = new Hooker();
            Method backup = hooker.doHook(method, callback);
            if (backup == null) {
              HOOKED_MEDIA_FILE_COLUMN_METHODS.remove(method);
  ...
  private static boolean isProviderOpenMethodName(String name) {
    return "openFile".equals(name)
        || "openAssetFile".equals(name)
  ...
  private static boolean isMediaProviderHooker(Hooker hooker) {
    if (hooker == null || !(hooker.target instanceof Method)) return false;
    return isMediaProviderClass(((Method) hooker.target).getDeclaringClass().getName());
  }

  private static boolean isMediaProviderHookerOrReceiver(Hooker hooker, Object[] rawArgs) {
    if (isMediaProviderHooker(hooker)) return true;
    ...
    for (Class<?> c = receiver.getClass(); c != null; c = c.getSuperclass()) {
      if (isMediaProviderClass(c.getName())) return true;
    }
    return false;
  }
  ...
  private static android.content.ContentProvider providerReceiver(Object[] rawArgs) {
    ...
    if (rawArgs != null && rawArgs.length > 0) {
      ...
      if (rawArgs.length == 1 && rawArgs[0] instanceof Object[]) {
        ...
        if (nested.length > 0) receiver = nested[0];
      }
    }
  ...
  private static void installQueryOn(Class<?> clazz) {
    try {
      ...
      for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
        for (Method method : c.getDeclaredMethods()) {
          if (!"query".equals(method.getName())) continue;
          String sig = describeMethod(method);
          Hooker hooker = new Hooker();
          Method backup = hooker.doHook(method, callback);
          if (backup == null) {
  ...
  private static void installOpenOn(Class<?> clazz) {
    try {
      Method callback = Hooker.class.getDeclaredMethod("providerOpenCallback", Object[].class);
      callback.setAccessible(true);
      for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
        for (Method method : c.getDeclaredMethods()) {
          String name = method.getName();
          if (!isProviderOpenMethodName(name)) continue;
          String sig = describeMethod(method);
          Hooker hooker = new Hooker();
          Method backup = hooker.doHook(method, callback);
          if (backup == null) {
  ...
  private static void installMutationOn(Class<?> clazz) {
    try {
      Method callback = Hooker.class.getDeclaredMethod("providerMutationCallback", Object[].class);
      callback.setAccessible(true);
      for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
        for (Method method : c.getDeclaredMethods()) {
          String name = method.getName();
          if (!"insert".equals(name) && !"update".equals(name) && !"bulkInsert".equals(name))
            continue;
          if (!hasContentValuesParameter(method)) continue;
          String sig = describeMethod(method);
          if (!HOOKED_MUTATION_METHODS.add(sig)) continue;
          Hooker hooker = new Hooker();
          Method backup = hooker.doHook(method, callback);
          if (backup == null) {
  ...
  private static boolean hasContentValuesParameter(Method method) {
    ...
    for (Class<?> param : params) {
      if (ContentValues.class.isAssignableFrom(param)
          || ContentValues[].class.isAssignableFrom(param)) return true;
    }
    return false;
  }
  ...
  private static void installFuseOn(Class<?> clazz) {
    try {
      Method fuseCallback = Hooker.class.getDeclaredMethod("providerFuseCallback", Object[].class);
      Method fileOpenCallback =
          Hooker.class.getDeclaredMethod("providerFileOpenCallback", Object[].class);
      fuseCallback.setAccessible(true);
      fileOpenCallback.setAccessible(true);
      for (Class<?> c = clazz; c != null; c = c.getSuperclass()) {
        for (Method method : c.getDeclaredMethods()) {
          boolean isFileOpen = isFuseFileOpenMethod(method);
          if (!isFileOpen && !isFuseMutationMethod(method)) continue;
          String sig = describeMethod(method);
          Hooker hooker = new Hooker();
          Method backup = hooker.doHook(method, isFileOpen ? fileOpenCallback : fuseCallback);
          if (backup == null) {
  ...
  private static boolean isFuseMutationMethod(Method method) {
    ...
    if (returnType != Integer.TYPE && returnType != Integer.class) return false;
    String name = method.getName();
    Class<?>[] params = method.getParameterTypes();
    if ("insertFileIfNecessaryForFuse".equals(name)) {
      return params.length == 2 && params[0] == String.class && params[1] == Integer.TYPE;
    }
  ...
  private static boolean isFuseFileOpenMethod(Method method) {
    if (method == null) return false;
    String name = method.getName();
    if (!"onFileOpenForFuse".equals(name) && !"onFileOpen".equals(name)) return false;
    Class<?> returnType = method.getReturnType();
    if (!"com.android.providers.media.FileOpenResult".equals(returnType.getName())) return false;
    Class<?>[] params = method.getParameterTypes();
  ...
  private static native Object onMediaProviderQuery(Hooker hooker, Object[] args) throws Throwable;
  ...
  private static native String rewriteMediaStoreBucketId(String bucketId, int callerUid);
  ...
  private static native boolean allowsPublicMappingTarget(String path, int callerUid);
  ...
  private static void logInfo(String message) {
    if (shouldLog()) android.util.Log.i("SRX", message);
  }

  private static void logWarn(String message) {
    if (shouldLog()) android.util.Log.w("SRX", message);
  }

  private static void logWarn(String message, Throwable t) {
    if (shouldLog()) android.util.Log.w("SRX", message, t);
  }

  private static void logDebug(String message) {
    if (shouldLog()) android.util.Log.d("SRX", message);
  }

  private static FuseReadOnlyRequest parseFuseReadOnlyRequest(Hooker hooker, Object[] args) {
    if (hooker == null || !(hooker.target instanceof Method) || args == null) return null;
    String methodName = ((Method) hooker.target).getName();
    if ("insertFileIfNecessaryForFuse".equals(methodName)) {
      if (args.length < 2 || !(args[0] instanceof String)) return null;
      int uid = intArg(args[1], -1);
      return new FuseReadOnlyRequest(
          FUSE_KIND_MKNOD, methodName, "open:create", (String) args[0], "", uid, -1);
    }
    if ("deleteFileForFuse".equals(methodName)) {
      if (args.length < 2 || !(args[0] instanceof String)) return null;
      int uid = intArg(args[1], -1);
      return new FuseReadOnlyRequest(
          FUSE_KIND_UNLINK, methodName, "delete", (String) args[0], "", uid, -1);
    }
    if ("renameForFuse".equals(methodName)) {
      ...
      return new FuseReadOnlyRequest(
          FUSE_KIND_RENAME, methodName, "rename", (String) args[1], (String) args[0], uid, -1);
    }
    if ("isDirAccessAllowedForFuse".equals(methodName)) {
      if (args.length < 3 || !(args[0] instanceof String)) return null;
      int uid = intArg(args[1], -1);
      int accessType = intArg(args[2], FUSE_DIR_ACCESS_FOR_READ);
      if (accessType == FUSE_DIR_ACCESS_FOR_READ) return null;
      if (accessType == FUSE_DIR_ACCESS_FOR_CREATE) {
        return new FuseReadOnlyRequest(
            FUSE_KIND_MKDIR, methodName, "mkdir", (String) args[0], "", uid, accessType);
      }
      if (accessType == FUSE_DIR_ACCESS_FOR_DELETE) {
        return new FuseReadOnlyRequest(
            FUSE_KIND_RMDIR, methodName, "rmdir", (String) args[0], "", uid, accessType);
      }
      if (accessType == FUSE_DIR_ACCESS_FOR_WRITE) {
        return new FuseReadOnlyRequest(
            FUSE_KIND_OPEN, methodName, "open:write", (String) args[0], "", uid, accessType);
      }
      return new FuseReadOnlyRequest(
          FUSE_KIND_OPEN, methodName, "fuse_dir_access", (String) args[0], "", uid, accessType);
    }
    if ("isDirectoryCreationOrDeletionAllowedForFuse".equals(methodName)) {
      if (args.length < 3 || !(args[0] instanceof String)) return null;
      int uid = intArg(args[1], -1);
      boolean forCreate = args[2] instanceof Boolean && ((Boolean) args[2]).booleanValue();
      return new FuseReadOnlyRequest(
          forCreate ? FUSE_KIND_MKDIR : FUSE_KIND_RMDIR,
          methodName,
          forCreate ? "mkdir" : "rmdir",
          (String) args[0],
          "",
          uid,
          forCreate ? FUSE_DIR_ACCESS_FOR_CREATE : FUSE_DIR_ACCESS_FOR_DELETE);
    }
    return null;
  }

  private static FusePathPatch patchFuseMutationArgs(
      Object[] rawArgs, Object[] actualArgs, FuseReadOnlyRequest request) {
    if (actualArgs == null || request == null || !shouldRewriteFusePath(request)) return null;
  ...
  private static FusePathPatch patchFuseFileOpenArgs(
      Object[] rawArgs, Object[] actualArgs, FuseFileOpenRequest request) {
    if (actualArgs == null
        || request == null
        || !request.forWrite
        || request.callerUid < ANDROID_APP_UID_START) return null;
    FuseReadOnlyRequest rewriteRequest =
        new FuseReadOnlyRequest(
            FUSE_KIND_OPEN,
            ...
            request.callerUid,
            -1);
    FusePathPatch patch = rewriteFusePathArg(rawArgs, actualArgs, rewriteRequest, 0, null);
  ...
  private static boolean shouldRewriteFusePath(FuseReadOnlyRequest request) {
    if (request.callerUid < ANDROID_APP_UID_START) return false;
    if (request.kind == FUSE_KIND_MKDIR
        || request.kind == FUSE_KIND_MKNOD
        || request.kind == FUSE_KIND_RENAME
        || request.kind == FUSE_KIND_UNLINK
        || request.kind == FUSE_KIND_RMDIR) return true;
    return request.kind == FUSE_KIND_OPEN
        && request.opFilter != null
        && (request.opFilter.indexOf("write") >= 0 || request.opFilter.indexOf("create") >= 0);
  }

  private static FusePathPatch rewriteFusePathArg(
      Object[] rawArgs,
      Object[] actualArgs,
    ...
    if (actualArgs == null
        || index < 0
        || index >= actualArgs.length
        || !(actualArgs[index] instanceof String)) return previous;
    String original = (String) actualArgs[index];
    String mapped = resolveOpenPathSafe(original, request.callerUid);
    if (mapped == null || mapped.length() == 0 || mapped.equals(original)) return previous;
    replaceActualArg(rawArgs, actualArgs, index, mapped);
    return new FusePathPatch(
        request.opName, request.kind, request.callerUid, index, original, mapped);
  }
  ...
  private static FuseFileOpenRequest parseFuseFileOpenRequest(Hooker hooker, Object[] args) {
    if (hooker == null || !(hooker.target instanceof Method) || args == null) return null;
    Method method = (Method) hooker.target;
    if (!isFuseFileOpenMethod(method)) return null;
    if (args.length < 6 || !(args[0] instanceof String) || !(args[1] instanceof String))
      return null;
    int uid = intArg(args[2], -1);
    boolean forWrite = isFuseFileOpenForWrite(args);
    return new FuseFileOpenRequest(method, (String) args[0], (String) args[1], uid, forWrite);
  }
  ...
  private static boolean fileOpenModeHasWriteIntent(int mode) {
    ...
    if (access == android.system.OsConstants.O_WRONLY
        || access == android.system.OsConstants.O_RDWR) return true;
    if ((mode
            & (android.system.OsConstants.O_CREAT
                | android.system.OsConstants.O_TRUNC
                | android.system.OsConstants.O_APPEND))
        != 0) return true;
    return (mode & 0x20000000) != 0
        || // ParcelFileDescriptor write bit.
  ...
  private static boolean allowsPublicMappingTargetSafe(String path, int callerUid) {
    if (path == null || path.length() == 0) return false;
    try {
      return allowsPublicMappingTarget(path, callerUid);
    } catch (Throwable ignored) {
      return false;
    }
  }
  ...
  private static int fileOpenThrowableStatus(Throwable t) {
    if (t instanceof FileNotFoundException) {
      ...
      if (message != null
          && (message.indexOf("EACCES") >= 0 || message.indexOf("Permission denied") >= 0)) {
  ...
  private static Object createAllowedFileOpenResult(Object original, FuseFileOpenRequest request) {
    try {
      Class<?> resultClass =
          original != null ? original.getClass() : request.method.getReturnType();
      Constructor<?> ctor =
      ...
      return ctor.newInstance(
          Integer.valueOf(0), Integer.valueOf(request.callerUid), Integer.valueOf(0), new long[0]);
    } catch (Throwable t) {
      logWarn(
          "fuse file open allow result failed path=" + (request == null ? "" : request.path), t);
      return null;
    }
  }
  ...
  private static void logFuseFileOpenAllow(FuseFileOpenRequest request, int originalStatus) {
    if (request == null) return;
    if (!shouldLog()) return;
    if (FUSE_FILE_OPEN_ALLOW_LOG_COUNT >= 96) return;
  ...
  private static void logFuseFileOpenMode(int mode, boolean write) {
    if (!shouldLog()) return;
    if (FUSE_FILE_OPEN_MODE_LOG_COUNT >= 64) return;
    FUSE_FILE_OPEN_MODE_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX",
          "java fuse file open mode=0x"
  ...
  private static void logFuseRedirect(FusePathPatch patch, Object result) {
    if (patch == null || !shouldLog()) return;
    if (FUSE_REDIRECT_LOG_COUNT >= 96) return;
    FUSE_REDIRECT_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX",
          "java fuse redirect op="
              + patch.opName
              + " kind="
              + patch.kind
              + " uid="
  ...
  private static void logReadOnlyDeny(String source, FuseReadOnlyRequest request) {
    if (request == null) return;
    if (!shouldLog()) return;
    if (FUSE_READ_ONLY_LOG_COUNT >= 96) return;
  ...
  private static FuseReadOnlyRequest parseProviderOpenReadOnlyRequest(
    ...
    try {
      if (hooker == null || !(hooker.target instanceof Method)) return null;
      Method method = (Method) hooker.target;
      if (!isProviderOpenMethodName(method.getName())) return null;
      OpenRequest request = parseOpenRequest(method.getName(), actualArgs);
      if (request == null || !isWriteMode(request.mode)) return null;
      String uriText = String.valueOf(request.uri);
      if (!uriText.startsWith("content://media/")) return null;

      String path = null;
      PendingDirectMediaWrite pending = peekPendingDirectMediaWrite(request.uri);
      if (pending != null && pending.callerUid == callerUid) path = pending.path;
      if (path == null || path.length() == 0) {
        android.content.ContentProvider receiver = providerReceiver(rawArgs);
        if (receiver != null) path = queryDataPath(receiver, request.uri);
      }
      return buildProviderOpenReadOnlyRequest(method.getName(), request, path, callerUid);
    } catch (Throwable ignored) {
      return null;
    }
  }
  ...
  private static FuseReadOnlyRequest buildProviderOpenReadOnlyRequest(
    ...
    if (request == null || path == null || path.length() == 0) return null;
  ...
  private static boolean recordReadOnlyProviderOpenIfNeeded(FuseReadOnlyRequest request) {
    return request != null
        && recordReadOnlyFuseOperation(
            request.kind,
            request.opName,
            request.opFilter,
            request.path,
  ...
  private static boolean hasCreateIntentMode(String mode) {
    ...
    return lower.indexOf('w') >= 0 || lower.indexOf('a') >= 0 || lower.indexOf('t') >= 0;
  }
  ...
  private static int openFlagsForMode(String mode) {
    ...
    boolean writable =
        lower.indexOf('w') >= 0 || lower.indexOf('a') >= 0 || lower.indexOf('t') >= 0;
    if (writable) {
      flags |= readable ? android.system.OsConstants.O_RDWR : android.system.OsConstants.O_WRONLY;
      flags |= android.system.OsConstants.O_CREAT;
      if (lower.indexOf('a') >= 0) flags |= android.system.OsConstants.O_APPEND;
      if (lower.indexOf('t') >= 0 || "w".equals(lower)) flags |= android.system.OsConstants.O_TRUNC;
      return flags;
    }
    if (readable) return android.system.OsConstants.O_RDONLY;
    return -1;
  }

  private static FileNotFoundException readOnlyOpenException(String path) {
    return new FileNotFoundException(
        (path == null || path.length() == 0 ? "media file" : path) + ": Read-only file system");
  }
  ...
  private static final class FuseFileOpenRequest {
    ...
    FuseFileOpenRequest(
        Method method, String path, String ioPath, int callerUid, boolean forWrite) {
      this.method = method;
      this.path = path == null ? "" : path;
  ...
  private static final class FusePathPatch {
    ...
    FusePathPatch(
        String opName, int kind, int callerUid, int argIndex, String fromPath, String toPath) {
      this.opName = opName == null ? "" : opName;
      this.kind = kind;
      this.callerUid = callerUid;
  ...
  private Object tryOpenMappedMediaFile(Object[] rawArgs, int callerUid) throws Throwable {
    String path = null;
    String mappedPath = null;
    ...
    try {
      ...
      if (mappedPath == null || mappedPath.length() == 0 || mappedPath.equals(path)) {
        logOpenDelegate("mapped_resolve_miss", path, mappedPath == null ? "" : mappedPath);
        return null;
      }
  ...
  private static OpenRequest parseOpenRequest(String methodName, Object[] args) {
    if (args == null || args.length == 0 || !(args[0] instanceof android.net.Uri)) return null;
    android.net.Uri uri = (android.net.Uri) args[0];
    if ("openTypedAssetFile".equals(methodName)) return new OpenRequest(uri, "r");
    if (args.length < 2 || !(args[1] instanceof String)) return null;
  ...
  private static boolean isSrxSandboxFallbackPathWithPrefix(String path, String prefix) {
  ...
  private static String queryDataPath(
  ...
  private static String packageNameForUid(int uid) {
    ...
    try {
      ...
      if (app == null) return "";
      if (!(app instanceof android.content.Context)) return "";
  ...
  private static MutationPatchResult patchMediaStoreValues(
    ...
    android.net.Uri mutationUri = uriIndex >= 0 ? (android.net.Uri) actualArgs[uriIndex] : null;
      ...
      if (arg instanceof ContentValues) {
        ...
        for (int j = 0; j < values.length; j++) {
          ...
          if (patch.values != values[j]) {
            if (patchedValues == null) patchedValues = values.clone();
    ...
    if (insertLike
      ...
      android.net.Uri patchedUri = rewriteMutationCollectionUri(mutationUri, requestedCollection);
  ...
  private static ContentValuesPatch patchContentValues(
    ...
    if (dataKey != null) {
      ...
      if (mediaPatch != null) {
        ...
        if (mediaPatch.mimeType != null) patched.put("mime_type", mediaPatch.mimeType);
    ...
    if (insertLike && relativePath != null && relativePath.length() > 0) {
      ...
      DownloadMediaPathPatch mediaPatch =
          rewriteDownloadPlaceholderRelative(
              relativePath, displayName, values, callerUid, mutationUri);
      if (mediaPatch != null) {
        patched = copyIfNeeded(patched, values);
        patched.put("relative_path", mediaPatch.relativePath);
        patchDirectoryColumns(patched, mediaPatch.relativePath);
    ...
    if (!insertLike && mutationUri != null) {
      ContentValues sizePatched = patchPendingDirectMediaUpdate(patched, values, mutationUri);
      if (sizePatched != null) patched = sizePatched;
    }

    return new ContentValuesPatch(
  ...
  private static int findMutationUriIndex(Object[] actualArgs) {
    ...
    for (int i = 0; i < actualArgs.length; i++) {
      if (actualArgs[i] instanceof android.net.Uri) return i;
    }
    return -1;
  }
  ...
  private static String mergeRequestedMediaCollection(String current, String next) {
    if (next == null || next.length() == 0) return current;
    if (current == null || current.length() == 0 || current.equals(next)) return next;
    return null;
  }

  private static android.net.Uri rewriteMutationCollectionUri(
      android.net.Uri uri, String mediaCollection) {
    if (uri == null || mediaCollection == null || mediaCollection.length() == 0) return null;
    try {
      String value = uri.toString();
      if (!isDownloadsCollectionUri(uri)) return null;
      ...
      if (base.endsWith(replacement)) return null;
      if (base.endsWith("/downloads")) {
        return android.net.Uri.parse(
            base.substring(0, base.length() - "/downloads".length()) + replacement + suffix);
      }
    } catch (Throwable ignored) {
    }
    return null;
  ...
  private static String mediaCollectionUriPath(String mediaCollection) {
    if ("video".equals(mediaCollection)) return "video/media";
    return "images/media";
  }
  ...
  private static ContentValues patchPendingDirectMediaUpdate(
    ...
    if (pending == null || pending.bytes <= 0) return null;
    boolean hasSize = original.containsKey("size");
    boolean hasDataSize = original.containsKey("_size");
    if (!hasSize && !hasDataSize) return null;
    ContentValues patched = copyIfNeeded(current, original);
    Long size = Long.valueOf(pending.bytes);
    if (hasSize) patched.put("size", size);
    if (hasDataSize) patched.put("_size", size);
    return patched;
  }

  private static DownloadMediaPathPatch rewriteDownloadPlaceholderPath(
      String originalPath, ContentValues values, int callerUid, android.net.Uri mutationUri)
      throws FileNotFoundException {
    if (!isDownloadsCollectionUri(mutationUri) || !isLikelyDownloadMediaPlaceholder(values))
      return null;
    DownloadPathParts parts = parseDownloadPathParts(originalPath, callerUid);
    if (parts == null || parts.bucket.length() == 0 || parts.fileName.length() == 0) return null;
    String collection = preferredMediaCollection(values);
    String mappedPath =
        resolveDownloadPlaceholderSafe(
            originalPath, null, parts.fileName, "video".equals(collection), callerUid);
    throwIfReadOnlyDownloadDenied(mappedPath);
    if (mappedPath == null || mappedPath.length() == 0) return null;
    String mappedRelative = relativePathFromStoragePath(mappedPath, callerUid);
    if (mappedRelative == null || mappedRelative.length() == 0) return null;
    String mimeType = normalizedMediaMimeType(values, "video".equals(collection));
    logDownloadMediaPathPatch(originalPath, mappedPath, mappedRelative, collection, callerUid);
    return new DownloadMediaPathPatch(mappedPath, mappedRelative, mimeType, collection);
  }

  private static DownloadMediaPathPatch rewriteDownloadPlaceholderRelative(
      String relativePath,
      String displayName,
    ...
    if (!isDownloadsCollectionUri(mutationUri) || !isLikelyDownloadMediaPlaceholder(values))
      return null;
    DownloadPathParts parts = parseDownloadRelativePathParts(relativePath, displayName, callerUid);
    if (parts == null || parts.bucket.length() == 0 || parts.fileName.length() == 0) return null;
    String collection = preferredMediaCollection(values);
    String mappedPath =
        resolveDownloadPlaceholderSafe(
            null, relativePath, parts.fileName, "video".equals(collection), callerUid);
    throwIfReadOnlyDownloadDenied(mappedPath);
    if (mappedPath == null || mappedPath.length() == 0) return null;
    String mappedRelative = relativePathFromStoragePath(mappedPath, callerUid);
    if (mappedRelative == null || mappedRelative.length() == 0) return null;
    String mimeType = normalizedMediaMimeType(values, "video".equals(collection));
    logDownloadMediaPathPatch(
        "relative:" + relativePath, mappedPath, mappedRelative, collection, callerUid);
    return new DownloadMediaPathPatch(mappedPath, mappedRelative, mimeType, collection);
  }

  private static String resolveDownloadPlaceholderSafe(
      String originalPath, String relativePath, String displayName, boolean video, int callerUid) {
    try {
      return resolveDownloadPlaceholder(originalPath, relativePath, displayName, video, callerUid);
    } catch (Throwable ignored) {
      return null;
    }
  }

  private static void throwIfReadOnlyDownloadDenied(String path) throws FileNotFoundException {
    if (path == null || !path.startsWith(READ_ONLY_DENIED_SENTINEL_PREFIX)) return;
  ...
  private static String preferredMediaCollection(ContentValues values) {
    ...
    if (mimeType != null && mimeType.toLowerCase(Locale.ROOT).startsWith("video/")) return "video";
    return "images";
  }

  private static String normalizedMediaMimeType(ContentValues values, boolean video) {
    String mimeType = values == null ? null : values.getAsString("mime_type");
    if (mimeType != null) {
      String lower = mimeType.toLowerCase(Locale.ROOT);
      if (lower.startsWith("image/") || lower.startsWith("video/")) return mimeType;
    }
    return video ? "video/mp4" : "image/jpeg";
  }

  private static DownloadPathParts parseDownloadPathParts(String path, int callerUid) {
    if (path == null || path.length() == 0) return null;
    String value = path.startsWith("file://") ? path.substring("file://".length()) : path;
    int userId = userIdFromUid(callerUid);
    if (userId < 0) return null;
    ...
    if (!relative.startsWith(downloadPrefix)) return null;
    ...
    if (bucket.length() == 0
        || hasUnsafeRelativePathSegment(bucket)
        || fileName.length() == 0
        || fileName.indexOf('\\') >= 0) return null;
  ...
  private static DownloadPathParts parseDownloadRelativePathParts(
    ...
    String downloadPrefix = "Download/";
    if (!relative.startsWith(downloadPrefix)) return null;
    String bucket = normalizeRelativePathValue(relative.substring(downloadPrefix.length()));
    if (bucket.length() == 0 || hasUnsafeRelativePathSegment(bucket)) return null;
    String fileName = displayName;
    if (fileName == null || fileName.length() == 0) fileName = ".srx_media";
    if (fileName.indexOf('/') >= 0 || fileName.indexOf('\\') >= 0) return null;
    return new DownloadPathParts("/storage/emulated/" + userId + "/", bucket, fileName);
  }
  ...
  private static boolean hasUnsafeRelativePathSegment(String relativePath) {
    ...
    for (String part : parts) {
      if (part.length() == 0 || ".".equals(part) || "..".equals(part)) return true;
    }
    return false;
  }

  private static String normalizeStorageDisplayPath(String path, int callerUid) {
    if (path == null || path.length() == 0) return null;
    String value = path.startsWith("file://") ? path.substring("file://".length()) : path;
    int userId = userIdFromUid(callerUid);
    if (userId < 0) return value;
    String dataRoot = "/data/media/" + userId + "/";
    if (value.startsWith(dataRoot)) {
      value = "/storage/emulated/" + userId + "/" + value.substring(dataRoot.length());
    }
  ...
  private static void logDownloadMediaPathPatch(
      String originalPath,
      String mappedPath,
      String relativePath,
      String collection,
      int callerUid) {
    if (!shouldLog()) return;
    if (DOWNLOAD_MEDIA_PATCH_LOG_COUNT >= 64) return;
  ...
  private static final class ContentValuesPatch {
    ...
    ContentValuesPatch(ContentValues values, String mediaCollection, boolean directWriteRequested) {
      this.values = values;
  ...
  private static final class DownloadMediaPathPatch {
    ...
    DownloadMediaPathPatch(
        String mappedPath, String relativePath, String mimeType, String mediaCollection) {
  ...
  private static void captureMediaSourceFileDescriptor(
      Hooker hooker, Object[] actualArgs, int callerUid) {
    try {
      if (!(hooker != null && hooker.target instanceof Method)) return;
      Method method = (Method) hooker.target;
      if (!"openTypedAssetFile".equals(method.getName())) return;
      if (actualArgs == null
          || actualArgs.length < 3
          || !(actualArgs[0] instanceof android.net.Uri)) return;
      String uri = String.valueOf(actualArgs[0]);
      if (!uri.startsWith("content://media/") || !uri.endsWith("/file")) return;
      ...
      if (bundle == null || !bundle.containsKey("file_descriptor")) return;
      Object fdValue = bundle.getParcelable("file_descriptor");
      if (!(fdValue instanceof ParcelFileDescriptor)) return;
      ParcelFileDescriptor dup =
          ParcelFileDescriptor.dup(((ParcelFileDescriptor) fdValue).getFileDescriptor());
      ...
      SourceFdCapture capture =
          new SourceFdCapture(
              dup, sourceFileDescriptorPath(dup), size, android.os.SystemClock.elapsedRealtime());
  ...
  private static void registerDirectWriteAfterInsert(
      Object[] rawArgs,
      Object result,
      int callerUid,
    ...
    try {
      if (!directWriteRequested
          || !"insert".equals(mutationMethod)
          || !(result instanceof android.net.Uri)) return;
      Object receiver = rawArgs != null && rawArgs.length > 0 ? rawArgs[0] : null;
      String path = null;
      if (receiver instanceof android.content.ContentProvider) {
        path = queryDataPath((android.content.ContentProvider) receiver, (android.net.Uri) result);
      }
  ...
  private static void completeDirectMediaWriteFromSource(
    ...
    try {
      ...
      if (request == null || !isWriteMode(request.mode)) return;
      PendingDirectMediaWrite pending = peekPendingDirectMediaWrite(request.uri);
      if (pending == null || pending.callerUid != callerUid) return;
      Object receiver = rawArgs != null && rawArgs.length > 0 ? rawArgs[0] : null;
      String path = pending.path;
      if ((path == null || path.length() == 0)
          && receiver instanceof android.content.ContentProvider) {
        path = queryDataPath((android.content.ContentProvider) receiver, request.uri);
        pending.path = path;
      }
      if (path == null || path.length() == 0) {
        logDirectMediaWrite("no_path", callerUid, String.valueOf(request.uri), null, 0, null);
        return;
      }
      FuseReadOnlyRequest readOnlyRequest =
          buildProviderOpenReadOnlyRequest(method.getName(), request, path, callerUid);
      if (recordReadOnlyProviderOpenIfNeeded(readOnlyRequest)) {
        logProviderOpenReadOnlyDeny(readOnlyRequest);
        logDirectMediaWrite("readonly", callerUid, String.valueOf(request.uri), path, 0, null);
        closeQuietly(result);
        throw readOnlyOpenException(path);
      }
      source = takeRecentMediaSourceFd(callerUid);
      if (source == null) {
        logDirectMediaWrite("no_source", callerUid, String.valueOf(request.uri), path, 0, null);
        return;
      }
      ...
      logDirectMediaWrite(
          bytes > 0 ? "copied" : "empty",
          callerUid,
          String.valueOf(request.uri),
          path,
          bytes,
          source.path);
    } catch (FileNotFoundException denied) {
  ...
  private static void finishDirectMediaWriteAfterUpdate(
    ...
    try {
      ...
      if (pending == null || pending.bytes <= 0) return;
      synchronized (PENDING_DIRECT_MEDIA_WRITES) {
        PENDING_DIRECT_MEDIA_WRITES.remove(String.valueOf(actualArgs[uriIndex]));
      }
    } catch (Throwable ignored) {
    }
  }
  ...
  private static long copySourceToPath(SourceFdCapture source, String destPath) throws Exception {
    if (source == null || source.pfd == null || destPath == null || destPath.length() == 0)
      return 0;
    File dest = new File(destPath);
    if (dest.exists() && dest.length() > 0) return dest.length();
    File parent = dest.getParentFile();
    if (parent != null && !parent.exists()) parent.mkdirs();
    try {
      android.system.Os.lseek(
          source.pfd.getFileDescriptor(), 0, android.system.OsConstants.SEEK_SET);
    } catch (Throwable ignored) {
    }
    long total = 0;
    java.io.FileInputStream in = new java.io.FileInputStream(source.pfd.getFileDescriptor());
  ...
  private static void closeQuietly(SourceFdCapture capture) {
    if (capture != null) closeQuietly(capture.pfd);
  }
  ...
  private static void logMediaSourceCapture(int callerUid, SourceFdCapture capture) {
    if (!shouldLog()) return;
    if (MEDIA_SOURCE_CAPTURE_LOG_COUNT >= 96) return;
    MEDIA_SOURCE_CAPTURE_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX",
          "java media source capture caller_uid="
              + callerUid
              + " size="
  ...
  private static void logDirectMediaWrite(
      String stage, int callerUid, String uri, String path, long bytes, String sourcePath) {
    if (!shouldLog()) return;
    if (DIRECT_MEDIA_WRITE_LOG_COUNT >= 96) return;
    DIRECT_MEDIA_WRITE_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX",
          "java media direct write stage="
              + stage
              + " caller_uid="
              + callerUid
              + " uri="
  ...
  private static final class SourceFdCapture {
    ...
    SourceFdCapture(ParcelFileDescriptor pfd, String path, long size, long elapsedMs) {
      this.pfd = pfd;
      this.path = path;
      this.size = size;
  ...
  private static void replaceActualArg(
      Object[] rawArgs, Object[] actualArgs, int index, Object value) {
    ...
    if (rewritten != null) {
      if (rewritten.equals(path)) {
        ...
        if (unchangedFallback == null)
          unchangedFallback = normalizeRelativeDataPath(path, callerUid);
  ...
  private static String mediaStoreDisplayPath(String path, int callerUid) {
    ...
    if (userId < 0) return null;
    String dataRoot = "/data/media/" + userId + "/";
    if (!value.startsWith(dataRoot)) return null;
    ...
    if (relative.length() > 0) displayPath = displayPath + "/" + relative;
    return hasFileScheme ? "file://" + displayPath : displayPath;
  }

  private static String normalizeRelativeDataPath(String path, int callerUid) {
  ...
  private static boolean isPublicMediaRoot(String root) {
    ...
    for (int i = 0; i < PUBLIC_MEDIA_ROOTS.length; i++) {
      if (PUBLIC_MEDIA_ROOTS[i].equals(root)) return true;
    }
    return false;
  }

  private static String buildMediaStoreProbePath(
      String relativePath, String displayName, int callerUid) {
    int userId = userIdFromUid(callerUid);
    if (userId < 0) return null;
    String relative = normalizeRelativePathValue(relativePath);
    ...
    if (relative.startsWith(absoluteRoot)) relative = relative.substring(absoluteRoot.length());
    else if (relative.startsWith("/" + absoluteRoot))
      relative = relative.substring(absoluteRoot.length() + 1);
    String name = displayName != null && displayName.length() > 0 ? displayName : ".srx_probe";
  ...
  private static void patchDirectoryColumns(ContentValues values, String relativePath) {
    String relative = normalizeRelativePathValue(relativePath);
    if (relative.length() == 0) return;
    int slash = relative.indexOf('/');
    String primary = slash >= 0 ? relative.substring(0, slash) : relative;
    String secondary = slash >= 0 ? relative.substring(slash + 1) : null;
    if (values.containsKey("primary_directory")) values.put("primary_directory", primary);
    if (values.containsKey("secondary_directory")) values.put("secondary_directory", secondary);
  }

  private static void patchRelativePathFromDataIfMissing(
      ContentValues values, String dataPath, int callerUid) {
    if (values == null || dataPath == null || dataPath.length() == 0) return;
  ...
  private static String relativePathFromDirectoryColumns(ContentValues values) {
    ...
    String primary = normalizeRelativePathValue(values.getAsString("primary_directory"));
    if (primary.length() == 0 || hasUnsafeRelativePathSegment(primary)) return null;
    String secondary = normalizeRelativePathValue(values.getAsString("secondary_directory"));
    if (secondary.length() > 0) {
      if (hasUnsafeRelativePathSegment(secondary)) return null;
      primary = primary + "/" + secondary;
    }
    return primary + "/";
  }

  private static String firstString(ContentValues values, String firstKey, String secondKey) {
  ...
  private boolean isStaticTarget() {
    return target instanceof Method && Modifier.isStatic(((Method) target).getModifiers());
  }

  private static void logQueryArgs(Hooker hooker, Object[] args, int callerUid) {
    if (!shouldLog()) return;
    if (callerUid == android.os.Process.myUid()) return;
    if (QUERY_LOG_COUNT >= 64) return;
    QUERY_LOG_COUNT++;
    try {
      String targetSig =
  ...
  private static void logInternalQueryBypass(Hooker hooker) {
    if (!shouldLog()) return;
    if (INTERNAL_QUERY_LOG_COUNT >= 16) return;
    ...
    try {
      ...
      android.util.Log.i(
          "SRX",
          "java query internal passthrough target=" + targetSig + " n=" + INTERNAL_QUERY_LOG_COUNT);
    } catch (Throwable ignored) {
    }
  }
  ...
  private static void logQueryResult(Object result) {
    if (!shouldLog()) return;
    if (CURSOR_LOG_COUNT >= 64) return;
    CURSOR_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX", "java query result=" + (result == null ? "null" : result.getClass().getName()));
    } catch (Throwable ignored) {
    }
  }

  private static void logQueryInvocationShape(Object[] actualArgs, int callerUid, Object result) {
    if (!shouldLog()) return;
    if (QUERY_LOG_COUNT >= 64) return;
    try {
  ...
  private static Cursor emptyQueryCursorIfSafe(
      Object[] actualArgs, ProjectionPatch projectionPatch, int callerUid) {
    NullQueryDecision decision = shouldConvertNullMediaQueryToEmpty(actualArgs, callerUid);
    ...
    } else if (actualArgs != null && actualArgs.length > 1 && actualArgs[1] instanceof String[]) {
      columns = (String[]) actualArgs[1];
    }
    if (columns == null || columns.length == 0) columns = new String[] {"_id"};
  ...
  private static NullQueryDecision shouldConvertNullMediaQueryToEmpty(
    ...
    if (callerUid < ANDROID_APP_UID_START
        || actualArgs == null
        || actualArgs.length < 2
        || !(actualArgs[0] instanceof android.net.Uri)) return NullQueryDecision.skip("shape");
    if (!isRedirectEnabledForCallerUid(callerUid)) return NullQueryDecision.skip("redirect");
    if (isSingleItemQuery(actualArgs)) return NullQueryDecision.skip("single_item");
    String uri = String.valueOf(actualArgs[0]);
    if (!uri.startsWith("content://media/external")) return NullQueryDecision.skip("uri");
    if (!isMediaEnumerationProjection(actualArgs)) return NullQueryDecision.skip("projection");
    return NullQueryDecision.convert();
  }

  private static boolean isMediaEnumerationProjection(Object[] actualArgs) {
    if (actualArgs == null || actualArgs.length < 2 || !(actualArgs[1] instanceof String[]))
      return false;
  ...
  private static void logEmptyQueryCursor(String[] columns, int callerUid, Object[] actualArgs) {
    if (!shouldLog()) return;
    if (QUERY_NULL_EMPTY_LOG_COUNT >= 32) return;
    QUERY_NULL_EMPTY_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX",
          "java query null_to_empty caller_uid="
              + callerUid
              + " columns="
              + Arrays.toString(columns)
              + " args="
              + describeQueryArgs(actualArgs)
              + " n="
              + QUERY_NULL_EMPTY_LOG_COUNT);
    } catch (Throwable ignored) {
    }
  ...
  private static void logNullQuerySkip(String reason, int callerUid, Object[] actualArgs) {
    if (!shouldLog()) return;
    if (QUERY_NULL_EMPTY_LOG_COUNT >= 32) return;
    QUERY_NULL_EMPTY_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX",
          "java query null_to_empty skip reason="
              + reason
              + " caller_uid="
              + callerUid
              + " args="
  ...
  private static void logOpenArgs(Hooker hooker, Object[] args, int callerUid, int callerPid) {
    if (!shouldLog()) return;
    if (OPEN_LOG_COUNT >= 96) return;
    OPEN_LOG_COUNT++;
    try {
      String targetSig =
          hooker != null && hooker.target instanceof Method
  ...
  private static void logOpenResult(String source, Object result) {
    if (!shouldLog()) return;
    if (OPEN_RESULT_LOG_COUNT >= 96) return;
    OPEN_RESULT_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX",
          "java open result source="
              + source
              + " class="
  ...
  private static void logMutationArgs(
      Hooker hooker, Object[] args, int callerUid, int callerPid, boolean patched) {
    if (!shouldLog()) return;
    if (callerUid == android.os.Process.myUid()) return;
  ...
  private static void logMutationResult(Hooker hooker, Object result) {
    if (!shouldLog()) return;
    if (MUTATION_RESULT_LOG_COUNT >= 96) return;
    ...
    try {
      ...
      android.util.Log.i(
          "SRX",
          "java media mutation result target="
              + targetSig
              + " result="
              + String.valueOf(result)
              + " class="
  ...
  private static Object[] unwrapArgs(Object[] args) {
    ...
    if (args.length == 1 && args[0] instanceof Object[]) return (Object[]) args[0];
  ...
  private static final class ProjectionPatch {
    ...
    static ProjectionPatch apply(Object[] rawArgs, Object[] actualArgs) {
      try {
        ...
        if (projection == null
            || projection.length == 0
            || hasPathColumn(projection)
            || !hasIdColumn(projection)) {
          return NONE;
        }
    ...
    private static boolean hasPathColumn(String[] columns) {
      for (String column : columns) {
        if ("_data".equals(column) || "data".equalsIgnoreCase(column)) return true;
      }
      return false;
    }
    ...
    private static boolean hasIdColumn(String[] columns) {
      for (String column : columns) {
        if ("_id".equals(column)) return true;
      }
      return false;
    }
  }
  ...
  private static final class SelectionPatch {
    ...
    private static void patchLegacySelection(Object[] rawArgs, Object[] actualArgs, int callerUid) {
      if (actualArgs.length < 4
          || !(actualArgs[2] instanceof String)
          || !(actualArgs[3] instanceof String[])) return;
      ...
      if (patched == selectionArgs && patchedSelection == selection) return;
    ...
    private static void patchBundleSelection(Object[] rawArgs, Object[] actualArgs, int callerUid) {
      for (int i = 0; i < actualArgs.length; i++) {
        Object arg = actualArgs[i];
        if (!(arg instanceof android.os.Bundle)) continue;
        android.os.Bundle bundle = (android.os.Bundle) arg;
        String selection = bundle.getString("android:query-arg-sql-selection");
        String[] selectionArgs = bundle.getStringArray("android:query-arg-sql-selection-args");
        String[] patched = patchSelectionArgs(selection, selectionArgs, callerUid);
        String patchedSelection = patchInlineBucketIds(selection, callerUid);
        if (patched == selectionArgs && patchedSelection == selection) continue;
    ...
    private static String patchInlineBucketIds(String selection, int callerUid) {
      if (selection == null
          || selection.length() == 0
          || !containsColumn(selection.toLowerCase(Locale.ROOT), "bucket_id")) return selection;
    ...
    private static String patchInlineBucketIdEquals(String selection, int callerUid) {
      ...
      while (matcher.find()) {
        ...
        if (replacement == null || replacement.equals(matcher.group(3))) continue;
        if (out == null) out = new StringBuffer(selection.length());
      ...
      if (out == null) return selection;
      matcher.appendTail(out);
      return out.toString();
    }

    private static String patchInlineBucketIdIn(String selection, int callerUid) {
      Matcher matcher = INLINE_BUCKET_ID_IN.matcher(selection);
      StringBuffer out = null;
      while (matcher.find()) {
        String values = matcher.group(2);
        String patchedValues = patchInlineBucketIdList(values, callerUid);
        if (patchedValues == null || patchedValues.equals(values)) continue;
        if (out == null) out = new StringBuffer(selection.length());
        matcher.appendReplacement(
            out, Matcher.quoteReplacement(matcher.group(1) + patchedValues + matcher.group(3)));
      }
      if (out == null) return selection;
      matcher.appendTail(out);
      return out.toString();
    ...
    private static String patchInlineBucketIdList(String values, int callerUid) {
      ...
      for (int i = 0; i < parts.length; i++) {
        ...
        if (!isInteger(numeric)) continue;
        String replacement = rewriteBucketIdValue(numeric, callerUid);
        if (replacement == null || replacement.equals(numeric)) continue;
        if (patched == null) patched = parts.clone();
    ...
    private static String[] patchSelectionArgs(
        String selection, String[] selectionArgs, int callerUid) {
      if (selection == null || selectionArgs == null || selectionArgs.length == 0)
        return selectionArgs;
      String normalized = selection.toLowerCase(Locale.ROOT);
      if (!mayReferenceMappedMediaPath(normalized)) return selectionArgs;
      ArrayList<String> columns = placeholderColumns(normalized, selectionArgs.length);
      String[] patched = null;
      patched = patchDirectorySelectionArgs(selectionArgs, columns, callerUid, patched);
      for (int i = 0; i < selectionArgs.length; i++) {
        String value = selectionArgs[i];
        String column = i < columns.size() ? columns.get(i) : null;
        String replacement = patchSelectionArg(normalized, column, value, callerUid);
        if (replacement == null || replacement.equals(value)) continue;
        if (patched == null) patched = selectionArgs.clone();
        patched[i] = replacement;
      }
      return patched == null ? selectionArgs : patched;
    }

    private static boolean mayReferenceMappedMediaPath(String selection) {
    ...
    private static String patchSelectionArg(
        String selection, String column, String value, int callerUid) {
      if (value == null || value.length() == 0) return null;
      if (("bucket_id".equals(column) || (column == null && selection.indexOf("bucket_id") >= 0))
          && isInteger(value)) {
        String mappedBucket = rewriteBucketIdValue(value, callerUid);
        if (mappedBucket != null) return mappedBucket;
      }
      if ("relative_path".equals(column) || (column == null && looksLikeRelativePath(value))) {
        String mappedRelative = rewriteRelativePathValue(value, callerUid);
        if (mappedRelative != null) return mappedRelative;
      }
      if (column != null && !"data".equals(column) && !"_data".equals(column)) return null;
    ...
    private static ArrayList<String> placeholderColumns(String selection, int argCount) {
    ...
    private static String nearestSelectionColumn(String selection, int questionIndex) {
      String[] candidates = {
    ...
    private static int selectionClauseStart(String selection, int questionIndex) {
      int start =
          Math.max(
              Math.max(
                  selection.lastIndexOf(" and ", questionIndex),
    ...
    private static int lastColumnIndexBefore(String selection, String column, int beforeIndex) {
      ...
      while (searchFrom >= 0) {
        ...
        if (isColumnBoundary(selection, idx - 1) && isColumnBoundary(selection, end)) return idx;
        searchFrom = idx - 1;
      }
      return -1;
    }
    ...
    private static String[] patchDirectorySelectionArgs(
        String[] selectionArgs, ArrayList<String> columns, int callerUid, String[] patched) {
      int primaryIndex = -1;
      int secondaryIndex = -1;
      for (int i = 0; i < columns.size() && i < selectionArgs.length; i++) {
        String column = columns.get(i);
        if ("primary_directory".equals(column)) primaryIndex = i;
        else if ("secondary_directory".equals(column)) secondaryIndex = i;
      }
      if (primaryIndex < 0 || secondaryIndex < 0) return patched;
      ...
      if (primary == null || primary.length() == 0 || secondary == null || secondary.length() == 0)
        return patched;
      String mappedRelative = rewriteRelativePathValue(primary + "/" + secondary + "/", callerUid);
      if (mappedRelative == null) return patched;
      String normalized = normalizeRelativePathValue(mappedRelative);
      int slash = normalized.indexOf('/');
      if (slash < 0) return patched;
      String mappedPrimary = normalized.substring(0, slash);
      String mappedSecondary = normalized.substring(slash + 1);
      if (mappedPrimary.equals(primary) && mappedSecondary.equals(secondary)) return patched;
      if (patched == null) patched = selectionArgs.clone();
    ...
    private static boolean looksLikeRelativePath(String value) {
      ...
      if (value.startsWith("/storage/")
          ...
          || value.startsWith("file://")) return false;
      return true;
    }
    ...
    private static String rewriteRelativePathValue(String value, int callerUid) {
      ...
      String probePath = buildMediaStoreProbePath(normalized, ".srx_probe", callerUid);
      String mappedPath = rewriteStoragePathForValues(probePath, callerUid);
      String mappedRelative = relativePathFromStoragePath(mappedPath, callerUid);
      if (mappedRelative == null) return null;
      if (normalizeRelativePathValue(mappedRelative).equals(normalized)) return null;
    ...
    private static void logSelectionPatch(String kind, int callerUid, String selection) {
      if (!shouldLog()) return;
      try {
        android.util.Log.i(
            "SRX",
            "java query selection patch kind="
                + kind
                + " caller_uid="
                + callerUid
                + " selection="
  ...
  private static void rememberBucketIdRewrite(
      int callerUid, String displayBucketId, String realBucketId) {
    if (callerUid < 0 || displayBucketId == null || realBucketId == null) return;
    displayBucketId = displayBucketId.trim();
    realBucketId = realBucketId.trim();
    if (displayBucketId.length() == 0
        || realBucketId.length() == 0
  ...
  private static String lookupRecentBucketIdRewrite(int callerUid, String displayBucketId) {
    if (callerUid < 0 || displayBucketId == null) return null;
    displayBucketId = displayBucketId.trim();
    if (displayBucketId.length() == 0) return null;
    String realBucketId;
    synchronized (RECENT_BUCKET_ID_REWRITES) {
      realBucketId = RECENT_BUCKET_ID_REWRITES.get(bucketIdRewriteKey(callerUid, displayBucketId));
    }
    if (realBucketId != null) logBucketIdRewrite("hit", callerUid, displayBucketId, realBucketId);
    return realBucketId;
  }

  private static String bucketIdRewriteKey(int callerUid, String displayBucketId) {
    return callerUid + ":" + displayBucketId;
  }

  private static void logBucketIdRewrite(
      String stage, int callerUid, String displayBucketId, String realBucketId) {
    if (!shouldLog()) return;
    if (callerUid == android.os.Process.myUid()) return;
    if (BUCKET_ID_REWRITE_LOG_COUNT >= 96) return;
    BUCKET_ID_REWRITE_LOG_COUNT++;
    try {
      android.util.Log.i(
          "SRX",
          "java bucket_id_rewrite "
              + stage
              + " caller_uid="
              + callerUid
              + " display="
  ...
  private static boolean isSingleItemQuery(Object[] args) {
    try {
      if (args == null || args.length == 0 || !(args[0] instanceof android.net.Uri)) return false;
      android.net.Uri uri = (android.net.Uri) args[0];
      String last = uri.getLastPathSegment();
      if (last != null && last.length() > 0 && isAllDigits(last)) return true;
  ...
  private static boolean hasSingleIdSelection(Object[] args) {
    for (int i = 0; i < args.length; i++) {
      ...
      if (arg instanceof String && isIdSelection((String) arg)) {
        if (i + 1 >= args.length || args[i + 1] == null) return true;
        if (args[i + 1] instanceof String[]) return isSingleIdSelectionArgs((String[]) args[i + 1]);
  ...
  private static boolean isSingleIdOperand(String value) {
    ...
    if (isIntegerText(value)) return true;
  ...
  private static boolean isSingleIdSelectionBundle(android.os.Bundle bundle) {
    ...
    String selection = bundle.getString("android:query-arg-sql-selection");
    if (!isIdSelection(selection)) return false;
  ...
  private static String describeQueryArgs(Object[] args) {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (arg instanceof android.net.Uri) {
        ...
        sb.append("ContentValues").append(describeContentValues((ContentValues) arg));
      } else if (arg instanceof ContentValues[]) {
        ContentValues[] values = (ContentValues[]) arg;
        sb.append("ContentValues[](").append(values.length).append(')');
        if (values.length > 0) sb.append(describeContentValues(values[0]));
      } else if (arg == null) {
        sb.append("null");
      } else {
  ...
  private static void logCursor(
      String stage, Cursor cursor, int pathColumn, int before, int after, int callerUid) {
    if (!shouldLog()) return;
    if (callerUid == android.os.Process.myUid()) return;
    if (CURSOR_LOG_COUNT >= 64) return;
    CURSOR_LOG_COUNT++;
  ...
  private static void logFilter(String reason, long id, int callerUid) {
    if (!shouldLog()) return;
    if (callerUid == android.os.Process.myUid()) return;
    if (FILTER_LOG_COUNT >= 64) return;
    FILTER_LOG_COUNT++;
    try {
      android.util.Log.i("SRX", "java cursor " + reason + " caller_uid=" + callerUid + " id=" + id);
    } catch (Throwable ignored) {
    }
  }
  ...
  private static final class FilteringCursor extends AbstractCursor {
    ...
    private FilteringCursor(
        Cursor base,
        int[] rows,
        String[] rewrites,
        int pathColumn,
        String[] relativeRewrites,
        int relativePathColumn,
        String[] bucketIdRewrites,
        int bucketIdColumn,
    ...
    static Cursor wrap(
        ...
        Object[] queryArgs) {
      if (base == null || base.isClosed()) return base;
      ...
      if (pathColumn < 0 && relativePathColumn < 0 && idColumn < 0 && bucketIdColumn < 0) {
        return wrapOpaqueAggregateIfHidden(base, callerUid, count, visibleColumns);
      }
      ...
      try {
        ...
        while (base.moveToNext()) {
          ...
          if (pathColumn >= 0) {
            ...
            String rewritten = path == null ? null : filterPath(path, callerUid);
            ...
            if (rewritten != null && !rewritten.equals(path)) {
              if (!allowMissingMappedTarget) {
                boolean rewrittenExists = storagePathExistsForCaller(rewritten, callerUid);
                boolean mappingViewRewrite = isMappingViewRewrite(path, rewritten, callerUid);
                if (!rewrittenExists && (!mappingViewRewrite || !isMediaStorePendingPath(path))) {
                  logFilter("filter_missing_rewrite", id, callerUid);
                  continue;
                }
                if (!rewrittenExists) logFilter("keep_missing_mapping_view", id, callerUid);
              }
              logFilter("rewrite", id, callerUid);
              logKeptRow("rewrite", id, callerUid, path, rewritten, base, bucketIdColumn);
              rewrites[row] = rewritten;
              if (relativePathColumn >= 0) {
                String relative = relativePathFromStoragePath(rewritten, callerUid);
                if (relative != null) relativeRewrites[row] = relative;
              }
              if (bucketIdColumn >= 0) {
                String bucketId = bucketIdFromStoragePath(rewritten, callerUid);
                if (bucketId != null) {
                  bucketIdRewrites[row] = bucketId;
                  rememberBucketIdRewrite(callerUid, bucketId, readString(base, bucketIdColumn));
                }
              }
            }
            logKeptRow("plain", id, callerUid, path, rewritten, base, bucketIdColumn);
            recordedQueryPaths =
                recordQueryAccessPaths(path, rewritten, callerUid, recordedQueryPaths);
            visibleRows.add(row);
            continue;
          }
          if (relativePathColumn >= 0) {
            String probePath = relativePathProbePath(base, relativePathColumn, callerUid);
            if (probePath != null) {
              ...
              if (rewrittenProbe != null && !rewrittenProbe.equals(probePath)) {
                String relative = relativePathFromStoragePath(rewrittenProbe, callerUid);
                if (relative != null) {
                  ...
                  if (bucketIdColumn >= 0) {
                    ...
                    if (bucketId != null) {
                      bucketIdRewrites[row] = bucketId;
                      rememberBucketIdRewrite(
                          callerUid, bucketId, readString(base, bucketIdColumn));
                    }
                  }
                }
              }
          ...
          if ((aggregateRootPath != null && isHiddenPath(aggregateRootPath, callerUid))
          ...
          if (bucketIdColumn >= 0 && aggregateRootPath != null) {
            ...
            if (rewrittenProbe != null && !rewrittenProbe.equals(aggregateRootPath)) {
              String bucketId = bucketIdFromStoragePath(rewrittenProbe, callerUid);
              if (bucketId != null) {
                bucketIdRewrites[row] = bucketId;
                rememberBucketIdRewrite(callerUid, bucketId, readString(base, bucketIdColumn));
              }
      ...
      logQueryKeep(
          ...
          idColumn,
          allowMissingMappedTarget);
      if (visibleRows.size() == count
          && !hasAnyRewrite(rewrites)
          && !hasAnyRewrite(relativeRewrites)
          && !hasAnyRewrite(bucketIdRewrites)
          && visibleColumns == null) {
        return base;
      }
      int[] rows = new int[visibleRows.size()];
      for (int i = 0; i < rows.length; i++) rows[i] = visibleRows.get(i);
      return new FilteringCursor(
          base,
          rows,
          rewrites,
          pathColumn,
    ...
    private static int recordQueryAccessPaths(
        String path, String rewritten, int callerUid, int recorded) {
      if (callerUid == android.os.Process.myUid()) return recorded;
    ...
    private static int recordQueryAccessPathIfNeeded(String path, int callerUid, int recorded) {
      if (recorded >= MAX_QUERY_PATH_RECORDS || path == null || path.length() == 0) {
        return recorded;
      }
      try {
    ...
    private static void logQueryKeep(
        Object[] queryArgs,
        Cursor cursor,
        int callerUid,
        int before,
      ...
      if (callerUid == android.os.Process.myUid()) return;
      if (after <= 0) return;
      if (QUERY_KEEP_LOG_COUNT >= 48) return;
      QUERY_KEEP_LOG_COUNT++;
      try {
        android.util.Log.i(
            "SRX",
            "java query_keep caller_uid="
                + callerUid
                + " before="
                + before
                + " after="
                ...
                + Arrays.toString(cursor.getColumnNames())
                + " args="
                + describeQueryArgs(queryArgs));
      } catch (Throwable ignored) {
      }
    }

    private static void logKeptRow(
        String reason,
        long id,
        int callerUid,
        String path,
      ...
      if (!shouldLog()) return;
      if (callerUid == android.os.Process.myUid()) return;
      if (KEPT_ROW_LOG_COUNT >= 96) return;
      KEPT_ROW_LOG_COUNT++;
      try {
        android.util.Log.i(
            "SRX",
    ...
    private static int findPathColumn(Cursor cursor) {
      ...
      for (int i = 0; i < names.length; i++) {
        String name = names[i];
        if ("_data".equals(name)) return i;
        if ("data".equalsIgnoreCase(name)) fallback = i;
      }
      return fallback;
    }
    ...
    private static int findColumnIgnoreCase(Cursor cursor, String target) {
      ...
      for (int i = 0; i < names.length; i++) {
        if (target.equalsIgnoreCase(names[i])) return i;
      }
      return -1;
    }

    private static Cursor wrapOpaqueAggregateIfHidden(
        Cursor base, int callerUid, int count, String[] visibleColumns) {
      if (!isAnyPublicMediaRootHidden(callerUid)) return base;
    ...
    private static String aggregateProbePath(Cursor cursor, int callerUid) {
      ...
      if (bucketIdColumn < 0) return null;
      long bucketId = readLong(cursor, bucketIdColumn, Long.MIN_VALUE);
      String bucketName = readString(cursor, bucketNameColumn);
      if (bucketId == Long.MIN_VALUE) return null;
      String path = bucketProbePath(bucketId, bucketName, callerUid);
      logBucketProbe(bucketId, bucketName, callerUid, path);
      return path;
    }

    private static String bucketProbePath(long bucketId, String bucketName, int callerUid) {
      int userId = userIdFromUid(callerUid);
      if (userId < 0) return null;
      String storageRoot = "/storage/emulated/" + userId;
      ArrayList<String> candidates = bucketProbeCandidates(bucketName, callerUid);
      for (String relative : candidates) {
        String path = storageRoot + "/" + normalizeRelativePath(relative);
        if (path.toLowerCase(Locale.ROOT).hashCode() == (int) bucketId) return path + "/.srx_probe";
      }
      return null;
    }

    private static ArrayList<String> bucketProbeCandidates(String bucketName, int callerUid) {
      ArrayList<String> candidates = new ArrayList<>();
      String normalizedName = normalizeRelativePath(bucketName);
      for (String root : PUBLIC_MEDIA_ROOTS) {
        candidates.add(root);
        if (normalizedName.length() > 0) candidates.add(root + "/" + normalizedName);
      }
      return candidates;
    }

    private static void logBucketProbe(
        long bucketId, String bucketName, int callerUid, String path) {
      if (!shouldLog()) return;
      if (callerUid == android.os.Process.myUid()) return;
      if (BUCKET_PROBE_LOG_COUNT >= 96) return;
      String name = bucketName == null ? "" : bucketName;
      BUCKET_PROBE_LOG_COUNT++;
      try {
        android.util.Log.i(
            "SRX",
            "java bucket probe caller_uid="
                + callerUid
                + " bucket_id="
                + bucketId
                + " bucket_name="
    ...
    private static String relativePathProbePath(
        Cursor cursor, int relativePathColumn, int callerUid) {
    ...
    private static String bucketIdFromRelativePath(String relativePath, int callerUid) {
      if (relativePath == null || relativePath.length() == 0) return null;
      int userId = userIdFromUid(callerUid);
      if (userId < 0) return null;
      String path = "/storage/emulated/" + userId + "/" + normalizeRelativePath(relativePath);
    ...
    private static boolean isAnyPublicMediaRootHidden(int callerUid) {
      ...
      if (userId < 0) return false;
      for (String root : PUBLIC_MEDIA_ROOTS) {
        String probePath = "/storage/emulated/" + userId + "/" + root + "/.srx_probe";
        if (isHiddenPath(probePath, callerUid)) return true;
      }
      return false;
    }
    ...
    private static boolean isHiddenByCallerVisibility(String path, int callerUid) {
      if (path == null || path.length() == 0) return false;
      try {
    ...
    private static boolean storagePathExistsForCaller(String path, int callerUid) {
      if (storagePathExists(path)) return true;
      try {
        String mappedPath = resolveOpenPath(path, callerUid);
        return mappedPath != null && mappedPath.length() > 0 && storagePathExists(mappedPath);
      } catch (Throwable ignored) {
        return false;
      }
    }

    private static boolean isMappingViewRewrite(
        String originalPath, String rewrittenPath, int callerUid) {
      try {
    ...
    private static boolean storagePathsEquivalent(String left, String right, int callerUid) {
    ...
    private static String normalizeComparableStoragePath(String path, int callerUid) {
      if (path == null || path.length() == 0) return null;
      String value = path.trim();
      if (value.startsWith("file://")) value = value.substring("file://".length());
      int userId = userIdFromUid(callerUid);
      ...
      if (value.startsWith(fuseRoot)) {
        value = "/storage/emulated/" + userId + "/" + value.substring(fuseRoot.length());
      }
      while (value.endsWith("/") && value.length() > 1)
        value = value.substring(0, value.length() - 1);
      return value;
    ...
    private static int findIdColumn(Cursor cursor) {
      ...
      if (names == null) return -1;
      for (int i = 0; i < names.length; i++) {
        if ("_id".equals(names[i])) return i;
      }
      return -1;
    }
    ...
    private int currentBaseRow() {
      if (mPos < 0 || mPos >= rows.length) return -1;
      return rows[mPos];
    }
  }
}
