...
#[cfg(test)]
pub(crate) use source_hint::clear_recent_private_owner_hint_for_tests;
#[cfg(test)]
pub(crate) use source_hint::infer_recent_private_owner_identity;
...
mod tests {
    ...
    #[test]
    fn normalize_storage_path_accepts_all_storage_users() {
        assert_eq!(
            normalize_storage_path_locked("/storage/emulated/0/DCIM/Camera/a.jpg"),
            "/storage/emulated/0/DCIM/Camera/a.jpg"
        );
        assert_eq!(
            normalize_storage_path_locked("/storage/emulated/10/Pictures/demo.png"),
            "/storage/emulated/10/Pictures/demo.png"
        );
    }

    #[test]
    fn normalize_storage_path_resolves_storage_aliases() {
        assert_eq!(
            normalize_storage_path_locked("/mnt/runtime/default/emulated/10/DCIM/a.jpg"),
            "/storage/emulated/10/DCIM/a.jpg"
        );
        assert_eq!(
            normalize_storage_path_locked("/storage/self/primary/Movies/demo.mp4"),
            "/storage/emulated/0/Movies/demo.mp4"
        );
        assert_eq!(
            normalize_storage_path_locked("/data/media/0/Download/demo.bin"),
            "/storage/emulated/0/Download/demo.bin"
        );
    }

    #[test]
    fn normalize_storage_path_rejects_non_storage_paths() {
        assert!(normalize_storage_path_locked("/data/user/0/com.demo/files/a.txt").is_empty());
        assert!(normalize_storage_path_locked("/system/bin/sh").is_empty());
    }

    #[test]
    fn filters_android_data_and_media_provider_internal_paths() {
        assert!(paths::is_android_data_path(
            "/storage/emulated/0/Android/data/com.demo/file.txt"
        ));
        assert!(paths::is_android_data_path(
            "/storage/emulated/10/Android/data"
        ));
        assert!(!paths::is_android_data_path(
            "/storage/emulated/0/Android/media/com.demo/file.txt"
        ));

        assert!(is_filtered_media_provider_path(
            "/storage/emulated/0/DCIM/.transforms/cache"
        ));
        assert!(is_filtered_media_provider_path(
            "/storage/emulated/10/Movies/.picker_transcoded/video"
        ));
        assert!(!is_filtered_media_provider_path(
    ...
    #[test]
    fn private_media_path_owner_takes_precedence_over_recent_caller() {
        let state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 1000,
            shared_uid_packages: String::new(),
            caller_package: "com.aliyun.tongyi".to_string(),
            caller_package_updated_ms: monotonic_ms(),
            recent_event_ms: HashMap::new(),
            recent_event_order: VecDeque::new(),
            query_access_paths: HashMap::new(),
            query_access_order: VecDeque::new(),
        ...
        let identity = resolve_caller_info_for_path_locked(
            &state,
            "",
            "/storage/emulated/0/Android/media/com.eg.android.AlipayGphone/XRadiant/log/error.log",
            false,
            false,
        );

        assert_eq!(identity.package_name, "com.eg.android.AlipayGphone");
        assert_eq!(identity.source, "path_owner");
        assert_eq!(identity.confidence, "high");
    }

    #[test]
    fn public_media_provider_path_does_not_use_recent_caller() {
        let state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 1000,
            shared_uid_packages: String::new(),
            caller_package: "com.android.vending".to_string(),
            caller_package_updated_ms: monotonic_ms(),
            recent_event_ms: HashMap::new(),
            recent_event_order: VecDeque::new(),
            query_access_paths: HashMap::new(),
        ...
        let identity = resolve_caller_info_for_path_locked(
            &state,
            "",
            "/storage/emulated/0/MIUI/Mirror/MainCache",
            true,
            false,
        );

        assert!(identity.package_name.is_empty());
        assert_eq!(identity.source, "unknown");
    }

    #[test]
    fn explicit_saf_provider_package_bypasses_missing_uid_cache() {
        source_hint::clear_recent_private_owner_hint_for_tests();
        let previous_uid = policy::replace_test_uid_cache(HashMap::new());

        let package = resolve_recent_path_caller_package(10357, "org.srx.manager");
        let intermediate =
            resolve_recent_path_caller_package(10085, "com.google.android.documentsui");
        ...
        assert_eq!(package.as_deref(), Some("org.srx.manager"));
        assert!(intermediate.is_none());
    }

    #[test]
    fn saf_provider_path_hint_uses_explicit_package() {
        source_hint::clear_recent_private_owner_hint_for_tests();
        let previous_uid = policy::replace_test_uid_cache(HashMap::new());
        ...
        let identity = source_hint::infer_recent_path_caller_identity(
            "/storage/emulated/0/Download/备份/storage-redirect-x-backup-20260624-120650.srxbak.zip",
            0,
        )
        .expect("path hint");
    ...
    #[test]
    fn query_access_identity_is_used_before_path_config() {
        ...
        remember_query_access_locked(
            &mut state,
            "/storage/emulated/0/Download/danmu-api-1.0.5.37.apk".to_string(),
            "com.example.danmuapiapp".to_string(),
            10281,
            "query_access",
            "medium",
        );
        ...
        let identity = resolve_caller_info_for_path_locked(
            &state,
            "",
            "/storage/emulated/0/Download/danmu-api-1.0.5.37.apk",
            false,
            true,
        );

        assert_eq!(identity.package_name, "com.example.danmuapiapp");
        assert_eq!(identity.source, "query_access");
        assert_eq!(identity.confidence, "medium");
    }

    #[test]
    fn duplicate_event_key_keeps_open_operation_filter() {
        let state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 1000,
            shared_uid_packages: String::new(),
        ...
        let identity = SourceIdentity::new("org.srx.testapp".to_string(), "caller", "high");
        let read_key = build_event_key_locked(
            &state,
            &identity,
            OpKind::Open,
            "open:read",
            "/storage/emulated/0/Download/demo.txt",
            3,
            0,
        );
        let write_key = build_event_key_locked(
            &state,
            &identity,
            OpKind::Open,
            "open:write",
        ...
        let repeated_read_key = build_event_key_locked(
            &state,
            &identity,
            OpKind::Open,
            "open:read",
        ...
        assert_eq!(read_key, repeated_read_key);
    }

    #[test]
    fn duplicate_event_key_coalesces_open_create_and_write_for_same_fd() {
        let state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 1000,
            shared_uid_packages: String::new(),
        ...
        let create_key = build_event_key_locked(
            &state,
            &identity,
            OpKind::Open,
            "open:create",
        ...
        assert_eq!(create_key, write_key);
    }

    #[test]
    fn monitor_extra_infers_backend_for_mapping_mode_sandbox_request() {
        let hub = SettingsHub::instance();
        let previous_apps = hub.replace_test_apps(HashMap::from([(
            "com.android.providers.downloads".to_string(),
            AppProfile {
        ...
        let state = MonitorStateSnapshot {
        ...
        let extra = build_monitor_extra_with_backend(
            &state,
            &identity,
            OpKind::Open,
            "open:create",
            ...
            179,
            "op=open|op_filter=open:create|flags=0xc2",
        );
        let text = extra.as_ref();
        ...
        hub.restore_test_apps(previous_apps.0, previous_apps.1);

        assert!(text.contains(
            "backend=/data/media/0/Android/data/com.android.providers.downloads/sdcard/.xlDownload/dp_so.log"
        ));
        assert!(text.contains("from=/storage/emulated/0/.xlDownload/dp_so.log"));
        assert!(text.contains("source=sandbox_path"));
    }

    #[test]
    fn recent_private_owner_hint_can_follow_open_write_export() {
        assert!(should_use_recent_private_owner_hint(
            OpKind::Open,
            "/storage/emulated/0/Download/XRadiant_backup_20260623_170751.json",
            "open:write"
        ));
    ...
    #[test]
    fn query_access_identity_matches_media_store_pending_path() {
        let mut state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 1000,
            shared_uid_packages: String::new(),
        ...
        remember_query_access_locked(
            &mut state,
            "/storage/emulated/0/Download/srt_qmark_a.txt".to_string(),
        ...
        assert_eq!(identity.package_name, "org.srx.testapp");
        assert_eq!(identity.source, "query_access");
        assert_eq!(identity.confidence, "medium");
    }

    #[test]
    fn query_access_identity_overrides_recent_caller() {
        let mut state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 1000,
            shared_uid_packages: String::new(),
            caller_package: "com.previous.app".to_string(),
            caller_package_updated_ms: monotonic_ms(),
            recent_event_ms: HashMap::new(),
            recent_event_order: VecDeque::new(),
            query_access_paths: HashMap::new(),
        ...
        remember_query_access_locked(
            &mut state,
            "/storage/emulated/0/Download/current-export.zip".to_string(),
            "com.current.app".to_string(),
            10281,
            "query_access",
            "medium",
        );
        ...
        let identity = resolve_caller_info_for_path_locked(
            &state,
            "",
            "/storage/emulated/0/Download/current-export.zip",
            false,
            true,
        );

        assert_eq!(identity.package_name, "com.current.app");
        assert_eq!(identity.source, "query_access");
        assert_eq!(identity.confidence, "medium");
    }

    #[test]
    fn recent_caller_remains_fallback_without_path_evidence() {
        let state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 1000,
            shared_uid_packages: String::new(),
        ...
        let identity = resolve_caller_info_for_path_locked(
            &state,
            "",
            "/storage/emulated/0/Download/unknown-export.zip",
            false,
            true,
        );

        assert_eq!(identity.package_name, "com.previous.app");
        assert_eq!(identity.source, "recent_caller");
        assert_eq!(identity.confidence, "medium");
    }

    #[test]
    fn recent_private_owner_hint_attributes_matching_download_export() {
        source_hint::clear_recent_private_owner_hint_for_tests();
        let state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 10226,
        ...
        let private_identity = resolve_caller_info_for_path_locked(
            &state,
            "",
        ...
        assert_eq!(private_identity.package_name, "com.eg.android.AlipayGphone");

        let public_identity = resolve_caller_info_for_path_locked(
            &state,
            "",
        ...
        assert_eq!(public_identity.package_name, "com.eg.android.AlipayGphone");
        assert_eq!(public_identity.source, "recent_private_owner");
        assert_eq!(public_identity.confidence, "medium");
    ...
    #[test]
    fn provider_open_identity_overrides_recent_private_owner_hint() {
        source_hint::clear_recent_private_owner_hint_for_tests();
        let mut state = AuditState {
            package_name: "com.android.providers.media.module".to_string(),
            uid: 10226,
        ...
        remember_query_access_locked(
            &mut state,
            "/storage/emulated/0/Download/XRadiant_backup_20260623_170751.json".to_string(),
        ...
        assert_eq!(public_identity.package_name, "com.leo.xposed.xradiant");
        assert_eq!(public_identity.source, "provider_open");
        assert_eq!(public_identity.confidence, "high");
    }

    #[test]
    fn recent_private_caller_hint_attributes_matching_download_export() {
        source_hint::clear_recent_private_owner_hint_for_tests();
        source_hint::remember_private_path_caller_hint(
            "/storage/emulated/0/Android/media/com.eg.android.AlipayGphone/XRadiant/XRadiant.db",
        ...
        assert_eq!(public_identity.package_name, "com.leo.xposed.xradiant");
        assert_eq!(public_identity.source, "recent_private_caller");
        assert_eq!(public_identity.confidence, "medium");
    }

    #[test]
    fn public_path_token_identity_fills_missing_saf_export_caller() {
        source_hint::clear_recent_private_owner_hint_for_tests();
        let previous_uid = policy::replace_test_uid_cache(HashMap::from([
