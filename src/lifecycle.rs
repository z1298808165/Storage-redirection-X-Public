...
#[cfg(test)]
mod tests {
    use super::{parse_auto_enable_redirect_for_new_apps, should_install_package_event_receiver};
    use std::fs;
    use std::time::{SystemTime, UNIX_EPOCH};

    #[test]
    fn package_event_receiver_requires_auto_new_apps_enabled() {
        assert!(parse_auto_enable_redirect_for_new_apps(
            r#"{"auto_enable_redirect_for_new_apps": true}"#
        ));
        assert!(!parse_auto_enable_redirect_for_new_apps(
            r#"{"auto_enable_redirect_for_new_apps": false}"#
        ));
        assert!(!parse_auto_enable_redirect_for_new_apps("{}"));
        assert!(!parse_auto_enable_redirect_for_new_apps("not json"));
    }

    #[test]
    fn package_event_receiver_reads_config_from_module_dir() {
        let dir = temp_test_dir("package_receiver_module_dir");
        ...
        assert!(should_install_package_event_receiver(
    ...
    #[test]
    fn package_event_receiver_rejects_missing_module_config() {
        let dir = temp_test_dir("package_receiver_missing_config");
