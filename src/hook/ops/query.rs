...
mod tests {
    ...
    fn seed_readlink_buffer(buf: &mut [u8], path: &str) -> isize {
    ...
    #[test]
    fn reverse_readlink_result_rewrites_sandbox_path() {
        let mut buf = [0u8; 256];
        let result = seed_readlink_buffer(
            &mut buf,
            "/data/media/0/Android/data/xyz.nextalone.nnngram/sdcard/Download/a.apk",
        );

        let len = unsafe {
            reverse_readlink_result_if_visible(
                result,
        ...
        assert_eq!(text, "/storage/emulated/0/Download/a.apk");
    }

    #[test]
    fn provider_passthrough_keeps_readlink_result_unmodified() {
        let _passthrough = PassthroughGuard;
        crate::hook::enter_provider_passthrough();
        let original = "/data/media/0/Android/data/xyz.nextalone.nnngram/sdcard/Download/a.apk";
        let mut buf = [0u8; 256];
        let result = seed_readlink_buffer(&mut buf, original);
    ...
    #[test]
    fn readlink_sandbox_reverse_candidate_detects_possible_rewrites() {
        assert!(!readlink_sandbox_reverse_may_change("/proc/880/maps"));
        assert!(!readlink_sandbox_reverse_may_change(
            "/storage/emulated/0/Download/a.apk"
        ));
        assert!(readlink_sandbox_reverse_may_change(
            "/data/media/0/Android/data/com.example/sdcard/Download/a.apk"
        ));
        assert!(readlink_sandbox_reverse_may_change(
            "/storage/emulated/0/Android/data/com.example/sdcard/Download/a.apk"
        ));
    }

    #[test]
    fn system_writer_self_readlink_preserves_sandbox_fd_path() {
        assert!(
            should_preserve_readlink_result_for_system_writer_self_context(
                "com.android.providers.media.module",
                "com.android.providers.media.module",
                writer::ANDROID_APP_UID_START + 217,
                false,
        ...
        assert!(
            should_preserve_readlink_result_for_system_writer_self_context(
                ...
                -1,
                false,
                "/data/media/0/Android/data/com.android.providers.media.module/sdcard/.xlDownload/dp_so.log",
            )
        );
        assert!(
            !should_preserve_readlink_result_for_system_writer_self_context(
    ...
    #[test]
    fn system_writer_app_caller_readlink_can_reverse_sandbox_fd_path() {
        assert!(
            !should_preserve_readlink_result_for_system_writer_self_context(
                "com.android.providers.media.module",
                "com.example.viewer",
                writer::ANDROID_APP_UID_START + 123,
                true,
                "/data/media/0/Android/data/com.android.providers.media.module/sdcard/DCIM/a.jpg",
            )
        );
        assert!(
            should_preserve_readlink_result_for_system_writer_self_context(
    ...
    #[test]
    fn system_writer_query_fallback_retries_missing_and_denied() {
        assert!(is_retryable_system_writer_query_error(libc::ENOENT));
        assert!(is_retryable_system_writer_query_error(libc::EACCES));
        assert!(is_retryable_system_writer_query_error(libc::EPERM));
    }

    #[test]
    fn system_writer_query_private_owner_fix_only_on_permission_errors() {
        assert!(!should_fix_system_writer_private_owner_for_query_error(
            libc::ENOENT
        ));
        assert!(should_fix_system_writer_private_owner_for_query_error(
            libc::EACCES
    ...
    #[test]
    fn system_writer_query_fallback_requires_recent_external_caller() {
        assert!(has_recent_external_caller_signal_for_query_fallback(
            "com.example.newapp",
            writer::ANDROID_APP_UID_START,
            10,
            true
        ));
        assert!(has_recent_external_caller_signal_for_query_fallback(
        ...
        assert!(!has_recent_external_caller_signal_for_query_fallback(
            "com.example.newapp",
            writer::ANDROID_APP_UID_START,
            QUERY_FALLBACK_CALLER_MAX_AGE_MS + 1,
            true
        ));
        assert!(!has_recent_external_caller_signal_for_query_fallback(
            "com.example.newapp",
