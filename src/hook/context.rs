...
mod tests {
    ...
    fn reset_thread_context() {
        clear_current_caller();
        clear_binder_saved_caller_uid();
        set_binder_identity_cleared(false);
        clear_fuse_caller_uid();
        while is_current_caller_scope_active() {
            pop_current_caller_scope();
        }
        while is_provider_passthrough_active() {
            exit_provider_passthrough();
        }
    }

    #[test]
    fn provider_passthrough_exit_clears_stale_caller_without_scope() {
        reset_thread_context();
        set_current_caller_package("org.srx.test");
        set_current_caller_uid(10123);
        set_binder_saved_caller_uid(10123);
        enter_provider_passthrough();

        exit_provider_passthrough();

        assert_eq!(get_current_caller_package(), "");
        assert_eq!(get_current_caller_uid(), -1);
        assert_eq!(get_binder_saved_caller_uid(), -1);
        reset_thread_context();
    }

    #[test]
    fn provider_passthrough_exit_preserves_active_caller_scope() {
        reset_thread_context();
        push_current_caller_scope("org.srx.caller", 10123);
        set_binder_saved_caller_uid(10123);
        enter_provider_passthrough();

        exit_provider_passthrough();

        assert_eq!(get_current_caller_package(), "org.srx.caller");
        assert_eq!(get_current_caller_uid(), 10123);
        assert_eq!(get_binder_saved_caller_uid(), 10123);
        reset_thread_context();
    }
}
