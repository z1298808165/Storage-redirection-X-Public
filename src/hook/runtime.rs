...
fn chmod_private_node_if_needed(
    ...
    let required_mode = match file_type {
        mode if mode == libc::S_IFDIR as mode_t => PRIVATE_CHILD_DIR_REQUIRED_MODE,
...
mod tests {
    ...
    fn sample_owner() -> RedirectDirOwner {
        RedirectDirOwner {
            uid: 10312,
            user_id: 0,
    ...
    #[test]
    fn shared_media_owner_matches_private_app_tree() {
        let owner = sample_owner();

        assert!(should_apply_shared_media_owner(
            "/data/media/0/Android/data/xyz.nextalone.nnngram",
            &owner
        ));
        assert!(should_apply_shared_media_owner(
            "/data/media/0/Android/data/xyz.nextalone.nnngram/sdcard",
            &owner
        ));
        assert!(should_apply_shared_media_owner(
            "/data/media/0/Android/media/xyz.nextalone.nnngram/Download",
            &owner
        ));
        assert!(should_apply_shared_media_owner(
            "/data/media/0/Android/obb/xyz.nextalone.nnngram",
            &owner
        ));
    }

    #[test]
    fn shared_media_owner_rejects_unrelated_paths() {
        let owner = sample_owner();
        ...
        assert!(!should_apply_shared_media_owner(
            "/data/media/0/Android/data/other.app/sdcard",
            &owner
        ));
        assert!(!should_apply_shared_media_owner(
            "/storage/emulated/0/Android/data/xyz.nextalone.nnngram",
            &owner
        ));
        assert!(!should_apply_shared_media_owner(
            "/data/media/0/Android/data/xyz.nextalone.nnngram",
    ...
    #[test]
    fn private_owner_fix_root_stays_inside_package_tree() {
    ...
    #[test]
    fn private_owner_fix_includes_existing_sidecar_without_create_intent() {
        let temp_root = std::env::temp_dir().join(format!(
            "srx-private-owner-fix-{}-{}",
            std::process::id(),
            paths::monotonic_ms()
        ));
        let private_root = temp_root
            .join("Android")
            .join("media")
            .join("com.example.owner");
        let sidecar = private_root.join("Viewer").join("Cache.db-shm");
        fs::create_dir_all(sidecar.parent().unwrap()).expect("create test private dir");
        fs::write(&sidecar, b"").expect("create sidecar");

        let private_root = private_root.to_string_lossy().into_owned();
        let sidecar = sidecar.to_string_lossy().into_owned();
        let missing = join_test_path(&private_root, "Viewer/missing.db-shm");

        assert!(should_fix_private_path_node(&sidecar, &private_root, false));
        assert!(should_fix_private_path_node(&missing, &private_root, true));
        assert!(!should_fix_private_path_node(
            &missing,
            &private_root,
            false
        ));
    ...
    fn join_test_path(root: &str, child: &str) -> String {
        if root.ends_with('/') {
            format!("{root}{child}")
        } else {
            format!("{root}/{child}")
    ...
    #[test]
    fn private_owner_fix_skips_intermediate_owners() {
        assert!(
            android_private_owner_package(
                "/storage/emulated/0/Android/data/com.android.providers.media.module/sdcard/a"
            )
            .is_none()
        );
        assert_eq!(
            ...
            .as_deref(),
            Some("xyz.nextalone.nnngram")
        );
    }

    #[test]
    fn private_owner_fix_requires_enabled_owner_config() {
        with_test_app_config("com.xiaomi.market", 10155, false, || {
            assert_eq!(
                ...
                .as_deref(),
                Some("com.xiaomi.market")
            );
            assert!(!is_private_owner_fix_enabled("com.xiaomi.market", 10155));
        });

        with_test_app_config("com.example.enabled", 10321, true, || {
            assert!(is_private_owner_fix_enabled("com.example.enabled", 10321));
        });
    }

    #[test]
    fn private_owner_fix_allows_cross_caller_sqlite_even_when_owner_disabled() {
        let hub = InterceptHub::instance();
        hub.clear_current_caller();
        hub.set_current_caller_package("com.leo.xposed.xradiant");
        hub.set_current_caller_uid(10164);

        with_test_app_config("com.eg.android.AlipayGphone", 10274, false, || {
            assert!(!is_private_owner_fix_enabled(
                "com.eg.android.AlipayGphone",
                10274
            ));
            assert!(should_allow_sqlite_private_owner_fix_for_current_caller(
    ...
    #[test]
    fn private_owner_fix_rejects_different_external_caller() {
        let hub = InterceptHub::instance();
        hub.clear_current_caller();

        assert!(is_private_owner_fix_allowed_for_current_caller(
            "com.example.owner",
            10274
        ));

        hub.set_current_caller_uid(10274);
        assert!(is_private_owner_fix_allowed_for_current_caller(
            "com.example.owner",
            10274
        ));

        hub.set_current_caller_package("com.example.viewer");
        hub.set_current_caller_uid(10399);
        assert!(!is_private_owner_fix_allowed_for_current_caller(
            "com.example.owner",
            10274
        ));
    ...
    #[test]
    fn private_owner_fix_allows_cross_caller_sqlite_sidecar_only() {
        let hub = InterceptHub::instance();
        hub.clear_current_caller();
        hub.set_current_caller_package("com.example.viewer");
        hub.set_current_caller_uid(10164);
        ...
        assert!(should_allow_sqlite_private_owner_fix_for_current_caller(
            "/storage/emulated/0/Android/media/com.example.owner/Viewer/Cache.db-shm",
            "com.example.owner",
            10274,
            0,
        ));
        assert!(!should_allow_sqlite_private_owner_fix_for_current_caller(
            "/storage/emulated/0/Android/media/com.example.owner/Viewer/cache.bin",
            "com.example.owner",
            10274,
            0,
        ));
        assert!(!should_allow_sqlite_private_owner_fix_for_current_caller(
            "/storage/emulated/10/Android/media/com.example.owner/Viewer/Cache.db-shm",
            "com.example.owner",
            10274,
            10,
        ));
