...
mod tests {
    ...
    #[test]
    fn caller_mapping_matches_only_exact_or_child_path() {
        let mappings = vec![PathMapping::new(
            "/storage/emulated/0/Download".to_string(),
            "/storage/emulated/0/Documents/Mapped".to_string(),
        )];

        assert_eq!(
            map_path_by_caller_mappings("/storage/emulated/0/Download", &mappings),
            "/storage/emulated/0/Documents/Mapped"
        );
        assert_eq!(
            map_path_by_caller_mappings("/storage/emulated/0/Download/file.txt", &mappings),
            "/storage/emulated/0/Documents/Mapped/file.txt"
        );
        assert!(
            map_path_by_caller_mappings("/storage/emulated/0/DownloadExtra/file.txt", &mappings)
                .is_empty()
        );
        assert!(is_path_in_caller_mapping_request(
    ...
    #[test]
    fn caller_mapping_matches_request_path_case_insensitively() {
        let mappings = vec![PathMapping::new(
    ...
    #[test]
    fn reverse_caller_mapping_matches_only_exact_or_child_path() {
        let mappings = vec![PathMapping::new(
            "/storage/emulated/0/Download".to_string(),
            "/storage/emulated/0/Documents/Mapped".to_string(),
        )];
        ...
        assert_eq!(
            reverse_map_path_by_caller_mappings(
                "/storage/emulated/0/Documents/Mapped/file.txt",
                &mappings
            ),
            "/storage/emulated/0/Download/file.txt"
        ...
        assert!(
            reverse_map_path_by_caller_mappings(
                "/storage/emulated/0/Documents/MappedExtra/file.txt",
                &mappings
            )
            .is_empty()
        );
    ...
    #[test]
    fn reverse_caller_mapping_matches_final_path_case_insensitively() {
        let mappings = vec![PathMapping::new(
            "/storage/emulated/0/Download/AppBucket".to_string(),
            "/storage/emulated/0/Documents/Mapped".to_string(),
        )];
    ...
    #[test]
    fn reverse_readlink_sandbox_path_strips_only_sdcard_segment() {
        assert_eq!(
            reverse_readlink_sandbox_path(
                "/data/media/0/Android/data/com.example.app/sdcard/Download/file.txt"
            ),
            "/storage/emulated/0/Download/file.txt"
        );
        assert_eq!(
        ...
        assert_eq!(
            reverse_readlink_sandbox_path(
                "/storage/emulated/0/Android/data/com.example.app/files/file.txt"
            ),
            "/storage/emulated/0/Android/data/com.example.app/files/file.txt"
        );
        assert_eq!(
            reverse_readlink_sandbox_path("/data/media/0/Download/file.txt"),
            "/storage/emulated/0/Download/file.txt"
        );
    }

    #[test]
    fn fallback_maps_user_storage_without_cross_user_leakage() {
        let redirect_target = "/storage/emulated/0/Android/data/com.example.app/sdcard";
        assert_eq!(
            map_path_by_caller_fallback("/storage/emulated/0", redirect_target, 0),
            redirect_target
        );
        assert_eq!(
            map_path_by_caller_fallback(
                "/storage/emulated/0/Download/file.txt",
                redirect_target,
                0
            ),
            "/storage/emulated/0/Android/data/com.example.app/sdcard/Download/file.txt"
        );
        assert!(
            map_path_by_caller_fallback(
                "/storage/emulated/0/Android/data/com.example.app/sdcard/Download/file.txt",
        ...
        assert!(
            map_path_by_caller_fallback(
                "/storage/emulated/10/Download/file.txt",
                redirect_target,
                0
            )
            .is_empty()
        ...
        assert!(
            map_path_by_caller_fallback("/storage/emulated/0/DownloadExtra", redirect_target, 10)
                .is_empty()
        );
    }
    ...
    #[test]
    fn media_store_pending_path_matches_display_name_rules() {
        let pending_path = "/storage/emulated/0/Download/.pending-1781788689-srt_qmark_a.txt";
        assert_eq!(
            media_store_pending_display_path(pending_path).as_deref(),
            Some("/storage/emulated/0/Download/srt_qmark_a.txt")
        );

        assert!(caller_path_list_matches(
            &["/storage/emulated/0/Download/srt_qmark_?.txt".to_string()],
            pending_path,
            false,
        ));
        assert!(!caller_path_list_matches(
            &["/storage/emulated/0/Download/srt_qmark_ab.txt".to_string()],
            pending_path,
            false,
        ));

        let pending_part = "/storage/emulated/0/Download/.pending-77-srt_ci_probe.part";
        assert!(caller_path_list_matches(
    ...
    #[test]
    fn caller_allowed_paths_expand_wildcard_to_mount_fallback_parent() {
        ...
        assert!(is_path_allowed_by_caller_real_paths(
            "/storage/emulated/0/Download/SrtMountNsAllow/srt_ci_probe.txt",
            "org.srx.testapp",
            10123,
        ));
        assert!(!is_path_allowed_by_caller_real_paths(
    ...
    #[test]
    fn caller_allowed_file_name_wildcards_stay_precise() {
        let hub = SettingsHub::instance();
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([(
            "org.srx.testapp".to_string(),
            AppProfile {
        ...
        assert!(is_path_allowed_by_caller_real_paths(
            "/storage/emulated/0/Download/srt_qmark_a.txt",
            "org.srx.testapp",
            10123,
        ));
        assert!(!is_path_allowed_by_caller_real_paths(
            "/storage/emulated/0/Download/srt_qmark_ab.txt",
            "org.srx.testapp",
            10123,
        ));
    ...
    #[test]
    fn mapped_target_read_only_matches_request_path() {
        let hub = SettingsHub::instance();
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([(
            "org.srx.testapp".to_string(),
            AppProfile {
        ...
        let request_path = "/storage/emulated/0/DCIM/MyApp/a.jpg";
        assert!(!is_path_read_only_by_caller_paths(
            request_path,
            "org.srx.testapp",
            10123
        ));
        assert!(is_path_or_mapped_target_read_only_by_caller_paths(
            request_path,
            "org.srx.testapp",
            10123
        ));
        assert_eq!(
            read_only_check_path_by_caller_paths(request_path, "org.srx.testapp", 10123),
    ...
    #[test]
    fn caller_mappings_keep_android_media_target_and_skip_private_targets() {
        let hub = SettingsHub::instance();
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([(
            "org.srx.testapp".to_string(),
            AppProfile {
                    ...
                    UserProfile {
                        ...
                        path_mappings: vec![
                            PathMapping::new(
                                ...
                                "/storage/emulated/0/Android/media/org.srx.testapp/cache"
                                    .to_string(),
                            ),
                            PathMapping::new(
                                "/storage/emulated/0/Download/Data".to_string(),
                                "/storage/emulated/0/Android/data/org.srx.testapp/files"
                                    .to_string(),
                            ),
                            PathMapping::new(
                                "/storage/emulated/0/Download/Obb".to_string(),
                                "/storage/emulated/0/Android/obb/org.srx.testapp".to_string(),
                            ),
                        ],
                    },
                )]),
        ...
        let mappings = get_caller_mappings("org.srx.testapp", 10123);
        ...
        assert_eq!(
            ...
            vec![PathMapping::new(
                "/storage/emulated/0/Download/Media".to_string(),
                "/storage/emulated/0/Android/media/org.srx.testapp/cache".to_string(),
            )]
        );
    }

    #[test]
    fn mapped_target_read_only_respects_excludes() {
        let hub = SettingsHub::instance();
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([(
            "org.srx.testapp".to_string(),
            AppProfile {
                user_profiles: HashMap::from([(
                    ...
                    UserProfile {
                        ...
                        is_mapping_mode_only: false,
                        allowed_real_paths: Vec::new(),
                        excluded_real_paths: vec!["/storage/emulated/0/Pictures/MyApp".to_string()],
                        sandboxed_paths: Vec::new(),
                        read_only_paths: vec!["/storage/emulated/0/Pictures/MyApp".to_string()],
                        path_mappings: vec![PathMapping::new(
                            "/storage/emulated/0/DCIM/MyApp".to_string(),
    ...
    #[test]
    fn mapped_target_read_only_uses_mapped_destination_when_request_is_under_read_only_parent() {
        let hub = SettingsHub::instance();
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([(
            "org.srx.testapp".to_string(),
            AppProfile {
    ...
    #[test]
    fn mapped_target_inherits_parent_read_only_without_exclusion() {
        let hub = SettingsHub::instance();
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([(
            "org.srx.testapp".to_string(),
            AppProfile {
        ...
        assert_eq!(
            read_only_check_path_by_caller_paths(
                "/storage/emulated/0/Download/QQ/a.jpg",
                "org.srx.testapp",
                10123
            ),
    ...
    #[test]
    fn mapped_request_read_only_rule_does_not_make_target_read_only() {
        let hub = SettingsHub::instance();
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([(
            "org.srx.testapp".to_string(),
            AppProfile {
                user_profiles: HashMap::from([(
                    ...
                    UserProfile {
                        ...
                        excluded_real_paths: Vec::new(),
                        sandboxed_paths: Vec::new(),
                        read_only_paths: vec!["/storage/emulated/0/Download/QQ".to_string()],
                        path_mappings: vec![PathMapping::new(
                            "/storage/emulated/0/Download/QQ".to_string(),
                            "/storage/emulated/0/Download/ThirdParty/QQ".to_string(),
                        )],
    ...
    #[test]
    fn read_only_exclusion_overrides_parent_read_only_rule() {
        let hub = SettingsHub::instance();
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([(
            "org.srx.testapp".to_string(),
            AppProfile {
    ...
    #[test]
    fn download_provider_profile_fallback_fills_empty_fields_only() {
        let hub = SettingsHub::instance();
        let fallback_mapping = PathMapping::new(
            "/storage/emulated/0/.xlDownload".to_string(),
            "/storage/emulated/0/Download/.xlDownload".to_string(),
        );
        let (previous_apps, previous_loaded) = hub.replace_test_apps(HashMap::from([
            (
            ...
            (
                DOWNLOAD_PROVIDER_UI.to_string(),
                AppProfile {
                    user_profiles: HashMap::from([(
                        0,
                        UserProfile {
                            ...
                            excluded_real_paths: vec![
                                "/storage/emulated/0/Pictures/.thumbnails".to_string(),
                            ],
                            sandboxed_paths: vec!["/storage/emulated/0/.xlDownload".to_string()],
                            read_only_paths: Vec::new(),
                            path_mappings: vec![fallback_mapping.clone()],
                        },
                    )]),
                },
            ),
        ...
        let profile =
            get_effective_profile(hub, DOWNLOAD_PROVIDER, 10123).expect("profile should exist");
        assert_eq!(
            profile.allowed_real_paths,
        ...
        assert_eq!(
            ...
            vec!["/storage/emulated/0/Pictures/.thumbnails".to_string()]
        );
        assert_eq!(
            profile.sandboxed_paths,
            vec!["/storage/emulated/0/.xlDownload".to_string()]
        );
        assert_eq!(profile.path_mappings, vec![fallback_mapping]);
