...
mod tests {
    ...
    fn flow_for(package_name: &str, app_uid: i32) -> RuntimeFlow {
        let mut flow = RuntimeFlow::new();
        flow.package_name = package_name.to_string();
        flow.app_uid = app_uid;
        flow
    }

    #[test]
    fn ordinary_app_monitor_does_not_install_plt_hook() {
        let mut flow = flow_for("com.tencent.mobileqq", 10123);
        flow.should_monitor = true;

        assert!(!should_install_process_plt_hook(&flow, false));
    }

    #[test]
    fn ordinary_app_mount_redirect_does_not_install_plt_hook() {
        let mut flow = flow_for("com.tencent.mobileqq", 10123);
    ...
    #[test]
    fn media_provider_and_saf_bridge_can_install_monitor_plt_hook() {
        let mut media = flow_for("com.android.providers.media.module", 10217);
        media.should_monitor = true;
        assert!(should_install_process_plt_hook(&media, false));

        let mut external_storage = flow_for("com.android.externalstorage", 10073);
        external_storage.should_monitor = true;
        assert!(should_install_process_plt_hook(&external_storage, false));

        let mut downloads = flow_for("com.android.providers.downloads", 10067);
        downloads.should_monitor = true;
        assert!(should_install_process_plt_hook(&downloads, false));

        let mut mtp = flow_for("com.android.mtp", 10067);
        mtp.should_monitor = true;
        assert!(!should_install_process_plt_hook(&mtp, false));
    }

    #[test]
    fn shared_uid_process_name_does_not_install_plt_hook() {
        let mut flow = flow_for("android.process.media", 10067);
        flow.should_monitor = true;
    ...
    #[test]
    fn file_picker_ui_processes_do_not_install_monitor_plt_hook() {
        let mut documents = flow_for("com.android.documentsui", 10071);
        documents.should_monitor = true;
        documents.is_file_monitor_ui = true;
        assert!(!should_install_process_plt_hook(&documents, false));

        let mut photo_picker = flow_for("com.android.photopicker", 10304);
        photo_picker.should_monitor = true;
        photo_picker.is_file_monitor_ui = true;
        assert!(!should_install_process_plt_hook(&photo_picker, false));

        let mut download_ui = flow_for("com.android.providers.downloads.ui", 10067);
        download_ui.should_monitor = true;
        download_ui.is_file_monitor_ui = true;
        assert!(!should_install_process_plt_hook(&download_ui, false));
    }
}
