...
mod tests {
    ...
    #[test]
    fn parses_shared_full_profile_fixture() {
        let mut state = SettingsState::new();
        let fixture = include_str!("../../docs/config-fixtures/app-profile-full.json");

        assert!(parse_app_config(&mut state, "com.example", fixture));

        let user = state
            .apps
            .get("com.example")
            .and_then(|app| app.user_profiles.get(&0))
            .expect("fixture should load user 0");

        assert!(user.is_enabled);
        assert!(!user.is_mapping_mode_only);
        assert_eq!(
            user.allowed_real_paths,
            vec![
                "/storage/emulated/0/Documents/MyApp".to_string(),
                "/storage/emulated/0/Download/Public".to_string(),
            ]
        );
        assert_eq!(
            user.read_only_paths,
            vec!["/storage/emulated/0/Documents/MyApp".to_string()]
        );
        assert_eq!(
            user.excluded_real_paths,
            vec!["/storage/emulated/0/Download/Public/tmp".to_string()]
        );
        assert_eq!(
            user.sandboxed_paths,
            vec!["/storage/emulated/0/.xlDownload".to_string()]
        );
        assert!(user.path_mappings.iter().any(|mapping| {
            mapping.request_path == "/storage/emulated/0/DCIM/MyApp"
                && mapping.final_path == "/storage/emulated/0/Pictures/MyApp"
        }));
        assert!(user.path_mappings.iter().any(|mapping| {
            mapping.request_path == "/storage/emulated/0/Download/Cache"
                && mapping.final_path == "/storage/emulated/0/Android/media/com.example/cache"
        }));
    }

    #[test]
    fn skips_mapping_targets_under_android_data_or_obb() {
        let mut state = SettingsState::new();
        let config = r#"{
            "users": {
                "0": {
                    "enabled": true,
                    "path_mappings": {
                        "Download/Game": "Android/data/com.example.game/files",
                        "Download/Obb": "Android/obb/com.example.game",
                        "Download/Media": "Android/media/com.example.game/cache",
                        "DCIM/App": "Pictures/App"
                    }
                }
            }
        }"#;

        assert!(parse_app_config(&mut state, "com.example", config));

        let user = state
            .apps
            .get("com.example")
            .and_then(|app| app.user_profiles.get(&0))
            .expect("config should load user 0");

        assert_eq!(user.path_mappings.len(), 2);
        assert!(user.path_mappings.iter().any(|mapping| {
            mapping.request_path == "/storage/emulated/0/DCIM/App"
                && mapping.final_path == "/storage/emulated/0/Pictures/App"
        }));
        assert!(user.path_mappings.iter().any(|mapping| {
            mapping.request_path == "/storage/emulated/0/Download/Media"
                && mapping.final_path == "/storage/emulated/0/Android/media/com.example.game/cache"
        }));
    }

    #[test]
    fn parses_normalized_profile_fixture() {
        let mut state = SettingsState::new();
        let fixture =
            include_str!("../../docs/config-fixtures/app-profile-normalization-output.json");
        ...
        let user = state
            .apps
            .get("com.example")
            .and_then(|app| app.user_profiles.get(&10))
            .expect("fixture should load user 10");
        ...
        assert_eq!(
            user.allowed_real_paths,
            vec![
                "/storage/emulated/10/Documents/ReadOnly".to_string(),
                "/storage/emulated/10/Movies/Public".to_string(),
            ]
        );
        assert_eq!(
            user.read_only_paths,
            vec!["/storage/emulated/10/Documents/ReadOnly".to_string()]
        );
        assert_eq!(
            user.excluded_real_paths,
            vec![
                "/storage/emulated/10/Download/Legacy".to_string(),
                "/storage/emulated/10/Movies/Public/tmp".to_string(),
                "/storage/emulated/10/Pictures/Blocked".to_string(),
            ]
        );
        assert_eq!(
            user.sandboxed_paths,
            vec![
                "/storage/emulated/10/.xlDownload".to_string(),
                "/storage/emulated/10/Android/media/App".to_string(),
            ]
        );
        assert_eq!(user.path_mappings.len(), 2);
        assert!(user.path_mappings.iter().any(|mapping| {
            mapping.request_path == "/storage/emulated/10/Cache"
                && mapping.final_path == "/storage/emulated/10/Android/media/com.example/cache"
        }));
        assert!(user.path_mappings.iter().any(|mapping| {
            mapping.request_path == "/storage/emulated/10/DCIM/App"
                && mapping.final_path == "/storage/emulated/10/Pictures/App"
        }));
    }

    #[test]
    fn parses_read_only_paths_without_merging_them_into_allow_rules() {
        let mut state = SettingsState::new();
        let config = r#"{
            "users": {
                "0": {
                    "enabled": true,
                    "allowed_real_paths": ["Pictures"],
                    "excluded_real_paths": ["Secret"],
                    "read_only_paths": [
                        "Download/Public",
                        "Download/*",
                        "Secret"
        ...
        assert_eq!(
            user.excluded_real_paths,
            vec!["/storage/emulated/0/Secret".to_string()]
        );
    }

    #[test]
    fn parses_read_only_exclusion_rules_without_allowing_them() {
        let mut state = SettingsState::new();
        let config = r#"{
            "users": {
                "0": {
        ...
        assert_eq!(user.allowed_real_paths, Vec::<String>::new());
    }

    #[test]
    fn parses_read_only_wildcards_without_fuse_daemon_for_mount_fallback() {
        let mut state = SettingsState::new();
        let config = r#"{
            "users": {
                "0": {
    ...
    #[test]
    fn read_only_path_does_not_mutate_allowed_rules() {
        let mut state = SettingsState::new();
        let config = r#"{
            "users": {
                "0": {
                    "enabled": true,
                    "read_only_paths": ["DCIM"]
                }
            }
        }"#;
        ...
        assert_eq!(user.read_only_paths, vec!["/storage/emulated/0/DCIM"]);
        assert_eq!(user.allowed_real_paths, Vec::<String>::new());
    }
}
