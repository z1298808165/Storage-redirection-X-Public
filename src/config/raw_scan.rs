...
mod tests {
    ...
    #[test]
    fn parses_enabled_raw_user_profile_with_sandbox_paths() {
        let config = r#"{
            "users": {
                "10": {
        ...
        let entry = parse_raw_config_entry("com.example.app", 10, &path, 7, 1234);

        assert!(entry.has_config);
        assert!(entry.is_enabled);
        assert!(entry.is_mapping_mode_only);
        assert_eq!(
            entry.sandboxed_paths,
            vec![
                "/storage/emulated/10/Download/Tasks".to_string(),
                "/storage/emulated/10/Pictures".to_string(),
            ]
        );
    }

    #[test]
    fn parses_disabled_raw_user_profile_without_exposing_paths() {
        let config = r#"{
            "users": {
                "0": {
                    "enabled": false,
                    "mapping_mode_only": true,
                    "sandboxed_paths": ["Download"]
                }
            }
        }"#;
        let path = write_temp_config(config);

        let entry = parse_raw_config_entry("com.example.app", 0, &path, 9, 5678);

        assert!(entry.has_config);
        assert!(!entry.is_enabled);
        assert!(entry.is_mapping_mode_only);
        assert_eq!(entry.sandboxed_paths, vec!["/storage/emulated/0/Download"]);
    }

    #[test]
    fn raw_flags_require_present_and_enabled_config() {
        let enabled = CacheEntry {
        ...
        let disabled = CacheEntry {
            is_enabled: false,
            ..enabled.clone()
        };
        let missing = CacheEntry {
            has_config: false,
            ..enabled.clone()
        };

        let enabled_flags = raw_flags_from_cache_entry(&enabled);
        assert!(enabled_flags.is_enabled);
        assert!(enabled_flags.is_mapping_mode_only);

        let disabled_flags = raw_flags_from_cache_entry(&disabled);
        assert!(!disabled_flags.is_enabled);
        assert!(!disabled_flags.is_mapping_mode_only);

        let missing_flags = raw_flags_from_cache_entry(&missing);
        assert!(!missing_flags.is_enabled);
        assert!(!missing_flags.is_mapping_mode_only);
    }

    #[test]
    fn parse_raw_sandboxed_paths_filters_unsafe_roots_and_absolute_paths() {
        let value = json!([
            "Download",
            "Download/Nested",
            "Download/../Secret",
        ...
        let paths = parse_raw_sandboxed_paths(Some(&value), 0);
    ...
    #[test]
    fn parse_raw_sandboxed_paths_accepts_single_string() {
        let value = json!("Pictures/Screenshots");

        let paths = parse_raw_sandboxed_paths(Some(&value), 10);

        assert_eq!(
            paths,
            vec!["/storage/emulated/10/Pictures/Screenshots".to_string()]
        );
    }

    #[test]
    fn parse_raw_read_only_paths_keeps_wildcards_for_fallback() {
        let value = json!(["Download/Public", "Download/*", "Pictures/????"]);

        let paths = parse_raw_read_only_paths(Some(&value), 0);
    ...
    #[test]
    fn scans_apps_dir_for_any_enabled_raw_user_profile() {
        let config_dir = std::env::temp_dir().join(format!(
            "srx_raw_scan_any_enabled_{}_{}",
            std::process::id(),
            current_time_ms()
        ));
        ...
        std::fs::write(
            apps_dir.join("com.example.disabled.json"),
            r#"{
                "users": {
                    "0": {
                        "enabled": false,
                        "mapping_mode_only": true,
                        "sandboxed_paths": [".disabled"]
                    }
                }
            }"#,
        )
        ...
        std::fs::write(
            apps_dir.join("com.example.enabled.json"),
            r#"{
                "users": {
                    "0": {
                        "enabled": true,
