...
mod tests {
    ...
    #[test]
    fn path_list_equivalence_ignores_order_and_case() {
        let left = vec![
        ...
        assert!(path_lists_equal_ignore_case(&left, &right));
    }

    #[test]
    fn path_list_equivalence_preserves_duplicate_counts() {
        let left = vec![
            "/storage/emulated/0/DCIM".to_string(),
            "/storage/emulated/0/dcim".to_string(),
        ];
        let right = vec!["/storage/emulated/0/DCIM".to_string()];

        assert!(!path_lists_equal_ignore_case(&left, &right));
    }

    #[test]
    fn user_profile_equivalence_ignores_path_list_order() {
        let left = UserProfile {
            ...
            excluded_real_paths: vec!["/storage/emulated/0/Movies".to_string()],
            sandboxed_paths: vec![],
        ...
        assert!(is_user_profile_equivalent(&left, &right));
    }

    #[test]
    fn user_profile_equivalence_ignores_mapping_order_and_case() {
        let left = UserProfile {
            is_enabled: true,
            is_mapping_mode_only: false,
            allowed_real_paths: vec![],
