...
impl SettingsHub {
    ...
    #[cfg(test)]
    pub(crate) fn replace_test_apps(
        &self,
        apps: HashMap<String, AppProfile>,
    ) -> (HashMap<String, AppProfile>, bool) {
        let mut state = self.state.lock().unwrap_or_else(|err| err.into_inner());
        let previous_apps = std::mem::replace(&mut state.apps, apps);
        let previous_loaded = state.is_loaded;
        state.is_loaded = true;
        self.bump_config_version();
        (previous_apps, previous_loaded)
    }

    #[cfg(test)]
    pub(crate) fn restore_test_apps(
        &self,
        previous_apps: HashMap<String, AppProfile>,
    ...
    #[cfg(test)]
    pub(crate) fn replace_test_file_monitor_enabled(&self, enabled: bool) -> (bool, bool) {
        let mut state = self.state.lock().unwrap_or_else(|err| err.into_inner());
        let previous_enabled = state.is_file_monitor_enabled;
        let previous_loaded = state.is_loaded;
        state.is_file_monitor_enabled = enabled;
        state.is_loaded = true;
        self.bump_config_version();
        (previous_enabled, previous_loaded)
    }

    #[cfg(test)]
    pub(crate) fn restore_test_file_monitor_enabled(
        &self,
        previous_enabled: bool,
        previous_loaded: bool,
    ) {
        let mut state = self.state.lock().unwrap_or_else(|err| err.into_inner());
        state.is_file_monitor_enabled = previous_enabled;
        state.is_loaded = previous_loaded;
        self.bump_config_version();
    }

    #[cfg(test)]
    pub(crate) fn replace_test_fuse_fix_enabled(&self, enabled: bool) -> (bool, bool) {
        let mut state = self.state.lock().unwrap_or_else(|err| err.into_inner());
        let previous_enabled = state.is_fuse_fix_enabled;
        let previous_loaded = state.is_loaded;
        state.is_fuse_fix_enabled = enabled;
        state.is_loaded = true;
        self.bump_config_version();
        (previous_enabled, previous_loaded)
    }
    ...
    #[cfg(test)]
    pub(crate) fn restore_test_fuse_fix_enabled(
        &self,
        previous_enabled: bool,
        previous_loaded: bool,
    ) {
        let mut state = self.state.lock().unwrap_or_else(|err| err.into_inner());
        state.is_fuse_fix_enabled = previous_enabled;
        state.is_loaded = previous_loaded;
        self.bump_config_version();
    }

    #[cfg(test)]
    pub(crate) fn replace_test_fuse_daemon_redirect_enabled(&self, enabled: bool) -> (bool, bool) {
        let mut state = self.state.lock().unwrap_or_else(|err| err.into_inner());
        let previous_enabled = state.is_fuse_daemon_redirect_enabled;
        let previous_loaded = state.is_loaded;
        state.is_fuse_daemon_redirect_enabled = enabled;
        state.is_loaded = true;
        self.bump_config_version();
        (previous_enabled, previous_loaded)
    }
    ...
    #[cfg(test)]
    pub(crate) fn restore_test_fuse_daemon_redirect_enabled(
        &self,
        previous_enabled: bool,
        previous_loaded: bool,
    ) {
        let mut state = self.state.lock().unwrap_or_else(|err| err.into_inner());
        state.is_fuse_daemon_redirect_enabled = previous_enabled;
        state.is_loaded = previous_loaded;
        self.bump_config_version();
    }

    #[cfg(test)]
    pub(crate) fn replace_test_config_dir(&self, config_dir: String) -> String {
        let mut state = self.state.lock().unwrap_or_else(|err| err.into_inner());
        let previous_config_dir = std::mem::replace(&mut state.config_dir, config_dir);
        self.bump_config_version();
        previous_config_dir
    }
}
