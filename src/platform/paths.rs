...
mod tests {
    ...
    #[test]
    fn matches_plain_prefix_without_wildcards() {
        assert!(matches(
        ...
        assert!(!matches(
            "/storage/emulated/0/Download",
            "/storage/emulated/0/DownloadExtra/file.txt",
            true
        ));
        assert!(!matches("/", "/", true));
    }
    ...
    #[test]
    fn matches_plain_prefix_ignores_path_case() {
        assert!(matches(
            "/storage/emulated/0/Download/AppBucket",
            "/storage/emulated/0/download/appbucket/file.txt",
            true
        ));
        assert!(!matches(
            "/storage/emulated/0/Download/AppBucket",
            "/storage/emulated/0/download/appbucketExtra/file.txt",
            true
        ));
    }
    ...
    fn matches_wildcard_segments() {
        assert!(matches(
            "/storage/emulated/0/Android/.android_*",
    ...
    #[test]
    fn matches_wildcard_segments_ignore_path_case() {
        assert!(matches(
            "/storage/emulated/0/Android/.Android_*",
            "/storage/emulated/0/android/.android_lq/cache.db",
            true
        ));
    }

    #[test]
    fn wildcard_mount_fallback_parent_uses_nearest_concrete_parent() {
        assert_eq!(
            wildcard_mount_fallback_parent(
                "/storage/emulated/0/Download/Team*/Deep",
                "/storage/emulated/0",
            ),
        ...
        assert_eq!(
            wildcard_mount_fallback_parent(
                "/storage/emulated/0/Download/TeamA/*/Deep",
                "/storage/emulated/0",
            ),
            Some("/storage/emulated/0/Download/TeamA".to_string())
        );
        assert_eq!(
            wildcard_mount_fallback_parent("/storage/emulated/0/*/Deep", "/storage/emulated/0"),
            None
        );
        assert_eq!(
            wildcard_mount_fallback_parent(
                "/storage/emulated/10/Download/Team*/Deep",
                "/storage/emulated/0",
            ),
            None
        );
    ...
    #[test]
    fn wildcard_policy_fallback_parent_keeps_file_name_wildcards_precise() {
        assert_eq!(
            wildcard_policy_fallback_parent(
                "/storage/emulated/0/Download/srt_qmark_?.txt",
                "/storage/emulated/0",
            ),
            None
        );
    ...
    #[test]
    fn same_or_child_matches_path_boundary() {
        ...
        assert!(!is_same_or_child(
            "/storage/emulated/0/Download",
            "/storage/emulated/0/DownloadExtra"
        ));
        assert!(!is_same_or_child("", "/storage/emulated/0/Download"));
        assert!(!is_same_or_child("/storage/emulated/0/Download", ""));
    }

    #[test]
    fn same_or_child_ignores_path_case() {
        assert!(is_same_or_child(
            "/storage/emulated/0/download/file.txt",
            "/storage/emulated/0/Download"
        ));
    ...
    #[test]
    fn child_suffix_matches_exact_or_child_path() {
        assert_eq!(
            child_suffix(
                "/storage/emulated/0/Download",
                "/storage/emulated/0/Download"
        ...
        assert_eq!(
            ...
            Some("Download/file.txt")
        );
        assert_eq!(
            relative_child_path("/storage/emulated/0", "/storage/emulated/0"),
            Some("")
        );
        assert_eq!(
            relative_child_path(
    ...
    #[test]
    fn child_suffix_ignores_root_case_and_preserves_actual_suffix() {
        assert_eq!(
            child_suffix(
                "/storage/emulated/0/download/AppBucket/File.txt",
                "/storage/emulated/0/Download"
            ),
            Some("/AppBucket/File.txt")
        );
    }

    #[test]
    fn sort_dedup_paths_case_insensitive_keeps_canonical_first_order() {
        let mut paths = vec![
            "/storage/emulated/0/Beta".to_string(),
            "/storage/emulated/0/alpha".to_string(),
            "/storage/emulated/0/Alpha".to_string(),
            "/storage/emulated/0/beta/Child".to_string(),
        ];

        sort_dedup_paths_case_insensitive(&mut paths);
    ...
    #[test]
    fn sort_dedup_paths_longest_first_case_insensitive_prefers_specific_paths() {
        let mut paths = vec![
        ...
        sort_dedup_paths_longest_first_case_insensitive(&mut paths);
    ...
    #[test]
    fn converts_storage_and_data_media_paths() {
        assert_eq!(
            data_media_to_storage_path("/data/media/0/DCIM/a.jpg"),
            "/storage/emulated/0/DCIM/a.jpg"
        );
        assert_eq!(
            storage_to_data_media_path("/storage/emulated/10/Download/a.txt"),
            "/data/media/10/Download/a.txt"
        );
        assert_eq!(
            storage_to_data_media_for_user("/storage/emulated/0/Download/a.txt", 0).as_deref(),
            Some("/data/media/0/Download/a.txt")
        );
        assert_eq!(
            storage_relative_path_for_user("/storage/emulated/10/Pictures/a.jpg", 10).as_deref(),
            Some("Pictures/a.jpg")
        );
        assert_eq!(
            storage_to_data_media_for_user("/storage/emulated/0", 0).as_deref(),
            None
        );
        assert_eq!(
            storage_to_data_media_for_user("/storage/emulated/10/Download/a.txt", 0).as_deref(),
            None
        );
        assert_eq!(
            storage_relative_path_for_user("/storage/emulated/10", 10).as_deref(),
            None
        );
        assert_eq!(
            storage_relative_path_for_user("/storage/emulated/0/Download/a.txt", 10).as_deref(),
            None
        );
        assert_eq!(
            data_media_user_root("/data/media/0/Download/a.txt").as_deref(),
            Some("/data/media/0")
        );
        assert_eq!(
            storage_user_root("/storage/emulated/10/Pictures/a.jpg").as_deref(),
            Some("/storage/emulated/10")
        );
        assert!(storage_user_root("/storage/emulated//Pictures/a.jpg").is_none());
        assert!(data_media_user_root("/data/media//Download/a.txt").is_none());
        assert_eq!(
            data_media_to_storage_path("/data/mediax/0/DCIM/a.jpg"),
            "/data/mediax/0/DCIM/a.jpg"
        );
        assert_eq!(
            storage_to_data_media_path("/storage/emulatedx/0/Download/a.txt"),
            "/storage/emulatedx/0/Download/a.txt"
        );
    }

    #[test]
    fn resolves_android_private_data_media_root() {
        assert_eq!(
            android_private_data_media_root(
    ...
    #[test]
    fn xldownload_alias_matches_case_variants() {
        assert!(matches_xldownload_alias(
    ...
    #[test]
    fn extracts_android_private_path_owner() {
        assert_eq!(
            extract_android_private_path_owner(
                "/storage/emulated/0/Android/media/xyz.nextalone.nnngram/Nnngram/a.jpg"
            ),
            "xyz.nextalone.nnngram"
        );
        assert_eq!(
            extract_android_private_path_owner(
                "/storage/emulated/10/Android/data/com.eg.android.AlipayGphone/files/a.log"
            ),
            "com.eg.android.AlipayGphone"
        );
        assert_eq!(
            extract_android_private_path_owner("/storage/emulated/0/Android/obb/com.demo.app"),
            "com.demo.app"
        );
    }

    #[test]
    fn rejects_non_private_path_owner() {
        assert!(
            extract_android_private_path_owner("/storage/emulated/0/Download/a.txt").is_empty()
        );
        assert!(extract_android_private_path_owner("/storage/emulated/0/Android/media").is_empty());
        assert!(
            extract_android_private_path_owner("/storage/emulated/0/Android/media/notpackage/a")
                .is_empty()
        );
        assert!(
            extract_android_private_path_owner("/storage/emulated/0/Android/media/.nomedia")
                .is_empty()
        );
        assert!(
            extract_android_private_path_owner("/storage/emulated/0/Android/media/com.demo./a")
                .is_empty()
        );
        assert!(
            extract_android_private_path_owner("/data/media/0/Android/media/com.demo/a").is_empty()
        );
    }

    #[test]
    fn detects_sqlite_database_and_sidecar_paths() {
        assert!(is_sqlite_database_or_sidecar_path(
            "/storage/emulated/0/Android/media/com.demo/app.db"
        ));
        assert!(is_sqlite_database_or_sidecar_path(
            "/storage/emulated/0/Android/media/com.demo/app.DB-SHM"
        ));
        assert!(is_sqlite_database_or_sidecar_path(
            "/storage/emulated/0/Android/media/com.demo/app.sqlite3-wal"
        ));
        assert!(!is_sqlite_database_or_sidecar_path(
            "/storage/emulated/0/Android/media/com.demo/cache.bin"
        ));
        assert!(!is_sqlite_database_or_sidecar_path(
            "/storage/emulated/0/Android/media/com.demo/db-shm.txt"
        ));
    }

    #[test]
    fn detects_android_data_and_obb_paths() {
        assert!(is_android_data_or_obb_path(
            "/storage/emulated/0/Android/data/com.demo.app/files"
        ));
        assert!(is_android_data_or_obb_path(
            "/data/media/10/Android/obb/com.demo.app/main.obb"
        ));
        assert!(is_android_data_or_obb_path(
            "/storage/emulated/0/Android/data"
        ));
        assert!(!is_android_data_or_obb_path(
            "/storage/emulated/0/Android/media/com.demo.app/file"
        ));
        assert!(!is_android_data_or_obb_path(
            "/storage/emulated/0/Android/datax/com.demo.app/file"
        ));
        assert!(!is_android_data_or_obb_path(
            "/storage/emulated/0/Download/Android/data/com.demo.app/file"
        ));
    }
}
