...
mod tests {
    ...
    #[test]
    fn prewarm_runs_only_reload_or_existing_mount_state() {
        let reload = plan(MountOperation::Reload, false);
        let disable_with_state = plan(MountOperation::Disable, true);
        let disable_without_state = plan(MountOperation::Disable, false);
    ...
    #[test]
    fn prewarm_prioritizes_new_reload_before_cleanup() {
        let mut plans = vec![
            plan(MountOperation::Disable, false),
            plan(MountOperation::Disable, true),
            plan(MountOperation::Reload, true),
            plan(MountOperation::Reload, false),
        ];

        plans.sort_by_key(|plan| plan.priority());

        assert_eq!(plans[0].request.operation, MountOperation::Reload);
        assert!(!plans[0].has_mount_state);
        assert_eq!(plans[1].request.operation, MountOperation::Reload);
        assert!(plans[1].has_mount_state);
        assert_eq!(plans[2].request.operation, MountOperation::Disable);
        assert!(plans[2].has_mount_state);
        assert_eq!(plans[3].request.operation, MountOperation::Disable);
        assert!(!plans[3].has_mount_state);
    }

    #[test]
    fn config_change_requests_prewarm() {
        assert!(should_prewarm_reconcile(0, false, 1, 1, 1));
        assert!(should_prewarm_reconcile(5, true, 1, 1, 1));
