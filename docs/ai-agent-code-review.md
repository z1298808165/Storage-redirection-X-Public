...
```text
变更：一句话说明实现了什么用户可见行为（一般不超过 50 字，单行）
用户影响：同上；生成器优先采用本字段，需要保留更技术性的 `变更：` 时用它承载精简句
范围：短语级说明适用组件、调用方或边界（一般不超过 20 字，单行）
限制：短语级说明校验、回退或兼容约束（一般不超过 20 字，单行）
验证：列出实际运行的检查或测试及结果（不会进入更新日志）
```
...
进入更新日志的字段必须精简：`变更：`/`用户影响：` 只能是一句面向用户的简体中文短句，不得包含文件路径、代码符号、内部模块名、实现细节或验证过程；`范围：`、`限制：` 只能是短语级概括，不得罗列文件清单。技术细节、涉及文件和验证过程一律写入 `验证：` 或不带字段前缀的正文段落。生成器优先采用 `用户影响：`，缺失时才回退 `变更：`；同一正文两者并存时 `变更：` 不会进入更新日志，可在其中保留更技术性的说明。
...
- `content_accuracy_and_user_messaging`：Commit 标题/正文和更新说明均来自实际 diff，准确描述用户可见行为、适用范围、边界限制和验证结果；没有夸大能力、遗漏限制或把测试过程写成产品行为。进入更新日志的字段（`变更：`/`用户影响：`/`范围：`/`限制：`）保持一句话或短语级精简，不含文件路径、代码符号、内部模块名或验证过程。
