...
- `storage.redirect.x-v<version>.zip`：日常使用和问题定位共用。默认只记录文件监控日志；需要排查问题时，在设置页“模块设置”中打开“详细日志”，模块会立即启用普通 `StorageRedirect`/`SRX`/`Stats` logcat 输出以及 running/media/app/status/stats 诊断采集，关闭后立即停止这些记录。
...
zip -r "../../storage.redirect.x-v${VERSION}.zip" .
```
