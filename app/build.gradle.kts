...
val cargoVersion = rootProject.file("Cargo.toml")
    .readText()
    .let { content ->
        Regex("""(?m)^version\s*=\s*"([^"]+)"""")
            .find(content)
...
fun headCargoVersion(): String? =
    runGit("show", "HEAD:Cargo.toml")?.let(::cargoVersionFromText)

fun versionStartCommit(version: String): String? {
    val commits = runGit("rev-list", "--first-parent", "--reverse", "HEAD", "--", "Cargo.toml")
        ?.lineSequence()
...
fun ciVersionCode(baseVersion: String, buildCount: Int): Int {
    ...
    if (buildCount !in 1..99) {
        throw GradleException("CI build count must be between 1 and 99. Bump Cargo.toml version before continuing.")
    }
...
fun resolveBuildVersion(baseVersion: String, includeDirty: Boolean): ResolvedBuildVersion {
    ...
    var count = if (start == null) {
        0
    } else {
        runGit("rev-list", "--first-parent", "--reverse", "$start^..HEAD")
            ?.lineSequence()
            ?.filter { it.isNotBlank() }
            ?.count { !isAutoManifestCommit(it) }
            ?: 0
    }
...
val configuredAppVersionName = providers.environmentVariable("VERSION").orNull
    ?: providers.gradleProperty("srx.versionName").orNull
val configuredAppVersionCode = providers.environmentVariable("VERSION_CODE").orNull
...
val appVersionCode = configuredAppVersionCode?.toIntOrNull()
...
fun githubRepositoryFromRemote(remoteUrl: String?): String? {
    ...
    return patterns.firstNotNullOfOrNull { pattern ->
        pattern.matchEntire(normalized)?.let { match -> "${match.groupValues[1]}/${match.groupValues[2]}" }
    }
}
...
val releaseBranch = (
    providers.gradleProperty("srx.releaseBranch").orNull
        ...
        ?: providers.environmentVariable("GITHUB_REF_NAME").orNull
            ?.takeIf { providers.environmentVariable("GITHUB_REF_TYPE").orNull == "branch" }
...
val configuredSigningPropertiesFile = providers.environmentVariable("SRX_APP_SIGNING_PROPERTIES").orNull
    ?: providers.gradleProperty("srx.signing.propertiesFile").orNull
val defaultUserSigningPropertiesFile = File(
    System.getProperty("user.home"),
    ".srx_core/signing/keystore.properties",
)
val signingPropertiesFileCandidates = listOfNotNull(
...
fun signingValue(propertyName: String, envName: String, gradlePropertyName: String): SigningSetting? =
    providers.environmentVariable(envName).orNull.takeIfNotBlank()
        ?.let { SigningSetting(it, rootProject.projectDir) }
        ?: providers.gradleProperty(gradlePropertyName).orNull.takeIfNotBlank()
            ?.let { SigningSetting(it, rootProject.projectDir) }
        ?: signingProperties.getProperty(propertyName).takeIfNotBlank()
            ?.let { SigningSetting(it, signingPropertiesFile?.parentFile ?: rootProject.projectDir) }
...
val releaseStoreFileSetting = signingValue("storeFile", "SRX_APP_SIGNING_STORE_FILE", "srx.signing.storeFile")
...
val releaseKeyAlias = signingValue("keyAlias", "SRX_APP_SIGNING_KEY_ALIAS", "srx.signing.keyAlias")?.value
val releaseKeyPassword = signingValue(
    "keyPassword",
    "SRX_APP_SIGNING_KEY_PASSWORD",
    "srx.signing.keyPassword",
)?.value
val hasReleaseSigningValues = listOf(
...
android {
    ...
    defaultConfig {
        ...
        buildConfigField("String", "OFFICIAL_RELEASE_REPOSITORY", buildConfigString(officialReleaseRepository))
...
val syncDebugUnitTestClasspathForArgfile by tasks.registering(Sync::class) {
    ...
    into("runtime") {
        ...
        from(layout.buildDirectory.file("intermediates/compile_and_runtime_r_class_jar/debug/processDebugResources/R.jar"))
    }
    into("java-res") {
