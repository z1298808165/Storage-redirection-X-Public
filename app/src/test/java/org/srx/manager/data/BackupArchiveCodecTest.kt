...
class BackupArchiveCodecTest {
    ...
    fun zippedPayloadKeepsIntegrityDigestStable() {
        val data = SrxConfigNormalizer.normalizeBackupData(
            BackupData(
        ...
        val decoded = BackupArchiveCodec.decode(BackupArchiveCodec.encodeZip(json.encodeToString(payload) + "\n"))
        val decodedPayload = json.decodeFromString<BackupPayload>(decoded)

        assertTrue(decodedPayload.integrity.value in SrxConfigNormalizer.backupDigestCandidates(json, decodedPayload.data))
    }
