...
class RootAppQueryTest {
    ...
    fun listUsersParsesDistinctUserIds() = runBlocking {
        ...
        assertEquals("cmd user list 2>/dev/null || pm list users 2>/dev/null", shell.invocations.single().command)
    }
    ...
    @Test
    fun loadDexAppLabelsRejectsUnsafeUserIdWithoutShellCall() = runBlocking {
        val shell = CapturingShell()
        val query = RootAppQuery(shell)
    ...
    fun loadDexAppLabelsRunsDexAndFiltersUnsafePackages() = runBlocking {
        ...
        assertTrue(shell.invocations[0].command.contains("--user 10 > ${shellQuote(ListAppsOutputPath)}"))
