    ...
    CenteredDialog(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                GlassTextButton(confirmText, { onConfirm(name.trim()) }, modifier = Modifier.weight(1f), primary = true)
            }
        }
    }
}
...
private fun AppConfigHeader(
    ...
    Box(
        ...
        val useBackdrop = isSrxLiquidGlassEnabled() && isSrxBlurEffectEnabled() && LocalSrxBackdrop.current != null
        Card(
            modifier = Modifier
                .fillMaxWidth()
                .floatingGlassPanel(shape),
            cornerRadius = 24.dp,
            insideMargin = PaddingValues(horizontal = 10.dp, vertical = 10.dp),
            colors = CardDefaults.defaultColors(
                color = if (useBackdrop) Color.Transparent else MiuixTheme.colorScheme.surfaceContainerHigh,
            ),
        ) {
            Row(
                modifier = Modifier.fillMaxWidth(),
...
internal fun AppConfigScreen(
    ...
    var readOnlyEditorEnabled by rememberSaveable(app.packageName, state.selectedUser) {
    ...
    val headerBackdrop = remember(baseBackdrop, pageContentBackdrop) {
    ...
    Scaffold(
        ...
        Box(
            modifier = Modifier
                .fillMaxSize()
                .then(if (baseBackdrop != null) Modifier.layerBackdrop(pageContentBackdrop) else Modifier),
        ) {
            LazyColumn(
                modifier = Modifier
                    .fillMaxSize()
                ...
                item {
                    GlassCard(alpha = 0.52f, shadowAlpha = 0.14f) {
                        ...
                        CompactSwitchRow(
                            ...
                            onCheckedChange = { checked -> onProfileChange { it.copy(mappingModeOnly = checked) } },
                        )
                        CompactSwitchRow(
                            title = "只读模式",
    ...
    CenteredDialog(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            GlassTextButton("删除", { confirmDelete = false; onDelete() }, modifier = Modifier.weight(1f), danger = true)
        }
    }
