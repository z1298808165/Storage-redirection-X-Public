...
internal class BgEffectPainter {
    val runtimeShader by lazy {
        RuntimeShader(OS3_BG_FRAG).also(::initStaticUniforms)
    }
    ...
    fun updateBoundIfNeeded(effectHeight: Float, totalHeight: Float, totalWidth: Float) {
        if (cachedLogoHeight == effectHeight && cachedTotalHeight == totalHeight && cachedTotalWidth == totalWidth) return
