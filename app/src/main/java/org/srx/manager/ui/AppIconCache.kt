...
object AppIconCache {
    ...
    private val cacheSizeKb = (Runtime.getRuntime().maxMemory() / 1024 / 16)
        .toInt()
        .coerceIn(MinCacheKb, MaxCacheKb)
    ...
    suspend fun load(context: Context, info: ApplicationInfo, size: Int): Bitmap = semaphore.withPermit {
        synchronized(cache) { cache.get(key(info)) }?.let { return@withPermit it }
    ...
    private fun key(info: ApplicationInfo): String = "${info.packageName}:${info.uid}:${info.sourceDir}"
}
