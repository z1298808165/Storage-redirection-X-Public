...
class RootModuleController(
    ...
    suspend fun restartMediaProvider(): Boolean {
        ...
        return result.isSuccess && waitForMediaProviderRestart(before, timeoutMs = 15_000L, intervalMs = 250L)
    }
    ...
    suspend fun version(): String =
        shell.exec(readModuleVersionCommand()).stdout.trim()
    ...
    private fun buildEnsureLogCollectorsCommand(): String =
        withSrxCtlFallback(
            "ensure-collectors",
            "if [ -r ${shellQuote("$ModuleDir/service.sh")} ]; then /system/bin/sh ${shellQuote("$ModuleDir/service.sh")} >/dev/null 2>&1 & fi",
        )
    ...
    private fun withSrxCtlFallback(action: String, fallback: String): String =
        "if [ -r ${shellQuote(SrxCtlPath)} ]; then /system/bin/sh ${shellQuote(SrxCtlPath)} $action; else $fallback; fi"

    private fun mediaProviderPackages(): String =
        MediaProviderPackages.joinToString(" ")

    private companion object {
        val MediaProviderPackages = listOf(
