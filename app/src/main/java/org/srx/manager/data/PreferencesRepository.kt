...
class PreferencesRepository(private val context: Context) {
    ...
    val uiPreferences: Flow<UiPreferences> = context.dataStore.data.map { prefs ->
        UiPreferences(
            floatingBottomBar = prefs[floatingBottomBarKey] ?: true,
            liquidGlass = prefs[liquidGlassKey] ?: true,
            blurEffect = prefs[blurEffectKey] ?: ((prefs[legacyLiquidGlassBlurKey] ?: 0.7f) > 0.01f),
            dynamicColor = prefs[dynamicColorKey] ?: false,
            accentColor = prefs[accentColorKey] ?: 0,
            colorStyle = prefs[colorStyleKey]?.let { runCatching { UiColorStyle.valueOf(it) }.getOrNull() } ?: UiColorStyle.TonalSpot,
            colorSpec = prefs[colorSpecKey]?.let { runCatching { UiColorSpec.valueOf(it) }.getOrNull() } ?: UiColorSpec.Spec2025,
            themeMode = prefs[themeModeKey]?.let { runCatching { UiThemeMode.valueOf(it) }.getOrNull() } ?: UiThemeMode.System,
            ...
            updateChannel = prefs[updateChannelKey]?.let { runCatching { UpdateChannel.valueOf(it) }.getOrNull() } ?: UpdateChannel.Stable,
        )
    }
