...
class RootFileStore(
    ...
    suspend fun read(path: String): String =
        shell.exec("cat ${shellQuote(path)} 2>/dev/null").stdout
    ...
    suspend fun clearFileMonitorLog(): Boolean {
        val result = shell.exec("mkdir -p ${shellQuote(LogsDir)} && : > ${shellQuote(FileMonitorLogPath)}")
        return result.isSuccess
    }
    ...
    private suspend fun createDiagnosticArchiveWithProgress(
        ...
        try {
            val start = shell.exec(
                buildDiagnosticArchiveStartCommand(stage, archive, progress, done, runLog, pid, worker),
                timeoutMs = 10_000L,
            )
            ...
            val exists = shell.exec("[ -s ${shellQuote(archive)} ] && echo 1 || echo 0", timeoutMs = 10_000L)
                .stdout
                .trim() == "1"
            keepArchive = exists
            ...
            val cleanupPaths = if (keepArchive) {
    ...
    private fun buildDiagnosticArchiveCommand(stage: String, archive: String, progress: String? = null): String {
        val stageQ = shellQuote(stage)
        val archiveQ = shellQuote(archive)
        val progressArg = progress?.let { " ${shellQuote(it)}" }.orEmpty()
        val scriptCommand = "if [ -r ${shellQuote(DiagnosticArchiveScriptPath)} ]; then " +
    ...
    private fun buildDiagnosticArchiveStartCommand(
        ...
        return managedTempCleanupCommand(stage, archive, progress, "$progress.tmp", done, runLog, pid, worker) + "; " +
    ...
    private fun progressCommand(progress: String?, percent: Int, phase: String, message: String): String {
