...
class RootStorageBrowser(
    ...
    suspend fun listDirectories(userId: String, dirRel: String): List<String> {
        ...
        val target = if (clean.isBlank()) "/storage/emulated/$userId" else "/storage/emulated/$userId/$clean"
        ...
        val command = buildString {
            ...
            append("; do [ -d \"\$dir\" ] || continue; out=$(ls -1Ap \"\$dir\" 2>/dev/null); [ -z \"\$out\" ] || { printf '%s\n' \"\$out\"; exit 0; }; done")
        }
        ...
        return out.lineSequence()
            ...
            .sortedWith(compareBy<String> { !it.endsWith("/") }.thenBy(String.CASE_INSENSITIVE_ORDER) { it.trimEnd('/') })
            .toList()
    }
