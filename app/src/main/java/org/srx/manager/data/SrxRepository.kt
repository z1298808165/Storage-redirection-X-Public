...
class SrxRepository(
      ...
      withContext(Dispatchers.IO) {
        val safeName =
            fileName.replace(Regex("[\\\\/:*?\"<>|\\u0000-\\u001f]"), "_").ifBlank {
              "storage-redirect-x-logs.tar.gz"
            }
        ...
        try {
          onProgress?.invoke(DiagnosticArchiveProgress(99, "copy", "正在写入目标文件"))
          val ok =
              exportDiagnosticArchiveToDocumentTree(archivePath, directoryUri, safeName) ||
  ...
  private fun exportDiagnosticArchiveToDocumentTree(
      archivePath: String,
      directoryUri: Uri,
      fileName: String,
  ): Boolean {
  ...
  private suspend fun copyDiagnosticArchiveToPrimaryTree(
      archivePath: String,
      directoryUri: Uri,
      fileName: String,
  ): Boolean {
