...
public final class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (ApplicationInfo app : apps) {
            ...
            try { label = pm.getApplicationLabel(app); } catch (Throwable ignored) {}
    ...
    private static Context getSystemContext() throws Exception {
        ...
        if (getSystemContext == null) throw new NoSuchMethodException("ActivityThread.getSystemContext");
    ...
    private static List<ApplicationInfo> getInstalledApplications(PackageManager pm, int userId) {
        if (userId >= 0) {
            try {
                Method asUser = findMethod(pm.getClass(), "getInstalledApplicationsAsUser", int.class, int.class);
                if (asUser != null) {
            ...
            try {
                ...
                Method asUser = findMethod(pm.getClass(), "getInstalledApplicationsAsUser", flagsClass, int.class);
                if (asUser != null) {
                    asUser.setAccessible(true);
    ...
    private static Method findMethod(Class<?> cls, String name, Class<?>... params) {
        for (Class<?> current = cls; current != null; current = current.getSuperclass()) {
            try { return current.getDeclaredMethod(name, params); } catch (NoSuchMethodException ignored) {}
        }
        return null;
    }
